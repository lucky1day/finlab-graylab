...
PREDICTION_CORRECTION_EVIDENCE_SCHEMA = "prediction-correction-evidence-v1"
PREDICTION_CORRECTION_REASON = "source_data_reconciliation_v1"
PREDICTION_CORRECTION_ATTESTATION_ID = "ecs-mac3-alignment-20260824-v1"
PREDICTION_CORRECTION_ATTESTATION_PATH = (
    Path(__file__).resolve().parents[1]
    / "source_evidence"
    / "prediction_corrections"
    / "ecs_mac3_alignment_20260824_v1.json"
)
PREDICTION_CORRECTION_ATTESTATION_SHA256 = (
    "08dc4c446eb62794ca38dfb4aed5f3871"
    "9270ccccd79389a0fca796924fb269a"
)
...
class PredictionCorrection:
    """一条已发布 live Prediction 的 append-only 展示纠错。"""
...
def insert_prediction_corrections_atomic(
    engine: Engine,
    corrections: Iterable[PredictionCorrection],
) -> int:
    """原子追加一组 live 展示纠错，不修改原 Prediction。"""
    rows = sorted(
        (_normalize_prediction_correction(item) for item in corrections),
        key=lambda row: (
            str(row["scheme_id"]),
            str(row["target_tenor"]),
            int(row["horizon"]),
    ...
    if not rows:
        raise ValueError("prediction corrections must not be empty")
    keys = [
        (
            row["scheme_id"],
            row["target_tenor"],
    ...
    if len(keys) != len(set(keys)):
        raise ValueError("prediction corrections contain duplicate business keys")
    if len({row["created_by"] for row in rows}) != 1:
        raise ValueError("prediction corrections must share one created_by")

    with engine.begin() as conn:
        authority = _prediction_correction_authority(conn)
        insert_rows: list[dict[str, object]] = []
        for row in rows:
            _validate_prediction_correction_authority(row, authority)
            prediction = _select_mapping_one_or_none(
                conn,
                """
            ...
            if prediction is None:
                raise RuntimeError(
                    "prediction correction source row is missing: "
                    f"{row['scheme_id']}/{row['target_tenor']}/"
                    f"h{row['horizon']}/{row['target_date']}"
                )
            _validate_prediction_correction_source(prediction, row)
            registry = _select_mapping_one_or_none(
                conn,
                """
                SELECT scheme_id, base_scheme_id, target_tenor, horizon, status
                FROM t_scheme_registry
                WHERE scheme_id = :registry_scheme_id
                """,
                row,
                for_update=True,
            )
            ...
            if registry is None or any(
                str(registry.get(field)) != str(value)
                for field, value in expected_registry.items()
            ):
                raise RuntimeError(
                    "prediction correction requires the exact active Registry row: "
                    f"expected={expected_registry} actual={dict(registry or {})}"
                )
            ...
            if existing:
                raise RuntimeError(
                    "prediction correction already exists: "
                    f"prediction_id={prediction_id}"
                )
            insert_rows.append({**row, "prediction_id": prediction_id})

        sqlite = _dialect_name(conn) == "sqlite"
        evidence_expression = ":evidence" if sqlite else "CAST(:evidence AS JSON)"
        conn.execute(
            text(
                f"""
                INSERT INTO t_scheme_prediction_corrections
                    (prediction_id, scheme_id, target_tenor, horizon,
                     predict_date, feature_date, target_date,
                     original_direction, corrected_direction,
                     scheme_version, prediction_phase, correction_reason,
                     operation_id, operation_scope_sha256, release_commit,
                     evidence, created_by)
                VALUES
                    (:prediction_id, :scheme_id, :target_tenor, :horizon,
                     :predict_date, :feature_date, :target_date,
                     ...
                     :scheme_version, :prediction_phase, :correction_reason,
                     :operation_id, :operation_scope_sha256, :release_commit,
                     {evidence_expression}, :created_by)
                """
            ),
            insert_rows,
        )
...
def _normalize_prediction_correction(
    correction: PredictionCorrection,
) -> dict[str, object]:
    if not isinstance(correction, PredictionCorrection):
        raise TypeError("correction must be PredictionCorrection")
    ...
    horizon = correction.horizon
    if type(horizon) is not int or horizon <= 0:
        raise ValueError("horizon must be a positive integer")
    original = correction.original_direction
    corrected = correction.corrected_direction
    if type(original) is not int or original not in {-1, 0, 1}:
        raise ValueError("original_direction must be -1, 0, or 1")
    if type(corrected) is not int or corrected not in {-1, 0, 1}:
        raise ValueError("corrected_direction must be -1, 0, or 1")
    if original == corrected:
        raise ValueError("prediction correction must change direction")
    phase = bounded(correction.prediction_phase, "prediction_phase", 32)
    if phase not in VALID_PREDICTION_PHASES:
        raise ValueError(
            f"prediction_phase must be one of {sorted(VALID_PREDICTION_PHASES)}"
        )
    reason = bounded(correction.correction_reason, "correction_reason", 64)
    if reason != PREDICTION_CORRECTION_REASON:
        raise ValueError(
            f"correction_reason must be {PREDICTION_CORRECTION_REASON!r}"
        )
    row = {
        "scheme_id": bounded(correction.scheme_id, "scheme_id", 64),
        "target_tenor": bounded(correction.target_tenor, "target_tenor", 16),
        "horizon": horizon,
        "predict_date": _require_iso_date(correction.predict_date, "predict_date"),
        "feature_date": _require_iso_date(correction.feature_date, "feature_date"),
        "target_date": _require_iso_date(correction.target_date, "target_date"),
        "original_direction": original,
        "corrected_direction": corrected,
        "scheme_version": bounded(correction.scheme_version, "scheme_version", 64),
        "prediction_phase": phase,
        "correction_reason": reason,
        "created_by": bounded(correction.created_by, "created_by", 128),
    }
    evidence = _normalize_prediction_correction_evidence(
        correction.evidence,
        correction=row,
    )
    ...
    return {
        **row,
        ...
        "operation_id": f"prediction-correction-{operation_scope_sha256}",
...
def _normalize_prediction_correction_evidence(
    evidence: Mapping[str, object],
    *,
    ...
    if not isinstance(evidence, Mapping) or set(evidence) != expected_fields:
        raise ValueError(
            "prediction correction evidence fields are invalid: "
            f"expected={sorted(expected_fields)} "
            f"actual={sorted(evidence) if isinstance(evidence, Mapping) else None}"
        )

    def nonempty(field: str, limit: int = 128) -> str:
        value = _require_nonempty(evidence[field], f"evidence.{field}")
        if len(value) > limit:
            raise ValueError(f"evidence.{field} exceeds {limit} characters")
        return value

    if evidence["schema_version"] != PREDICTION_CORRECTION_EVIDENCE_SCHEMA:
        raise ValueError("prediction correction evidence schema_version mismatch")
    if evidence["attestation_id"] != PREDICTION_CORRECTION_ATTESTATION_ID:
        raise ValueError("prediction correction attestation_id mismatch")
    attestation_sha256 = _require_sha256(
        str(evidence["attestation_sha256"]),
        "evidence.attestation_sha256",
    )
    if attestation_sha256 != PREDICTION_CORRECTION_ATTESTATION_SHA256:
        raise ValueError("prediction correction attestation_sha256 mismatch")
    release_commit = nonempty("release_commit", 40).lower()
    if len(release_commit) != 40 or any(
        character not in "0123456789abcdef" for character in release_commit
    ):
        raise ValueError("evidence.release_commit must be a 40-character git SHA")
    deployment_target = nonempty("deployment_target", 32)
    if deployment_target != "aliyun-gray":
        raise ValueError("prediction corrections are restricted to aliyun-gray")
    database_identity = _require_sha256(
        str(evidence["database_identity_sha256"]),
        "evidence.database_identity_sha256",
    )
    replay_business_digest = _require_sha256(
        str(evidence["replay_business_digest"]),
        "evidence.replay_business_digest",
    )
    replay_direction = evidence["replay_direction"]
    mac3_direction = evidence["mac3_direction"]
    mac3_prediction_id = evidence["mac3_prediction_id"]
    corrected_direction = correction["corrected_direction"]
    if (
        type(replay_direction) is not int
        or type(mac3_direction) is not int
        or replay_direction != corrected_direction
        or mac3_direction != corrected_direction
    ):
        raise ValueError(
            "prediction correction evidence directions must equal "
            "corrected_direction"
        )
    if type(mac3_prediction_id) is not int or mac3_prediction_id <= 0:
        raise ValueError("evidence.mac3_prediction_id must be a positive integer")
    if evidence["comparison_status"] != "matched":
        raise ValueError("prediction correction evidence comparison did not pass")
    request_id = nonempty("replay_request_id", 256)
    expected_request_id = ":".join(
    ...
    if request_id != expected_request_id:
        raise ValueError(
            "prediction correction evidence request_id does not match scope"
        )
    cutoffs = evidence["replay_cutoffs"]
    if not isinstance(cutoffs, Mapping) or set(cutoffs) != {
        "daily",
        "weekly",
        "monthly",
    }:
        raise ValueError("prediction correction evidence cutoffs are invalid")
    normalized_cutoffs = {
        field: _require_nonempty(cutoffs[field], f"evidence.cutoffs.{field}")
        for field in ("daily", "weekly", "monthly")
    }
    if normalized_cutoffs["daily"] != correction["feature_date"]:
        raise ValueError(
            "prediction correction daily cutoff must equal feature_date"
        )
    try:
        date.fromisoformat(normalized_cutoffs["daily"])
    except ValueError as exc:
        raise ValueError(
            "prediction correction daily cutoff must be YYYY-MM-DD"
        ) from exc
    if re.fullmatch(r"\d{6}", normalized_cutoffs["weekly"]) is None:
        raise ValueError("prediction correction weekly cutoff must be YYYYWW")
    weekly_number = int(normalized_cutoffs["weekly"][4:])
    if weekly_number < 1 or weekly_number > 53:
        raise ValueError("prediction correction weekly cutoff week is invalid")
    if re.fullmatch(r"\d{6}", normalized_cutoffs["monthly"]) is None:
        raise ValueError("prediction correction monthly cutoff must be YYYYMM")
    monthly_number = int(normalized_cutoffs["monthly"][4:])
    if monthly_number < 1 or monthly_number > 12:
        raise ValueError("prediction correction monthly cutoff month is invalid")
    normalized = {
        ...
        "replay_generation_id": nonempty("replay_generation_id"),
        "replay_parent_snapshot_id": nonempty("replay_parent_snapshot_id"),
        "replay_combined_snapshot_id": nonempty("replay_combined_snapshot_id"),
        "replay_business_digest": replay_business_digest,
        "replay_request_id": request_id,
        "replay_direction": replay_direction,
        "mac3_direction": mac3_direction,
        "mac3_prediction_id": mac3_prediction_id,
        "comparison_status": "matched",
        "replay_cutoffs": normalized_cutoffs,
    }
    ...
    actual_result_sha256 = _require_sha256(
        str(evidence["replay_result_sha256"]),
        "evidence.replay_result_sha256",
    )
    if actual_result_sha256 != expected_result_sha256:
        raise ValueError("prediction correction replay_result_sha256 mismatch")
    normalized["replay_result_sha256"] = actual_result_sha256
    _verify_prediction_correction_attestation(
...
def _verify_prediction_correction_attestation(
    *,
    correction: Mapping[str, object],
    evidence: Mapping[str, object],
) -> None:
    """只接受 release 内审计清单明确列出的八条纠错 scope。"""
    try:
        raw = PREDICTION_CORRECTION_ATTESTATION_PATH.read_bytes()
    except OSError as exc:
        raise ValueError(
            "prediction correction attestation file is unavailable"
        ) from exc
    actual_sha256 = hashlib.sha256(raw).hexdigest()
    if actual_sha256 != PREDICTION_CORRECTION_ATTESTATION_SHA256:
        raise ValueError("prediction correction attestation file checksum mismatch")
    try:
        attestation = json.loads(raw)
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError("prediction correction attestation file is invalid") from exc
    expected_top_fields = {
        "schema_version",
        "attestation_id",
        "evidence_origin",
    ...
    if not isinstance(attestation, dict) or set(attestation) != expected_top_fields:
        raise ValueError("prediction correction attestation structure is invalid")
    if (
        attestation["schema_version"]
        != "prediction-correction-attestation-v1"
        or attestation["attestation_id"]
        != PREDICTION_CORRECTION_ATTESTATION_ID
        or attestation["evidence_origin"]
        != "codex_targeted_replay_terminal_attestation_v1"
    ):
        raise ValueError("prediction correction attestation identity is invalid")
    common_evidence = {
        "replay_generation_id": evidence["replay_generation_id"],
        "replay_parent_snapshot_id": evidence["replay_parent_snapshot_id"],
        "replay_business_digest": evidence["replay_business_digest"],
    }
    if any(
        attestation[field] != value
        for field, value in common_evidence.items()
    ):
        raise ValueError("prediction correction attestation lineage mismatch")
    expected_entry = {
    ...
    expected_entry.update(
        {
            ...
            "replay_cutoffs": evidence["replay_cutoffs"],
            "replay_combined_snapshot_id": evidence[
                "replay_combined_snapshot_id"
            ],
            "replay_direction": evidence["replay_direction"],
            "mac3_prediction_id": evidence["mac3_prediction_id"],
    ...
    if not isinstance(entries, list) or expected_entry not in entries:
        raise ValueError(
            "prediction correction scope is not in the release attestation"
        )


def _prediction_correction_authority(conn: Connection) -> dict[str, str]:
    release_commit = _require_nonempty(
        os.getenv("BFL_RELEASE_COMMIT", ""),
        "BFL_RELEASE_COMMIT",
    ).lower()
    if len(release_commit) != 40 or any(
        character not in "0123456789abcdef" for character in release_commit
    ):
        raise RuntimeError("BFL_RELEASE_COMMIT must be an exact 40-character SHA")
    deployment_target = _require_nonempty(
        os.getenv("BFL_DEPLOYMENT_TARGET", ""),
        "BFL_DEPLOYMENT_TARGET",
    )
    if deployment_target != "aliyun-gray":
        raise RuntimeError("prediction corrections are restricted to aliyun-gray")
    operator = _require_nonempty(
        os.getenv("BFL_OPERATOR_ID") or os.getenv("USER") or "",
        "prediction correction operator",
    )
    if _dialect_name(conn) == "sqlite":
        identity_payload = {"database": "sqlite", "server_uuid": "test"}
    else:
        database_name, server_uuid = conn.execute(
...
def _validate_prediction_correction_authority(
    correction: Mapping[str, object],
    authority: Mapping[str, str],
) -> None:
    evidence = json.loads(str(correction["evidence"]))
    expected = {
        "release_commit": evidence["release_commit"],
        "deployment_target": evidence["deployment_target"],
        "database_identity_sha256": evidence["database_identity_sha256"],
        "operator": correction["created_by"],
    }
    if dict(authority) != expected:
        raise RuntimeError(
            "prediction correction authority mismatch: "
            f"expected={expected} actual={dict(authority)}"
        )


def _validate_prediction_correction_source(
    prediction: Mapping[str, object],
    correction: Mapping[str, object],
) -> None:
    expected = {
        "scheme_id": correction["scheme_id"],
        "target_tenor": correction["target_tenor"],
        "horizon": correction["horizon"],
        "predict_date": correction["predict_date"],
        "feature_date": correction["feature_date"],
        "target_date": correction["target_date"],
        "predicted_direction": correction["original_direction"],
        "scheme_version": correction["scheme_version"],
        "prediction_phase": correction["prediction_phase"],
    }
    mismatches = {
        field: {"expected": value, "actual": prediction.get(field)}
        for field, value in expected.items()
