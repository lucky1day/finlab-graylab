...
def _is_prediction_corrections_migration(path: Path) -> bool:
    """migration 021 只允许缺表或精确完整目标表。"""
    return path.name == PREDICTION_CORRECTIONS_MIGRATION_FILENAME
...
PREDICTION_CORRECTIONS_MIGRATION_VERSION = 21
PREDICTION_CORRECTIONS_MIGRATION_FILENAME = (
    "021_prediction_corrections.sql"
)
PREDICTION_CORRECTIONS_MIGRATION_SHA256 = (
    "6f2eed2ed11ee345dc22437bad25be8bfa"
    "fd792d2ea334c0b4bdbecbadfd1cfd"
)
...
def _expected_prediction_corrections_schema() -> dict[str, object]:
    """返回 migration 021 新表的精确闭世界定义。"""
    return {
        "table": {
            "table_type": "base table",
            "engine": "innodb",
        ...
        "columns": {
            "id": ("bigint", "no", None, "auto_increment"),
            "prediction_id": ("bigint", "no", None, ""),
            "scheme_id": ("varchar(64)", "no", None, ""),
            "target_tenor": ("varchar(16)", "no", None, ""),
            "horizon": ("int", "no", None, ""),
            "predict_date": ("date", "no", None, ""),
            "feature_date": ("date", "no", None, ""),
            "target_date": ("date", "no", None, ""),
            "original_direction": ("tinyint", "no", None, ""),
            "corrected_direction": ("tinyint", "no", None, ""),
            "scheme_version": ("varchar(64)", "no", None, ""),
            "prediction_phase": ("varchar(32)", "no", None, ""),
            "correction_reason": ("varchar(64)", "no", None, ""),
            "operation_id": ("varchar(128)", "no", None, ""),
            "operation_scope_sha256": ("char(64)", "no", None, ""),
            "release_commit": ("char(40)", "no", None, ""),
            "evidence": ("json", "no", None, ""),
            "created_by": ("varchar(128)", "no", None, ""),
            "created_at": (
                "datetime",
                "no",
                "current_timestamp",
        ...
        "indexes": {
            ...
            "uk_prediction_correction_prediction": {
                "unique": True,
                "columns": ("prediction_id",),
                "sub_parts": (None,),
            },
            "uk_prediction_correction_business": {
                "unique": True,
                "columns": (
                    "scheme_id",
                    "target_tenor",
                ...
                "sub_parts": (None, None, None, None),
            },
            "uk_prediction_correction_operation": {
                "unique": True,
                "columns": ("operation_id",),
                "sub_parts": (None,),
            },
            "uk_prediction_correction_scope": {
                "unique": True,
                "columns": ("operation_scope_sha256",),
                "sub_parts": (None,),
            },
        },
        "checks": {
            "chk_prediction_correction_original_direction": (
                "original_directionin-1,0,1"
            ),
            "chk_prediction_correction_corrected_direction": (
                "corrected_directionin-1,0,1"
            ),
            "chk_prediction_correction_changes_direction": (
                "original_direction<>corrected_direction"
            ),
        },
        "foreign_keys": {
            "fk_prediction_correction_prediction": {
                "columns": ("prediction_id",),
                "referenced_table": "t_scheme_predictions",
...
def _normalize_check_clause(value: object) -> str:
    """归一化 information_schema 的 CHECK 文本用于精确比较。"""
    return re.sub(r"[\s`()]+", "", str(value or "").lower())


def _read_prediction_corrections_schema(
    connection: object,
) -> dict[str, object]:
    """只读 migration 021 目标表的完整定义。"""
    table_name = "t_scheme_prediction_corrections"
    table_row = connection.execute(
        text(
            """
            SELECT table_type AS table_type,
    ...
    grouped_indexes: dict[str, list[Mapping[str, object]]] = {}
    for row in index_rows:
        grouped_indexes.setdefault(str(row["index_name"]), []).append(row)
    grouped_foreign_keys: dict[str, list[Mapping[str, object]]] = {}
    for row in foreign_key_rows:
        grouped_foreign_keys.setdefault(
            str(row["constraint_name"]), []
        ).append(row)
    return {
        ...
        "checks": {
            str(row["constraint_name"]): _normalize_check_clause(
                row["check_clause"]
            )
            for row in check_rows
        },
        "foreign_keys": {
            name: {
                ...
                "update_rule": str(rows[0]["update_rule"]).lower(),
                "delete_rule": str(rows[0]["delete_rule"]).lower(),
            }
            for name, rows in grouped_foreign_keys.items()
        },
    }
...
def _validate_prediction_corrections_schema(
    schema: Mapping[str, object],
    *,
    allow_missing: bool,
) -> None:
    """只接受缺表或 migration 021 完整终态，拒绝部分/漂移定义。"""
    if schema == {"exists": False}:
        if allow_missing:
            return
        raise MigrationPreflightError(
            "t_scheme_prediction_corrections is missing after migration 021"
        )
    expected = {"exists": True, **_expected_prediction_corrections_schema()}
    if dict(schema) != expected:
        raise MigrationPreflightError(
            "unexpected t_scheme_prediction_corrections definition: "
            f"{dict(schema)}"
        )
...
def _classify_prediction_corrections_schema(
    schema: Mapping[str, object],
) -> str:
    """把 021 目标表归类为可重放缺表、完整终态或不可恢复漂移。"""
    if dict(schema) == {"exists": False}:
        return "COMPATIBLE_PARTIAL"
    _validate_prediction_corrections_schema(schema, allow_missing=False)
    return "COMPLETE"
...
def _validate_applying_021_history(
    ...
    migrations = [
        migration
        for migration in manifest
        if migration.version <= PREDICTION_CORRECTIONS_MIGRATION_VERSION
    ]
    expected_versions = list(
        range(1, PREDICTION_CORRECTIONS_MIGRATION_VERSION + 1)
    )
    if [migration.version for migration in migrations] != expected_versions:
        raise MigrationHistoryError(
    ...
    target = migrations[-1]
    if (
        target.path.name != PREDICTION_CORRECTIONS_MIGRATION_FILENAME
        or target.sha256 != PREDICTION_CORRECTIONS_MIGRATION_SHA256
    ):
        raise MigrationHistoryError(
            "migration 021 recovery identity/checksum is not the reviewed "
            "prediction-correction migration"
        )
    rows = sorted(history, key=lambda row: int(row["version"]))
    if [int(row["version"]) for row in rows] != expected_versions:
        raise MigrationHistoryError(
    ...
    for row in rows:
        ...
        expected_state = (
            "APPLYING"
            if version == PREDICTION_CORRECTIONS_MIGRATION_VERSION
            else "APPLIED"
        )
        if str(row.get("state") or "").upper() != expected_state:
            raise MigrationHistoryError(
    ...
    fresh_history = [0] * PREDICTION_CORRECTIONS_MIGRATION_VERSION
    legacy_bootstrap_history = [1] * 16 + [0] * 5
    if tuple(bootstrap_flags) not in {
        tuple(fresh_history),
        tuple(legacy_bootstrap_history),
    }:
        raise MigrationHistoryError(
            "migration 021 APPLYING history has an invalid baseline "
            f"bootstrap pattern: {bootstrap_flags}"
        )
    return target
...
def build_applying_021_inspection(
    *,
    ...
    history: Iterable[Mapping[str, object]],
    database_identity: Mapping[str, object],
    corrections_schema: Mapping[str, object],
) -> dict[str, object]:
    """构造只读 APPLYING 021 状态检查及 canonical digest。"""
    migrations = list(manifest)
    history_rows = sorted(
        (dict(row) for row in history),
        key=lambda row: int(row["version"]),
    ...
    canonical_state = {
        ...
        "history": history_rows,
        "corrections_schema": dict(corrections_schema),
    }
    digest = _applying_017_state_digest(canonical_state)
    classification = "UNSAFE"
    reason: str | None = None
    ...
    try:
        ...
        target = _validate_applying_021_history(
            migrations,
            history_rows,
        )
        migration_identity = {
        ...
        classification = _classify_prediction_corrections_schema(
            corrections_schema
        )
    except (
        KeyError,
...
def _read_applying_021_inspection(
    ...
    try:
        ...
        return build_applying_021_inspection(
            ...
            history=_read_migration_history(connection),
            database_identity=database_identity,
            corrections_schema=_read_prediction_corrections_schema(
                connection
            ),
        )
    except (
...
def _prediction_corrections_recovery_target(
    manifest: Iterable[PreparedMigration],
) -> PreparedMigration:
    candidates = [
        migration
...
def recover_applying_migration_021(
    ...
    *,
    ...
    manifest = validate_release_migration_manifest(paths)
    target = _prediction_corrections_recovery_target(manifest)
    with _migration_owner_connection(engine) as owner_connection:
        initial = _read_applying_021_inspection(
            owner_connection,
            manifest,
        )
        observed_digest = str(initial.get("state_digest") or "")
...
def _execute_prepared_migration_files(
    ...
    for path, raw_statements in prepared:
        ...
        prediction_corrections = _is_prediction_corrections_migration(path)
        mysql_session = False
        ...
        try:
            with engine.begin() as connection:
                ...
                if prediction_corrections and mysql_session:
                    _validate_prediction_corrections_schema(
                        _read_prediction_corrections_schema(connection),
                        allow_missing=True,
                    )
        ...
        if period_average_actuals and mysql_session and execution_started:
            try:
                ...
                postcondition_error = exc
        if prediction_corrections and mysql_session and execution_started:
            try:
                with engine.begin() as connection:
                    preflight_migration_session(connection)
                    _validate_prediction_corrections_schema(
