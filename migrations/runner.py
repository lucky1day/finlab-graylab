...
def _is_auth_profile_migration(path: Path) -> bool:
    """migration 023 只允许精确资料列与强制改密状态。"""
...
def _registry_owner_authority(
    migration: PreparedMigration,
) -> dict[str, str]:
    """从已校验的 021 SQL 读取一次性 owner authority。"""
    insert_statement = next(
        (
            statement
            for statement in migration.statements
            if statement.lstrip().startswith(
                "INSERT INTO `_bfl_registry_owner_021`"
            )
        ),
        None,
    )
    if insert_statement is None:
        raise MigrationHistoryError(
            "migration 021 owner authority statement is missing"
        )
    rows = re.findall(
        r"\('([^']+)',\s*'([^']+)'\)",
        insert_statement,
    ...
    authority = dict(rows)
    if len(rows) != 96 or len(authority) != 96:
        raise MigrationHistoryError(
            "migration 021 owner authority must contain 96 unique rows"
        )
    return authority


def _read_registry_owner_recovery_state(
    connection: object,
) -> dict[str, object]:
    """读取 021 可恢复性所需的精确 schema 与 Registry owner。"""
    schema = _read_registry_owner_schema(connection)
    owner_exists = bool(schema.get("exists"))
    owner_expression = "owner" if owner_exists else "NULL AS owner"
    rows = connection.execute(
        text(
            f"""
            SELECT scheme_id, {owner_expression}
            FROM t_scheme_registry
            ORDER BY scheme_id
            """
        )
        ...
        "rows": [
            {
                ...
                "owner": (
                    None if row.get("owner") is None else str(row["owner"])
                ),
            }
            for row in rows
        ],
...
def _classify_registry_owner_recovery_state(
    state: Mapping[str, object],
    *,
    authority: Mapping[str, str],
) -> str:
    """只接受 021 的缺列、nullable partial 或完整终态。"""
    schema = state.get("schema")
    rows = state.get("rows")
    if not isinstance(schema, Mapping) or not isinstance(rows, list):
        raise MigrationPreflightError(
            "migration 021 recovery state is malformed"
        )
    _validate_registry_owner_schema(schema, allow_missing=True)
    observed: dict[str, object] = {}
    for raw_row in rows:
        if not isinstance(raw_row, Mapping):
            raise MigrationPreflightError(
                "migration 021 Registry state row is malformed"
            )
        scheme_id = str(raw_row.get("scheme_id") or "")
        if not scheme_id or scheme_id in observed:
            raise MigrationPreflightError(
                "migration 021 Registry state has an invalid identity"
            )
        observed[scheme_id] = raw_row.get("owner")
    if set(observed) != set(authority):
        raise MigrationPreflightError(
            "migration 021 Registry identity set does not match authority"
        )
    for scheme_id, owner in observed.items():
        if owner is not None and owner != authority[scheme_id]:
            raise MigrationPreflightError(
                "migration 021 Registry owner conflicts with authority"
            )
    ...
    if dict(schema) == target_schema:
        if any(observed[key] != authority[key] for key in authority):
            raise MigrationPreflightError(
                "migration 021 complete schema has incomplete owner values"
            )
        return "COMPLETE"
    return "COMPATIBLE_PARTIAL"


def _validate_applying_021_history(
    manifest: Iterable[PreparedMigration],
    history: Iterable[Mapping[str, object]],
) -> PreparedMigration:
    """只接受 001..020 APPLIED + 021 APPLYING 的精确连续历史。"""
    migrations = [
        migration
        for migration in manifest
        if migration.version <= REGISTRY_OWNER_MIGRATION_VERSION
    ]
    expected_versions = list(range(1, REGISTRY_OWNER_MIGRATION_VERSION + 1))
    if [migration.version for migration in migrations] != expected_versions:
        raise MigrationHistoryError(
            "APPLYING recovery requires contiguous migration files 001..021"
        )
    target = migrations[-1]
    if (
        ...
        raise MigrationHistoryError(
            "migration 021 recovery identity/checksum is not reviewed"
        )
    rows = sorted(history, key=lambda row: int(row["version"]))
    if [int(row["version"]) for row in rows] != expected_versions:
        raise MigrationHistoryError(
            "APPLYING recovery history must be the exact 001..021 prefix"
        )
    ...
    for row in rows:
        ...
        expected_state = (
            "APPLYING"
            if version == REGISTRY_OWNER_MIGRATION_VERSION
            else "APPLIED"
        )
        if str(row.get("state") or "").upper() != expected_state:
            raise MigrationHistoryError(
                "only migration 021 may be APPLYING"
            )
    bootstrap_flags = [
        int(row.get("baseline_bootstrap") or 0) for row in rows
    ]
    if tuple(bootstrap_flags) not in {
        tuple([0] * REGISTRY_OWNER_MIGRATION_VERSION),
        tuple([1] * 16 + [0] * 5),
    }:
        raise MigrationHistoryError(
            "migration 021 APPLYING history has an invalid bootstrap pattern"
        )
    return target


def build_applying_021_inspection(
    *,
    manifest: Iterable[PreparedMigration],
    history: Iterable[Mapping[str, object]],
    database_identity: Mapping[str, object],
    ...
    """构造只读 APPLYING 021 状态及 canonical digest。"""
    migrations = list(manifest)
    history_rows = sorted(
        (dict(row) for row in history),
        key=lambda row: int(row["version"]),
    ...
    try:
        ...
        target = _validate_applying_021_history(migrations, history_rows)
        migration_identity = {
            "version": target.version,
            "filename": target.path.name,
            "sha256": target.sha256,
        }
        classification = _classify_registry_owner_recovery_state(
            registry_state,
...
def _read_applying_021_inspection(
    connection: object,
    manifest: list[PreparedMigration],
) -> dict[str, object]:
    ...
    try:
        ...
        return build_applying_021_inspection(
            manifest=manifest,
            history=_read_migration_history(connection),
            database_identity=database_identity,
            registry_state=_read_registry_owner_recovery_state(connection),
        )
    except (
        AssertionError,
        KeyError,
        ...
        return _unsafe_applying_021_inspection(
            manifest=manifest,
            database_identity=database_identity,
            reason=exc,
        )
...
def inspect_applying_migration_021(
    engine: object,
    paths: Iterable[Path],
) -> dict[str, object]:
...
def _validate_applying_023_history(
    manifest: Iterable[PreparedMigration],
    history: Iterable[Mapping[str, object]],
) -> PreparedMigration:
    """只接受 001..022 APPLIED + 023 APPLYING 的精确连续历史。"""
    migrations = [
        migration
        for migration in manifest
        if migration.version <= AUTH_PROFILE_MIGRATION_VERSION
    ]
    expected_versions = list(range(1, AUTH_PROFILE_MIGRATION_VERSION + 1))
    if [migration.version for migration in migrations] != expected_versions:
        raise MigrationHistoryError(
            "APPLYING recovery requires contiguous migration files 001..023"
        )
    target = migrations[-1]
    if (
        target.path.name != AUTH_PROFILE_MIGRATION_FILENAME
        or target.sha256 != AUTH_PROFILE_MIGRATION_SHA256
    ):
        raise MigrationHistoryError(
            "migration 023 recovery identity/checksum is not reviewed"
        )
    rows = sorted(history, key=lambda row: int(row["version"]))
    if [int(row["version"]) for row in rows] != expected_versions:
        raise MigrationHistoryError(
            "APPLYING recovery history must be the exact 001..023 prefix"
        )
    by_version = {migration.version: migration for migration in migrations}
    for row in rows:
        version = int(row["version"])
        ...
        expected_state = "APPLYING" if version == 23 else "APPLIED"
        if str(row.get("state") or "").upper() != expected_state:
            raise MigrationHistoryError("only migration 023 may be APPLYING")
    bootstrap_flags = [
        int(row.get("baseline_bootstrap") or 0) for row in rows
    ]
    if tuple(bootstrap_flags) not in {
        tuple([0] * AUTH_PROFILE_MIGRATION_VERSION),
        tuple([1] * 16 + [0] * 7),
    }:
        raise MigrationHistoryError(
            "migration 023 APPLYING history has an invalid bootstrap pattern"
        )
    return target


def build_applying_023_inspection(
    *,
    manifest: Iterable[PreparedMigration],
    history: Iterable[Mapping[str, object]],
    database_identity: Mapping[str, object],
    auth_profile_schema: Mapping[str, object],
) -> dict[str, object]:
    """构造用户资料 migration 状态与 canonical digest。"""
    migrations = list(manifest)
    history_rows = sorted(
        (dict(row) for row in history),
        key=lambda row: int(row["version"]),
    ...
    canonical_state = {
        ...
        "history": history_rows,
        "auth_profile_schema": dict(auth_profile_schema),
    }
    digest = _applying_017_state_digest(canonical_state)
    classification = "UNSAFE"
    reason: str | None = None
    ...
    try:
        ...
        if not str(database_identity.get("server_uuid") or "").strip():
            raise MigrationHistoryError("MySQL server UUID is required")
        target = _validate_applying_023_history(migrations, history_rows)
        migration_identity = {
            "version": target.version,
            "filename": target.path.name,
            "sha256": target.sha256,
        }
        classification = classify_auth_profile_schema(auth_profile_schema)
        if classification == "UNSAFE":
            reason = "authentication profile schema definition drift"
    except (KeyError, MigrationHistoryError, TypeError, ValueError) as exc:
        reason = str(exc)
    return {
        "classification": classification,
...
def _unsafe_applying_023_inspection(
    *,
    manifest: Iterable[PreparedMigration],
    database_identity: Mapping[str, object],
    reason: BaseException,
...
def _read_applying_023_inspection(
    connection: object,
    manifest: list[PreparedMigration],
) -> dict[str, object]:
    ...
    try:
        ...
        return build_applying_023_inspection(
            manifest=manifest,
            history=_read_migration_history(connection),
            database_identity=database_identity,
            auth_profile_schema=read_auth_profile_schema(connection),
        )
    except (
        AssertionError,
        KeyError,
        ...
        return _unsafe_applying_023_inspection(
            manifest=manifest,
            database_identity=database_identity,
            reason=exc,
        )
...
def inspect_applying_migration_023(
    engine: object,
    paths: Iterable[Path],
) -> dict[str, object]:
    """连接 live DB，以 SELECT + named lock 检查 023，不执行 DDL/DML。"""
    return _inspect_applying_migration(
        engine,
        paths,
        read_inspection=_read_applying_023_inspection,
    )


def recover_applying_migration_023(
    engine: object,
    paths: Iterable[Path],
    *,
    expected_state_digest: str,
) -> dict[str, object]:
    """以 inspect digest 为 fence 恢复唯一受支持的 APPLYING 023。"""
    return _recover_applying_migration(
        engine,
        paths,
        expected_state_digest=expected_state_digest,
    ...
    for path, raw_statements in prepared:
        ...
        try:
            with _migration_execution_connection(
                ...
                if auth_profile_migration and mysql_session:
                    if (
                        classify_auth_profile_schema(
        ...
        if auth_migration and mysql_session and execution_started:
            try:
                ...
                postcondition_error = exc
        if auth_profile_migration and mysql_session and execution_started:
            try:
                with _migration_execution_connection(
                    engine,
                    owner_connection,
