...
from schemes.weekly_avg_5y_direct_0529.core.rule_vote import build_rule_vote


PROJECT_ROOT = Path(__file__).resolve().parents[1]
CONFIG_PATH = PROJECT_ROOT / "schemes" / "weekly_avg_5y_direct_0529" / "config.yaml"
...
def build_backtest_rows(
    ...
    *,
    ...
    if full_history.empty:
        return []
    prediction_df = build_rule_vote(full_history)
    ...
    def predict_for_feature(history: pd.DataFrame, feature_week_id: int) -> WeeklyPredictionPoint | None:
        del history
        vote_row = prediction_by_week_id.get(int(feature_week_id))
        if vote_row is None:
            return None
        return WeeklyPredictionPoint(
            source_row=dict(vote_row),
            predicted_direction=_int_or_none(vote_row.get("final_pred_label")),
            confidence=_float_or_none(vote_row.get("final_prob_up")),
            extra={
                "vote_score": _float_or_none(vote_row.get("rule_vote")),
                "rule_vote": _float_or_none(vote_row.get("rule_vote")),
                "source_spec": _str_or_none(vote_row.get("source_spec")),
                "score_spec": _str_or_none(vote_row.get("score_spec")),
            },
        )
...
def run_weekly_avg_5y_direct_0529_reproduction(
    engine: Engine | None = None,
    persist: bool = True,
) -> dict[str, Any]:
    """执行 weekly_avg_5y_direct_0529 DB-aligned 历史复现。"""
    ...
    try:
        ...
        if not full_rows:
            raise RuntimeError("weekly_avg_5y_direct_0529 reproduction produced no rows")
...
def main(argv: list[str] | None = None) -> dict[str, Any]:
    parser = argparse.ArgumentParser(description="Run weekly_avg_5y_direct_0529 historical reproduction.")
    parser.add_argument("--no-persist", action="store_true", help="只输出 JSON，不写 t_backtest_*")
    args = parser.parse_args(argv)
    payload = run_weekly_avg_5y_direct_0529_reproduction(persist=not args.no_persist)
