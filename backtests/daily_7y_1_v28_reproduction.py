...
from schemes.daily_7y_1_v28.core.v28_common import MODEL_VERSION
from schemes.daily_7y_1_v28.inference import run_v28_for_feature_window


SCHEME_ID = "daily_7y_1_v28"
BENCHMARK_ID = "v28_daily_7y_1"
...
def run_historical_prediction(
    *,
    ...
    if not frames:
        raise RuntimeError("daily_7y_1_v28 historical prediction has no feature windows")
...
def run_daily_7y_1_v28_reproduction(
    *,
    persist: bool = True,
    n_workers: int = DEFAULT_N_WORKERS,
    engine: Engine | None = None,
) -> dict[str, Any]:
    """执行 daily_7y_1_v28 DB-aligned 历史复现。"""
    started = time.time()
    ...
    try:
        ...
        if not rows:
            raise RuntimeError("daily_7y_1_v28 reproduction produced no rows")
...
def main() -> None:
    ...
    payload = run_daily_7y_1_v28_reproduction(
