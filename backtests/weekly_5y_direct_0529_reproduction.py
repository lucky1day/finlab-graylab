...
CONFIG_PATH = PROJECT_ROOT / "schemes" / "weekly_5y_direct_0529" / "config.yaml"
...
def run_weekly_5y_direct_0529_reproduction(
    ...
    """执行 weekly_5y_direct_0529 DB-aligned 历史复现。"""
    started = time.time()
...
def main(argv: list[str] | None = None) -> dict[str, Any]:
    parser = argparse.ArgumentParser(description="Run weekly_5y_direct_0529 historical reproduction.")
    parser.add_argument("--no-persist", action="store_true", help="只输出 JSON，不写 t_backtest_*")
    args = parser.parse_args(argv)
    payload = run_weekly_5y_direct_0529_reproduction(persist=not args.no_persist)
