...
from schemes.weekly_avg_7y_cross_d_overlay_0529.core.cross_d_overlay import build_cross_d_overlay


PROJECT_ROOT = Path(__file__).resolve().parents[1]
CONFIG_PATH = PROJECT_ROOT / "schemes" / "weekly_avg_7y_cross_d_overlay_0529" / "config.yaml"
    ...
    *,
    ...
    def predict_for_feature(history: pd.DataFrame, feature_week_id: int) -> WeeklyPredictionPoint | None:
        ...
        return WeeklyPredictionPoint(
            ...
            extra={
                "main_dir": _int_or_none(prediction_row.get("main_pred_label")),
                "main_score": _float_or_none(prediction_row.get("main_prob_up")),
                "d5_d_dir": _int_or_none(prediction_row.get("d5_d_pred_label")),
                "d5_d_score": _float_or_none(prediction_row.get("d5_d_prob_up")),
                "cross_d_score": _float_or_none(prediction_row.get("cross_d_prob_up")),
...
def run_weekly_avg_7y_cross_d_overlay_0529_reproduction(
    engine: Engine | None = None,
    persist: bool = True,
) -> dict[str, Any]:
    """执行 weekly_avg_7y_cross_d_overlay_0529 DB-aligned 历史复现。"""
    ...
    try:
        ...
        if not full_rows:
            raise RuntimeError("weekly_avg_7y_cross_d_overlay_0529 reproduction produced no rows")
...
def main(argv: list[str] | None = None) -> dict[str, Any]:
    parser = argparse.ArgumentParser(description="Run weekly_avg_7y_cross_d_overlay_0529 historical reproduction.")
    parser.add_argument("--no-persist", action="store_true", help="只输出 JSON，不写 t_backtest_*")
    args = parser.parse_args(argv)
    payload = run_weekly_avg_7y_cross_d_overlay_0529_reproduction(persist=not args.no_persist)
