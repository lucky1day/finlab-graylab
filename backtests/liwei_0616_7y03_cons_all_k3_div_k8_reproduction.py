...
from schemes.liwei_0616_7y03_cons_all_k3_div_k8.inference import (
    liwei_0616_pit_window,
    run_7y03_for_window_silent,
)


SCHEME_ID = "liwei_0616_7y03_cons_all_k3_div_k8"
BENCHMARK_ID = "liwei_0616_7y_03"
...
def run_historical_prediction(
    *,
    ...
    """逐 feature_date 运行 7Y_03 PIT 窗口，返回历史预测明细。"""
    is_targeted_sample = feature_dates is not None
    dates = _historical_feature_dates(daily_df, feature_dates=feature_dates)
    if not dates:
        raise RuntimeError("liwei_0616 7Y_03 historical prediction has no feature dates")
    ...
    if batch_mode == "monthly":
        ...
        if detail.empty:
            raise RuntimeError("liwei_0616 7Y_03 historical prediction has no rows")
        return detail.sort_values("anchor_date").reset_index(drop=True)
    if batch_mode != "daily":
        raise ValueError(f"unsupported 7Y_03 batch mode: {batch_mode}")
    ...
    if parallel_shards <= 1 or len(dates) <= 1:
        ...
        raise ValueError(f"unsupported 7Y_03 parallel backend: {parallel_backend}")
...
def _all_monthly_row_cache_exists(
    *,
    ...
    for group in month_groups:
        window_end = max(group)
        for feature_date in group:
...
def _run_monthly_batch_group(
    *,
    ...
    if missing:
        ...
        detail = run_7y03_for_window_silent(
            daily_df=daily_df,
            weekly_df=weekly_df,
            monthly_df=monthly_df,
            date_to_week=date_to_week,
    ...
    if missing_after_batch:
        raise RuntimeError(f"liwei_0616 7Y_03 monthly batch missed feature dates: {missing_after_batch}")
    ...
    *,
    ...
    for baseline in required_baselines():
        ...
        if not isinstance(ctx, dict) or "phase_a_cache" not in ctx:
            raise RuntimeError(f"7Y_03 baseline {baseline} did not return Phase A cache")
...
def run_liwei_0616_7y03_cons_all_k3_div_k8_reproduction(
    *,
    persist: bool = True,
    n_workers: int = DEFAULT_N_WORKERS,
    ...
    """执行 liwei_0616 7Y_03 DB-aligned 历史复现。"""
    ...
    try:
        ...
        if not rows:
            raise RuntimeError("liwei_0616 7Y_03 reproduction produced no rows")
        ...
        if sample_date_list or input_end:
...
def _validate_before_persist(rows: list[dict[str, Any]]) -> None:
    if not rows:
        raise RuntimeError("cannot persist empty 7Y_03 backtest output")
...
def main() -> None:
    ...
    result = run_liwei_0616_7y03_cons_all_k3_div_k8_reproduction(
