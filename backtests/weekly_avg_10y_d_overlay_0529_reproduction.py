...
from schemes.weekly_avg_10y_d_overlay_0529.core.d_overlay import build_d_overlay


PROJECT_ROOT = Path(__file__).resolve().parents[1]
CONFIG_PATH = PROJECT_ROOT / "schemes" / "weekly_avg_10y_d_overlay_0529" / "config.yaml"
...
def build_backtest_rows(
    ...
    *,
    ...
    average_labels: Mapping[int, Any] | None = None,
) -> list[dict[str, Any]]:
    """将 10Y D-overlay 全历史预测转换为周平均回测行。"""
    ...
    def normalize_for_calendar(frame: pd.DataFrame) -> pd.DataFrame:
        return _attach_db_week_dates(_normalize_weekly_frame(frame), calendar)

    full_history = normalize_for_calendar(weekly_df)
    if full_history.empty:
        return []
    prediction_df = build_d_overlay(full_history)
    ...
    def predict_for_feature(history: pd.DataFrame, feature_week_id: int) -> WeeklyPredictionPoint | None:
        ...
        return WeeklyPredictionPoint(
            ...
            extra={
                ...
                "score_dir": _int_or_none(prediction_row.get("score_pred_label")),
                "score_score": _float_or_none(prediction_row.get("score_prob_up")),
                "model2_dir": _int_or_none(prediction_row.get("d_model2_pred_label")),
                "model2_score": _float_or_none(prediction_row.get("model2_prob_up")),
                "d_final_score": _float_or_none(prediction_row.get("d_prob_up")),
                "d_model2_overlay": _bool_or_none(prediction_row.get("d_model2_overlay")),
                "d_signal_source": _str_or_none(prediction_row.get("d_signal_source")),
    ...
    rows = build_weekly_backtest_rows(
        weekly_df,
        calendar=calendar,
        spec=spec,
    ...
    return apply_weekly_average_labels(rows, average_labels) if average_labels is not None else rows
...
def validate_original_benchmark_rows(
    ...
    *,
    ...
    row_by_week: dict[int, dict[str, Any]] = {}
    for row in rows:
        week_id = _int_or_none((row.get("extra") or {}).get("feature_week_id"))
        if week_id is not None:
            row_by_week[week_id] = row

    matched = 0
    for _, expected in benchmark_df.iterrows():
        week_id = _int_or_none(expected.get("feature_week_id"))
        if week_id is None:
            raise AssertionError("original benchmark row missing feature_week_id")
        actual = row_by_week.get(week_id)
        if actual is None:
            raise AssertionError(f"missing benchmark feature_week_id={week_id}")

        _assert_equal(
            _int_or_none(actual.get("predicted_direction")),
            _int_or_none(expected.get("direction")),
            f"direction mismatch feature_week_id={week_id}",
        )
        _assert_equal(
            str(actual.get("target_date")),
            str(expected.get("target_date")),
            f"target_date mismatch feature_week_id={week_id}",
        )
        _assert_equal(
        ...
        _assert_equal(
            ...
            f"label mismatch feature_week_id={week_id}",
        )
        actual_correct = actual.get("predicted_direction") == actual.get("label")
        expected_correct = _bool_or_none(expected.get("is_correct"))
        _assert_equal(actual_correct, expected_correct, f"is_correct mismatch feature_week_id={week_id}")
        ...
        if actual_conf is None or expected_conf is None or abs(actual_conf - expected_conf) > confidence_tol:
            raise AssertionError(
                f"confidence mismatch feature_week_id={week_id}: "
...
def run_weekly_avg_10y_d_overlay_0529_reproduction(
    engine: Engine | None = None,
    persist: bool = True,
) -> dict[str, Any]:
    """执行 weekly_avg_10y_d_overlay_0529 DB-aligned 历史复现。"""
    require_weekly_average_source_evidence(SCHEME_ID, project_root=PROJECT_ROOT)
    started = time.time()
    ...
    try:
        ...
        average_label_rows = read_weekly_average_label_rows(
            engine,
            target_tenor=TARGET_TENOR,
            end_date=BACKTEST_MAX_AS_OF_DATE,
        )
        average_labels = build_weekly_average_label_map_from_rows(
            average_label_rows,
            calendar=calendar,
            target_tenor=TARGET_TENOR,
            live_target_start_date=LIVE_TARGET_START_DATE,
        ...
        if not full_rows:
            raise RuntimeError("weekly_avg_10y_d_overlay_0529 reproduction produced no rows")
...
def main(argv: list[str] | None = None) -> dict[str, Any]:
    parser = argparse.ArgumentParser(description="Run weekly_avg_10y_d_overlay_0529 historical reproduction.")
    parser.add_argument("--no-persist", action="store_true", help="只输出 JSON，不写 t_backtest_*")
    args = parser.parse_args(argv)
    payload = run_weekly_avg_10y_d_overlay_0529_reproduction(persist=not args.no_persist)
