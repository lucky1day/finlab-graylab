...
from schemes.liwei_0616_10y01_cons_say_k3_div_k10.inference import (
...
BENCHMARK_ID = "liwei_0616_10y_01"
...
def run_historical_prediction(
    *,
    ...
    if batch_mode == "monthly":
        ...
        if detail.empty:
            raise RuntimeError("liwei_0616 10Y_01 historical prediction has no rows")
        return detail.sort_values("anchor_date").reset_index(drop=True)
    if batch_mode != "daily":
        raise ValueError(f"unsupported 10Y_01 batch mode: {batch_mode}")
    ...
    if parallel_shards <= 1 or len(dates) <= 1:
        ...
        raise ValueError(f"unsupported 10Y_01 parallel backend: {parallel_backend}")
...
def _run_monthly_batch_group(
    *,
    ...
    if missing_after_batch:
        raise RuntimeError(f"liwei_0616 10Y_01 monthly batch missed feature dates: {missing_after_batch}")
    ...
    *,
    ...
    for baseline in required_baselines():
        ...
        if not isinstance(ctx, dict) or "phase_a_cache" not in ctx:
            raise RuntimeError(f"10Y_01 baseline {baseline} did not return Phase A cache")
...
def run_liwei_0616_10y01_cons_say_k3_div_k10_reproduction(
    *,
    persist: bool = True,
    n_workers: int = DEFAULT_N_WORKERS,
    ...
    """执行 liwei_0616 10Y_01 DB-aligned 历史复现。"""
...
def _validate_before_persist(rows: list[dict[str, Any]]) -> None:
    if not rows:
        raise RuntimeError("cannot persist empty 10Y_01 backtest output")
...
def main() -> None:
    ...
    result = run_liwei_0616_10y01_cons_say_k3_div_k10_reproduction(
