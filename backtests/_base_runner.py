...
单方案 runner 最小写法示例:
    ...
    class MyDailyRunner(BaseDailyBacktestRunner):
        def predict_rows(self, daily_df, *, n_jobs: int = 4):
            # 调用 schemes/my_daily/core 下的纯算法，返回 backtest row dict 列表。
            ...

    def run_my_daily_reproduction(*, persist: bool = True, n_jobs: int = 4):
        runner = MyDailyRunner(SPEC)
        output = runner.run_framework_db_aligned(persist=persist, n_jobs=n_jobs)
        return {"runs": [output.summary]}
"""
...
class BaseDailyBacktestRunner:
    """日频历史回测 runner 的薄模板。"""

    def __init__(self, spec: BacktestSpec):
        self.spec = spec

    def read_daily_csv(self, path: str | Path | None = None) -> pd.DataFrame:
        """读取 spec 指向的 canonical daily_output。"""
        source = path or self.spec.canonical_csv
        if source is None:
            raise ValueError("canonical_csv is required when csv_df is not provided")
        return read_daily_csv(source)
    ...
    def predict_rows(self, daily_df: pd.DataFrame, *, n_jobs: int = 4) -> list[dict[str, Any]]:
        """基于对齐后的 daily dataframe 生成历史回测逐样本记录。"""
        raise NotImplementedError("subclasses must implement predict_rows()")

    def run_framework_db_aligned(
        self,
        *,
        csv_df: pd.DataFrame | None = None,
        engine: Engine | None = None,
        ...
        """执行单方案 framework_db_aligned 回测骨架。

        模板流程为: 公共输入层构建并对齐 daily 输入 -> 子类逐日预测 -> 生成
        月度指标与 summary -> 可选通过 backtests.repository 落库。
        """
        _, db_aligned = self.build_db_aligned_daily(
            csv_df=csv_df,
            engine=engine,
            upstream_mode=False,
            artifact_scheme_id=artifact_scheme_id or self.spec.scheme_id,
        ...
        output = self.make_run_output(
            data_source="framework_db_aligned",
            rows=self.predict_rows(db_aligned, n_jobs=n_jobs),
            report_path=report_path,
        )
        if persist:
            if engine is None:
                raise ValueError("engine is required when persist=True")
            self.persist_run_output(engine, output)
        return output
    ...
    def make_run_output(
        ...
        """按 spec 生成标准 RunOutput。"""
        return make_run_output(
            scheme_id=scheme_id or self.spec.scheme_id,
            data_source=data_source,
            start_date=start_date or self.spec.start_date,
            end_date=end_date or self.spec.end_date,
            rows=rows,
            benchmark_id=self.spec.benchmark_id,
            excluded_target_ranges=self.spec.excluded_target_ranges,
            report_path=report_path,
        )

    def persist_run_output(self, engine: Engine, output: RunOutput) -> int:
        """通过 backtests.repository 落库。"""
        return persist_run_output(engine, output, benchmark_id=self.spec.benchmark_id)
