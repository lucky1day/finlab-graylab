#!/usr/bin/env python3
"""
Standalone 10Y D-overlay production script, 0529 version.

Runtime contract:
- this file contains the embedded Score production logic, Model2 logic, and D overlay logic;
- the only project data file read at runtime is weekly_output0529.csv;
- no external project .py file is imported or executed.

Run directly:
    python3 production_10y_d_overlay_0529/weekly_10y_d_overlay_0529.py
"""
...
ROOT = PROJECT_ROOT
LOCAL_DEPS = ROOT / ".codex" / "python_deps"
...
def read_weekly(path: Path) -> pd.DataFrame:
    ...
    if "week_id" not in df.columns:
        raise ValueError("weekly data must contain week_id")
    ...
    meta = pd.DataFrame(
        {
            ...
            "week_date": weekly["week_id"].map(week_id_to_friday),
        ...
        final_rows.append(
            {
                ...
                "actual_label": int(winner["actual_label"]),
                "future_return": winner["future_return"],
                "is_correct": bool(winner["pred_label"] == winner["actual_label"]),
            }
        )
...
WEEKLY_DATA_PATH = PROJECT_ROOT / "weekly_output0529.csv"
...
def week_id_to_date(week_value: Any) -> pd.Timestamp:
    if pd.isna(week_value):
        return pd.NaT
    s = str(int(week_value)) if not isinstance(week_value, str) else week_value.strip()
    if len(s) != 6:
    ...
    first_mon = first_thu - pd.Timedelta(days=3)
    return first_mon + pd.Timedelta(weeks=week - 1)
...
def load_engineered_frame() -> tuple[pd.DataFrame, list[str]]:
    df = pd.read_csv(WEEKLY_DATA_PATH, encoding="utf-8-sig")
    df.columns = [col.strip().lstrip("\ufeff") for col in df.columns]
    require_columns(df, [DATE_COL, TARGET_RATE_COL], WEEKLY_DATA_PATH)
    df["date"] = df[DATE_COL].apply(week_id_to_date)
    df = df.dropna(subset=["date"]).sort_values("date").reset_index(drop=True)
    df = create_label(df, TARGET_RATE_COL)

    exclude_cols = ["date", DATE_COL, TARGET_RATE_COL, "future_return", "label_5d"]
    original_features = [
...
def build_model2_segment(
    ...
    test_df = test_df.dropna(subset=all_features + ["label_5d"])
    ...
    return pd.DataFrame(
        {
            ...
            "actual_label": test_df["label_5d"].astype(int).to_numpy(),
...
def build_model2_predictions() -> pd.DataFrame:
    base_df, all_features = load_engineered_frame()
    frames = [
...
def build_score_signals() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    weekly = read_weekly(WEEKLY_DATA_PATH)
    ...
    valid_weeks = weekly.loc[
        ...
        & (weekly["week_id"] <= SCORE_TEST_END_WEEK)
        & pd.Series(labels).notna()
        & close.notna(),
        "week_id",
    ].astype(int).tolist()
    test_weeks = [week for week in valid_weeks if SCORE_TEST_START_WEEK <= week <= SCORE_TEST_END_WEEK]
    ...
    test_model_predictions = model_predictions[
        ...
        & (model_predictions["week_id"] <= SCORE_TEST_END_WEEK)
    ].copy()
...
def build_base(model2: pd.DataFrame, score: pd.DataFrame) -> pd.DataFrame:
    ...
    df["label_mismatch"] = df["score_actual_label"].notna() & (
    ...
    df = df[(df["month_date"] >= START_DATE) & (df["month_date"] <= END_DATE)].copy()
...
def apply_d_overlay(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["d_is_correct"] = out["d_pred_label"].astype(int).eq(out["actual_label"].astype(int))
    return out
...
def write_d_report(monthly: pd.DataFrame, overall: dict[str, Any]) -> Path:
    report_path = OUTPUT_DIR / "10Y_D_OVERLAY_0529_MONTHLY_2025_07_2026_05.md"
    lines = [
        "# 10Y D方案 0529 完全独立脚本预测结果",
        "",
        "## 固定口径",
        "",
        f"- 周频原始数据：`{WEEKLY_DATA_PATH}`",
        "- Score主模型：本脚本内嵌生产算法即时生成，不读取外部 Score 输出文件，也不导入外部项目 Python 文件。",
        "- Model2：本脚本内部分段训练生成。",
        f"- 输出目录：`{OUTPUT_DIR}`",
        f"- 月份归属字段：`{MONTH_SOURCE}`",
        f"- 评估区间：`{START_DATE.date()}` 至 `{END_DATE.date()}`",
        f"- Model2参数：`{json.dumps(MODEL2_PARAMS, ensure_ascii=False, sort_keys=True)}`",
        f"- D方案参数：`{json.dumps(D_CONFIG, ensure_ascii=False, sort_keys=True)}`",
        "",
        "## 汇总",
        "",
        "| 样本 | 正确 | 方向准确率 | 涨精确率 | 跌精确率 | 涨召回 | 跌召回 | Overlay | Overlay准确率 |",
        "|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
        (
            f"| {overall['total']} | {overall['correct']} | {d_pct(overall['direction_accuracy'])} | "
            f"{d_pct(overall['up_precision'])} | {d_pct(overall['down_precision'])} | "
            f"{d_pct(overall['up_recall'])} | {d_pct(overall['down_recall'])} | "
            f"{overall['overlay_total']} | {d_pct(overall['overlay_accuracy'])} |"
        ),
        "",
        "## 逐月准确率",
        "",
        "| 月份 | 样本 | 正确 | 方向准确率 | 实际涨 | 实际跌 | 实际平 | 预测涨 | 预测跌 | 涨精确率 | 跌精确率 | 涨召回 | 跌召回 | Overlay | Overlay准确率 |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for _, row in monthly.iterrows():
        lines.append(
            f"| {row['month']} | {int(row['total'])} | {int(row['correct'])} | {d_pct(row['direction_accuracy'])} | "
            f"{int(row['true_up'])} | {int(row['true_down'])} | {int(row['true_flat'])} | "
            f"{int(row['pred_up'])} | {int(row['pred_down'])} | {d_pct(row['up_precision'])} | "
            f"{d_pct(row['down_precision'])} | {d_pct(row['up_recall'])} | {d_pct(row['down_recall'])} | "
            f"{int(row['overlay_total'])} | {d_pct(row['overlay_accuracy'])} |"
        )
...
def main() -> None:
    ...
    score, score_test_model_predictions, score_history = build_score_signals()
    model2 = build_model2_predictions()
    predictions = apply_d_overlay(build_base(model2, score))
    monthly = d_monthly_metrics(predictions)
    overall = d_metrics(predictions)

    score_path = OUTPUT_DIR / "10y_score_0529_generated_final_predictions.csv"
    score_test_path = OUTPUT_DIR / "10y_score_0529_generated_test_model_predictions.csv"
    score_history_path = OUTPUT_DIR / "10y_score_0529_generated_score_history.csv"
    model2_path = OUTPUT_DIR / "10y_model2_0529_segment_predictions.csv"
    weekly_path = OUTPUT_DIR / "10y_d_overlay_0529_weekly_predictions_2025_07_2026_05.csv"
    monthly_path = OUTPUT_DIR / "10y_d_overlay_0529_monthly_accuracy_2025_07_2026_05.csv"
    report_path = write_d_report(monthly, overall)

    score.to_csv(score_path, index=False, encoding="utf-8-sig")
    score_test_model_predictions.to_csv(score_test_path, index=False, encoding="utf-8-sig")
    score_history.to_csv(score_history_path, index=False, encoding="utf-8-sig")
    model2.to_csv(model2_path, index=False, encoding="utf-8-sig")

    # Weekly detail output columns:
    # - week_id/date/week_date/month: time keys and monthly reporting bucket.
    # - TARGET_RATE_COL: current target rate series, default 10Y TB0YWI3C.
    # - actual_label/future_return: realized next-week direction and return.
    # - score_*: main Score model direction and up probability.
    # - model2_prob_up/d_model2_pred_label: Model2 up probability and thresholded direction.
    # - d_score_conf/d_model2_conf/d_model_disagree: D-overlay gate diagnostics.
    # - d_model2_overlay/d_signal_source: whether Model2 replaced Score and why.
    # - d_pred_label/d_prob_up/d_is_correct: final D prediction, final up probability, and correctness.
    keep_cols = [
        "week_id",
        "date",
        "week_date",
    ...
    print(f"Saved generated Score final predictions: {score_path}")
    print(f"Saved generated Score test model predictions: {score_test_path}")
    print(f"Saved generated Score history: {score_history_path}")
    print(f"Saved Model2 predictions: {model2_path}")
