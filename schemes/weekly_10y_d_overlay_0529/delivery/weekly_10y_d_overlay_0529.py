...
def build_model2_predictions(weekly: pd.DataFrame) -> pd.DataFrame:
    ...
    # Request-cutoff adaptation: do not demand fixed evaluation segments that
    # have not started yet. Each retained segment keeps the original training
    # boundary and model parameters, and its test end is clipped to the
    # available cutoff. Test rows never participate in fitting.
    segments = [
        (
            name,
            cutoff,
            max(end, max_date)
            if name == MODEL2_SEGMENTS[-1][0]
            else min(end, max_date),
        )
        for name, cutoff, end in MODEL2_SEGMENTS
        if cutoff <= max_date
    ]
    if not segments:
        raise RuntimeError("No Model2 segment is available at the Request cutoff")
    frames = [
...
DELIVERY_SCHEME_ID = "weekly_10y_d_overlay_0529_bbv2"
DELIVERY_LOOKBACK_WEEKS = 600
DELIVERY_REQUIRED_CURRENT_COLUMNS = ["TB0YWI3C","TB1YWI3C","TB5YWI3C"]
REQUEST_FIELDS = (
    "request_id",
    "predict_date",
    "feature_date",
...
def _run_algorithm(weekly: pd.DataFrame) -> pd.DataFrame:
    ...
    prepared["week_date"] = prepared["week_id"].map(_WEEK_END_BY_ID)
    prepared["model_date"] = prepared["week_id"].map(_legacy_segment_anchor_date)
    result = build_d_overlay(prepared)
    return result.loc[:, ["week_id", "d_pred_label"]].rename(
        columns={"d_pred_label": "predicted_direction"}
    )
