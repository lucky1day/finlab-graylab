#!/usr/bin/env python3
"""Blackbox V2 contract implementation for the frozen 1Y T+5 hybrid model."""
import argparse
import csv
import hashlib
import json
...
MARKET_FACTORS = ("DR007IBC", "USDCNH0C", "SH000300", "IFCFE00C", "S0031525", "AUSHF00C")
CONFIGURATIONS = {
    "one_y_t5_liq_excess_a_v1": {"members": ("excess_7Y_anti5", "excess_7Y_anti20", "dr007_dir60"), "lgbm": None},
    "one_y_t5_liq_excess_b_v1": {"members": ("excess_7Y_anti5", "3Y_cond_rev20", "dr007_dir60"), "lgbm": None},
    "one_y_t5_liq_excess_b_w160_l3_v1": {"members": ("excess_7Y_anti5", "3Y_cond_rev20", "dr007_dir60"), "lgbm": {"window": 160, "n_estimators": 60, "learning_rate": 0.03, "num_leaves": 3, "min_child_samples": 20, "reg_alpha": 1.0, "reg_lambda": 1.0, "colsample_bytree": 0.70}},
    "one_y_t5_liq_excess_b_w200_l7_v1": {"members": ("excess_7Y_anti5", "3Y_cond_rev20", "dr007_dir60"), "lgbm": {"window": 200, "n_estimators": 80, "learning_rate": 0.03, "num_leaves": 7, "min_child_samples": 20, "reg_alpha": 1.0, "reg_lambda": 1.0, "colsample_bytree": 0.70}},
    "one_y_t5_liq_excess_a_w252_l7_v1": {"members": ("excess_7Y_anti5", "excess_7Y_anti20", "dr007_dir60"), "lgbm": {"window": 252, "n_estimators": 80, "learning_rate": 0.03, "num_leaves": 7, "min_child_samples": 25, "reg_alpha": 1.0, "reg_lambda": 1.0, "colsample_bytree": 0.80}},
    "one_y_t5_liq_excess_b_w252_l7_v1": {"members": ("excess_7Y_anti5", "3Y_cond_rev20", "dr007_dir60"), "lgbm": {"window": 252, "n_estimators": 80, "learning_rate": 0.03, "num_leaves": 7, "min_child_samples": 25, "reg_alpha": 1.0, "reg_lambda": 1.0, "colsample_bytree": 0.80}},
    "one_y_t5_liq_excess_a_w350_l7_v1": {"members": ("excess_7Y_anti5", "excess_7Y_anti20", "dr007_dir60"), "lgbm": {"window": 350, "n_estimators": 100, "learning_rate": 0.02, "num_leaves": 7, "min_child_samples": 30, "reg_alpha": 1.0, "reg_lambda": 1.5, "colsample_bytree": 0.80}},
    "one_y_t5_liq_excess_b_w350_l7_v1": {"members": ("excess_7Y_anti5", "3Y_cond_rev20", "dr007_dir60"), "lgbm": {"window": 350, "n_estimators": 100, "learning_rate": 0.02, "num_leaves": 7, "min_child_samples": 30, "reg_alpha": 1.0, "reg_lambda": 1.5, "colsample_bytree": 0.80}},
}
SCHEME_ID = Path(__file__).stem
CONFIG = CONFIGURATIONS.get(SCHEME_ID)
if CONFIG is None:
    raise RuntimeError("script filename is not a registered fixed scheme_id")
...
def build_signals(frame: pd.DataFrame) -> pd.DataFrame:
    returns = {name: pd.to_numeric(frame[column], errors="coerce").pct_change() for name, column in {
        "1Y": "TB1YWI0C", "3Y": "TB3YWI0C", "5Y": "TB5YWI0C",
        "7Y": "TB7YWI0C", "10Y": "TB0YWI0C",
    }.items()}
    excess_7y = returns["7Y"] - 0.5 * (returns["5Y"] + returns["10Y"])
    ret_3y = returns["3Y"]
    short_var = ret_3y.rolling(20).var()
    long_var = ret_3y.rolling(40).var()
    volatility_ratio = long_var / (2.0 * short_var).replace(0.0, np.nan)
    momentum = ret_3y.rolling(20).sum()
    conditional = np.where(volatility_ratio < 0.85, -safe_sign(momentum), safe_sign(momentum))
    dr = pd.to_numeric(frame["DR007IBC"], errors="coerce").ffill()
    return pd.DataFrame({
        "excess_7Y_anti5": -safe_sign(excess_7y.rolling(5).sum()),
        "excess_7Y_anti20": -safe_sign(excess_7y.rolling(20).sum()),
        "3Y_cond_rev20": conditional.astype(np.int8),
        "dr007_dir60": safe_sign(dr.diff().rolling(60).sum()),
...
def predict_direction(truncated: pd.DataFrame) -> int:
    ...
    close = pd.to_numeric(truncated["TB1YWI0C"], errors="coerce").to_numpy(dtype=np.float64)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="1Y T+5 Blackbox V2 prediction scheme")
