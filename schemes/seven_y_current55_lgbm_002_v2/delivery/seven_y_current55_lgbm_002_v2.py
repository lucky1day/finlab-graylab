#!/usr/bin/env python3
"""Blackbox V2 Contract 1.0: 7Y T+1 local-causal monthly-refit LightGBM.

Each invocation keeps only process-local dictionaries. A prediction state is
defined by feature_date and never needs a daily row at target_date.
"""
...
RANDOM_STATE = 42
TARGET = "TB7YWI0C"
IS_LONG_WINDOW = "_002_" in f"_{Path(__file__).stem}_"
...
class Engine:
    def __init__(self, snapshot: Snapshot) -> None:
        ...
        self.valid_state = np.isin(self.truth, [-1.0, 1.0])
    ...
    def _labeled_before(self, feature_idx: int) -> np.ndarray:
        """返回严格早于 feature_idx - 1 的已观察 H=1 标签位置。"""
        eligible = np.arange(max(0, feature_idx - 1))
        return eligible[np.isin(self.target_by_pred[eligible], [-1.0, 1.0])]

    def _first_feature_state_in_month(self, feature_idx: int) -> int:
        """找到 feature_date 所在月首次可用的预测状态。"""
        state_month = self.dates[feature_idx].to_period("M")
        states = np.flatnonzero(
            (self.dates.to_period("M") == state_month)
            & (self.dates >= pd.Timestamp("2024-01-01"))
            & self.valid_state
        )
        return int(states[0]) if len(states) else feature_idx

    def _select_features(self, feature_idx: int) -> np.ndarray:
        year = int(self.dates[feature_idx].year)
        if year in self.selected_by_year:
            return self.selected_by_year[year]
        same_year = np.flatnonzero(
            (self.dates.year == year)
            & (self.dates >= pd.Timestamp("2024-01-01"))
            & self.valid_state
        )
        first_feature = int(same_year[0]) if len(same_year) else feature_idx
        eligible = self._labeled_before(first_feature)
        ...
        for idx in order:
            ...
            if (
                scores[idx] > 0
                and group not in {"self", "curve", "weekly", "monthly"}
        ...
        for idx in order:
            ...
            if scores[idx] > 0 and int(idx) not in selected:
                selected.append(int(idx))
        chosen = np.array(
            (selected or order[:TOP_FEATURES].tolist())[:TOP_FEATURES],
            dtype="int32",
        )
        self.selected_by_year[year] = chosen
        return chosen

    def _model(self, feature_idx: int) -> tuple[LGBMClassifier, np.ndarray] | None:
        month = self.dates[feature_idx].strftime("%Y-%m")
        if month in self.models:
            return self.models[month]
        refit_idx = self._first_feature_state_in_month(feature_idx)
        eligible = self._labeled_before(refit_idx)
        ...
        if len(eligible) < 120 or len(np.unique(self.target_by_pred[eligible])) < 2:
            return None
        chosen = self._select_features(refit_idx)
        ...
        model.fit(
            self.x[eligible][:, chosen],
            (self.target_by_pred[eligible] > 0).astype("int8"),
        )
        self.models[month] = (model, chosen)
        return self.models[month]

    def raw_signal(self, feature_idx: int) -> int:
        if feature_idx in self.raw_cache:
            return self.raw_cache[feature_idx]
        if self.dates[feature_idx] < pd.Timestamp("2024-01-01"):
            self.raw_cache[feature_idx] = 0
            return 0
        if not self.valid_state[feature_idx]:
            self.raw_cache[feature_idx] = 0
            return 0
        state = self._model(feature_idx)
        if state is None or feature_idx < 1:
            self.raw_cache[feature_idx] = 0
            return 0
        model, selected = state
        probability = float(
            model.predict_proba(self.x[[feature_idx]][:, selected])[0, 1]
        )
        signal = (
            0
            if abs(probability - THRESHOLD) < BAND
            else (1 if probability >= THRESHOLD else -1)
        )
        self.raw_cache[feature_idx] = signal
        return signal

    def predict(self, request: dict[str, str]) -> int:
        feature_idx = self.positions[request["feature_date"]]
        signal = self.raw_signal(feature_idx)
        current = self.dates[feature_idx].to_period("M")
        prior = current - FLIP_LOOKBACK
        historical = np.flatnonzero(self.dates.to_period("M") == prior)
        signals = np.array(
            [self.raw_signal(int(index)) for index in historical],
            dtype="int8",
        )
        labels = self.target_by_pred[historical]
        traded = (signals != 0) & np.isin(labels, [-1.0, 1.0])
        if (
            int(traded.sum()) >= FLIP_MIN_TRADES
            and float((signals[traded] == labels[traded]).mean()) < FLIP_BELOW
        ):
            signal = -signal
        return int(signal)
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="7Y T+1 local-causal monthly-refit LightGBM"
    )
