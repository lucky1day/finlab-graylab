...
def load_weekly_simple(path, daily_dates) -> pd.DataFrame:
    ...
    if not existing:
        return pd.DataFrame(index=range(len(daily_dates)))
    # FIX(2026-06-15): use AUTHORITATIVE rdate->week_id (api_wind_date.csv),
    # place each week's value on the LAST TRADING DAY of that week, then ffill.
    # weekly_output[W] = snapshot at week W's last trading day (known at its
    # close) -> placed there and ffilled forward only; NEVER on earlier days
    # of week W. No look-ahead.
    date_map_path = os.path.join(os.path.dirname(path), "api_wind_date.csv")
    amap = pd.read_csv(date_map_path)
    amap.columns = [c.strip().lstrip("﻿") for c in amap.columns]
    amap_dict = dict(zip(
        pd.to_datetime(amap["rdate"]).dt.normalize(),
        amap["week_id"].astype(int)))

    n = len(daily_dates)
    norm_dates = [pd.Timestamp(d).normalize() for d in daily_dates]
    wid = np.array([amap_dict.get(d, -1) for d in norm_dates], dtype=np.int64)

    # last trading-day row index per week_id (chronological)
    order = np.argsort([pd.Timestamp(d) for d in daily_dates], kind="stable")
    last_idx = {}
    for oi in order:
        w = int(wid[oi])
        if w >= 0:
            last_idx[w] = int(oi)   # ascending -> keeps latest (last) row
    ...
    for col in existing:
        ...
        for w, oi in last_idx.items():
            if w in s.index:
                v = s.loc[w]
                if isinstance(v, pd.Series):   # duplicate week_id guard
                    v = v.iloc[-1]
                placed[oi] = v
        ff = pd.Series(placed[order]).ffill().values   # ffill chronologically
        out = np.full(n, np.nan)
        out[order] = ff                                # back to df row order
        result[col] = out
    return pd.DataFrame(result, index=range(n))
...
# ============================================================================
# INTERACTION FEATURES  (cross-asset dynamics)
# ============================================================================
_IX_MF_COLS = [
    ("SH000300", "equity"), ("IFCFE00C", "equity"),
    ("USDCNH0C", "fx"), ("DR007IBC", "money_market"),
    ("S0031525", "commodity"), ("AUSHF00C", "commodity"),
    ("G0006352", "credit"), ("DRS00001", "swap"),
]


def build_interaction_features(df, close_col: str,
                               aux_pairs: list) -> Tuple[pd.DataFrame, dict]:
    """Cross-asset interaction features. Returns (feat_df, feat_cat_map)."""
    n = len(df)
    feats: dict = {}
    feat_cat: dict = {}

    # Yield returns (lagged to avoid leakage)
    yc = pd.to_numeric(df[close_col], errors="coerce").ffill().values
    yl = np.empty(n, dtype=np.float64)
    yl[0] = np.nan; yl[1:] = yc[:-1]
    yr1 = np.empty(n); yr1[0] = np.nan
    yr1[1:] = yl[1:] / yl[:-1] - 1.0
    yr5 = np.empty(n); yr5[:5] = np.nan
    yr5[5:] = yl[5:] / yl[:-5] - 1.0

    # Yield volatility (lagged)
    y_vol20 = _rolling_std(yr1, 20, min_p=10)
    y_vol60 = _rolling_std(yr1, 60, min_p=30)

    for col, cat in _IX_MF_COLS:
        if col not in df.columns:
            continue
        v = pd.to_numeric(df[col], errors="coerce").ffill().values
        vl = np.empty(n, dtype=np.float64)
        vl[0] = np.nan; vl[1:] = v[:-1]
        mr1 = np.empty(n); mr1[0] = np.nan
        mr1[1:] = vl[1:] / vl[:-1] - 1.0
        mr5 = np.empty(n); mr5[:5] = np.nan
        mr5[5:] = vl[5:] / vl[:-5] - 1.0

        # Return products
        feats[f"ix_{col}_yr1"] = yr1 * mr1
        feats[f"ix_{col}_yr5"] = yr5 * mr5
        feat_cat[f"ix_{col}_yr1"] = f"ix_{cat}"
        feat_cat[f"ix_{col}_yr5"] = f"ix_{cat}"

        # Rolling correlation (20d, 60d)
        for w in [20, 60]:
            mp = max(w // 2, 10)
            yr_w = yr1.copy(); mr_w = mr1.copy()
            yr_w[np.isnan(yr_w)] = 0.0; mr_w[np.isnan(mr_w)] = 0.0
            # Cov / (std_y × std_m)
            my = _rolling_mean(yr_w, w, mp)
            mm = _rolling_mean(mr_w, w, mp)
            cov = _rolling_mean(yr_w * mr_w, w, mp) - my * mm
            sy = _rolling_std(yr_w, w, mp)
            sm = _rolling_std(mr_w, w, mp)
            denom = sy * sm
            denom_safe = np.where(denom < 1e-12, np.nan, denom)
            feats[f"ix_corr_{col}_{w}d"] = cov / denom_safe
            feat_cat[f"ix_corr_{col}_{w}d"] = f"ix_{cat}"

    # Volatility regime
    v_safe = np.where(y_vol60 < 1e-12, np.nan, y_vol60)
    feats["ix_vol_ratio"] = y_vol20 / v_safe
    feats["ix_vol_regime"] = np.where(y_vol20 > y_vol60, 1.0, 0.0)
    feat_cat["ix_vol_ratio"] = "ix_regime"
    feat_cat["ix_vol_regime"] = "ix_regime"

    # Yield curve slope dynamics
    for name, aux_col in aux_pairs:
        if "5Y" not in name:
            continue
        ac = pd.to_numeric(df[aux_col], errors="coerce").ffill().values
        al = np.empty(n); al[0] = np.nan; al[1:] = ac[:-1]
        sp = yl - al  # spread (lagged)
        sp_ma20 = _rolling_mean(sp, 20, 10)
        sp_sd20 = _rolling_std(sp, 20, 10)
        sp_sd_safe = np.where(sp_sd20 < 1e-12, np.nan, sp_sd20)
        feats["ix_slope_z20"] = (sp - sp_ma20) / sp_sd_safe
        sp5 = np.empty(n); sp5[:5] = np.nan; sp5[5:] = sp[5:] - sp[:-5]
        feats["ix_slope_chg5"] = sp5
        feat_cat["ix_slope_z20"] = "ix_regime"
        feat_cat["ix_slope_chg5"] = "ix_regime"
...
def select_dynamic_signals(sig_accs: np.ndarray, cat_indices: dict,
                           budget: int = 6, min_acc: float = 0.48,
                           max_per_cat: int = 2, min_cats: int = 3
                           ) -> np.ndarray:
    """V40: Dynamic category-based signal selection.

    Scores each category by its top signals' accuracy, allocates budget
    proportionally to best-performing categories.
    """
    cat_scores = {}
    for cat, indices in cat_indices.items():
        accs = sig_accs[indices]
        eligible = accs[~np.isnan(accs) & (accs >= min_acc)]
        if len(eligible) >= 1:
            top_n = min(2, len(eligible))
            cat_scores[cat] = float(np.mean(np.sort(eligible)[-top_n:]))
    if not cat_scores:
        return np.array([], dtype=int)
    ranked = sorted(cat_scores.items(), key=lambda x: -x[1])
    template = {}
    remaining = budget
    for cat, _score in ranked:
        if remaining <= 0:
            break
        n = min(max_per_cat, remaining)
        template[cat] = n
        remaining -= n
    if len(template) < min_cats:
        for cat, _score in ranked:
            if cat not in template and remaining > 0:
                template[cat] = 1
                remaining -= 1
            if len(template) >= min_cats:
                break
    return select_combo_signals(sig_accs, cat_indices, template, min_acc)
...
# MAIN RUNNER
# ============================================================================
def run_prediction(cfg: dict) -> np.ndarray:
    ...
    # V40: walk-forward IC screening + dynamic signals
    ic_rebal = cfg.get("ic_rebal", None)       # "quarterly"/"monthly"/"yearly"/None
    sig_decay = cfg.get("sig_decay", None)      # float (e.g. 0.95) or None
    dynamic_combo = cfg.get("dynamic_combo", False)
    combo_budget = cfg.get("combo_budget", 6)
    ...
    eval_end = cfg.get("eval_end", "2026-04-30")
    ...
    df = df[df["date"] <= eval_end].reset_index(drop=True)
    ...
    # V50: Optional interaction features
    if cfg.get("interaction_features", False):
        ix_f, ix_cat = build_interaction_features(df, close_col, aux_pairs)
        mf_feat_cat.update(ix_cat)
    else:
        ix_f = pd.DataFrame(index=df.index)
    self_cols = list(bond_f.columns)
    mf_cols = list(mf_f.columns) + list(wkmo_f.columns) + list(ix_f.columns)
    n_self = len(self_cols)
    feat_df = pd.concat([bond_f, mf_f, wkmo_f, ix_f], axis=1)
    feat_all = feat_df.values.astype(np.float64)
    print(f"  Self features: {n_self}, MF features: {len(mf_cols)}, "
          f"Total: {feat_all.shape[1]}")

    # -- Test period (V31: pre starts 2024-01, extended pre window) --
    ts_s = pd.Timestamp("2024-01-01")
    ts_e = pd.Timestamp(eval_end)
    test_idx = np.flatnonzero(
        (df["date"] >= ts_s).values & (df["date"] <= ts_e).values
        & ~np.isnan(labels) & ~np.isnan(close))
    test_dates = pd.to_datetime(df["date"].values[test_idx])
    true_labels = labels[test_idx]
    pre_m = np.asarray(
        (test_dates >= "2024-01-01") & (test_dates <= "2024-12-31"))
    sim_m = np.asarray(
        (test_dates >= "2025-01-01") & (test_dates <= "2025-06-30"))
    real_m = np.asarray(
        (test_dates >= "2025-07-01") & (test_dates <= ts_e))
    ...
    labels_valid_mask = np.isin(labels, [-1.0, 1.0])
    close_valid_mask = ~np.isnan(close)

    def _ic_screen_at(cutoff_idx):
        """IC screen using data before cutoff_idx (with horizon gap)."""
        mask = ((np.arange(len(df)) < cutoff_idx - horizon)
                & labels_valid_mask & close_valid_mask)
        idx = np.flatnonzero(mask)
        if len(idx) > 1000:
            idx = idx[-1000:]
        return ic_screen(
            feat_all[idx], labels[idx], n_self, mf_feat_cat, mf_cols,
            top_self=ic_top_self, top_mf=ic_top_mf, min_cats=6)

    _lgbm_cache = None  # populated by the standard (non-ic_rebal) path

    if ic_rebal is not None:
        # V40: Walk-forward IC screening — re-screen features per period
        if ic_rebal == "monthly":
            ic_periods = test_dates.to_period("M")
        elif ic_rebal == "quarterly":
            ic_periods = test_dates.to_period("Q")
        else:
            ic_periods = test_dates.to_period("Y")
        unique_ic_periods = sorted(ic_periods.unique())

        n_configs = len(lgbm_grid)
        n_test = len(test_idx)
        full_preds = np.zeros((n_configs, n_test), dtype=np.int32)
        full_probs = np.full((n_configs, n_test), 0.5, dtype=np.float64)
        config_valid = np.zeros(n_configs, dtype=bool)

        t1 = time.time()
        print(f"\n  Phase A: Walk-forward IC rebal={ic_rebal} "
              f"({len(unique_ic_periods)} periods, "
              f"{len(lgbm_grid)} configs, {n_workers} workers)")
        for pi, period in enumerate(unique_ic_periods):
            p_mask = ic_periods == period
            p_local = np.where(p_mask)[0]
            p_test_idx = test_idx[p_mask]
            cutoff = int(p_test_idx[0])
            sel_p, n_cats_p = _ic_screen_at(cutoff)
            feat_p = feat_all[:, sel_p]
            n_mf_p = int((sel_p >= n_self).sum())
            print(f"    [{pi+1}/{len(unique_ic_periods)}] {period}: "
                  f"{len(p_local)} days, {len(sel_p)} feats "
                  f"({len(sel_p)-n_mf_p}s+{n_mf_p}mf/{n_cats_p}cat)")
            with mp.Pool(
                n_workers,
                initializer=_init_worker,
                initargs=(len(df), feat_p, labels, close, fallback,
                          p_test_idx, horizon, purge_gap, seeds,
                          lgbm_subsample, lgbm_colsample,
                          time_weight_alpha, early_stopping_rounds),
            ) as pool:
                raw = pool.map(run_config, lgbm_grid)
            for ci, r in enumerate(raw):
                if r is not None:
                    full_preds[ci, p_local] = r["preds"]
                    full_probs[ci, p_local] = r["probs"]
                    config_valid[ci] = True
        results = [{"preds": full_preds[ci], "probs": full_probs[ci],
                     "config": lgbm_grid[ci]}
                    for ci in range(n_configs) if config_valid[ci]]
        selected_feats = np.arange(feat_all.shape[1])
        n_cats = 0
    else:
        # V31: IC screening once on pre-test data
        print(f"  IC screening on pre-test data (strict gap)...")
        _ts_row = int(np.flatnonzero(
            df["date"].values >= np.datetime64("2024-01-01"))[0])
        selected_feats, n_cats = _ic_screen_at(_ts_row)
        ...
        # ── Incremental LGBM cache ──────────────────────────────────────
        cached_lgbm = cfg.get("cached_lgbm")
        n_grid = len(lgbm_grid)
        n_test = len(test_idx)
        _incremental = False
        _full_cache = False
        _n_cached = 0

        if cached_lgbm is not None and n_grid == cached_lgbm.get("n_configs", -1):
            cached_td = pd.to_datetime(cached_lgbm["test_dates"])
            _n_cached = len(cached_td)
            if _n_cached > 0 and _n_cached <= n_test:
                if np.array_equal(test_dates[:_n_cached].values, cached_td.values):
                    if _n_cached >= n_test:
                        _full_cache = True
                    else:
                        _incremental = True
                else:
                    print(f"  LGBM cache: date prefix mismatch, full retrain")
            elif _n_cached > n_test:
                if np.array_equal(test_dates.values,
                                  cached_td.values[:n_test]):
                    _full_cache = True
                else:
                    print(f"  LGBM cache: date mismatch, full retrain")
        elif cached_lgbm is not None:
            print(f"  LGBM cache: grid size mismatch "
                  f"({cached_lgbm.get('n_configs', '?')} vs {n_grid}), "
                  f"full retrain")

        t1 = time.time()
        if _full_cache:
            print(f"\n  Phase A: Full cache hit ({n_test} dates), "
                  f"skipping LGBM training")
            _all_preds = cached_lgbm["preds"][:, :n_test].copy()
            _all_probs = cached_lgbm["probs"][:, :n_test].copy()
            _valid = cached_lgbm["valid"].copy()
            results = [
                {"config": lgbm_grid[ci],
                 "preds": _all_preds[ci], "probs": _all_probs[ci]}
                for ci in range(n_grid) if _valid[ci]]
        elif _incremental:
            _n_new = n_test - _n_cached
            new_test_idx = test_idx[_n_cached:]
            print(f"\n  Phase A: Incremental — {_n_cached} cached + "
                  f"{_n_new} new dates, {n_grid} configs, "
                  f"{n_workers} workers ({len(seeds)} seeds)...")
            with mp.Pool(
                n_workers,
                initializer=_init_worker,
                initargs=(len(df), feat_selected, labels, close, fallback,
                          new_test_idx, horizon, purge_gap, seeds,
                          lgbm_subsample, lgbm_colsample,
                          time_weight_alpha, early_stopping_rounds),
            ) as pool:
                _new_raw = list(pool.map(run_config, lgbm_grid))
            _all_preds = np.zeros((n_grid, n_test), dtype=np.int32)
            _all_probs = np.full((n_grid, n_test), 0.5, dtype=np.float64)
            _valid = cached_lgbm["valid"].copy()
            for ci in range(n_grid):
                if _valid[ci]:
                    _all_preds[ci, :_n_cached] = cached_lgbm["preds"][ci]
                    _all_probs[ci, :_n_cached] = cached_lgbm["probs"][ci]
                if _new_raw[ci] is not None:
                    _all_preds[ci, _n_cached:] = _new_raw[ci]["preds"]
                    _all_probs[ci, _n_cached:] = _new_raw[ci]["probs"]
                    _valid[ci] = True
            results = [
                {"config": lgbm_grid[ci],
                 "preds": _all_preds[ci], "probs": _all_probs[ci]}
            ...
            print(f"\n  Phase A: Training {n_grid} LGBM configs "
            ...
            with mp.Pool(
                ...
                _raw_results = list(pool.map(run_config, lgbm_grid))
            _all_preds = np.zeros((n_grid, n_test), dtype=np.int32)
            _all_probs = np.full((n_grid, n_test), 0.5, dtype=np.float64)
            _valid = np.zeros(n_grid, dtype=bool)
            for ci, r in enumerate(_raw_results):
                if r is not None:
                    _all_preds[ci] = r["preds"]
                    _all_probs[ci] = r["probs"]
                    _valid[ci] = True
            results = [r for r in _raw_results if r is not None]

        _lgbm_cache = {
            "test_dates": test_dates.values.copy(),
            "preds": _all_preds, "probs": _all_probs,
            "valid": _valid, "n_configs": n_grid,
        }

    print(f"\n{'=' * 80}")
    print(f"  {tenor} -- LGBM weighted ML signal + voting")
    print(f"  H={horizon}, IC rebal={ic_rebal or 'once'}")
    ...
    for period in sorted(periods.unique()):
        ...
        if len(before) > lb:
            before = before[-lb:]
        if len(before) >= 5:
            if sig_decay is not None:
                # V40: exponential decay weighting
                w = sig_decay ** np.arange(len(before))[::-1]
                w /= w.sum()
                avg_sa = np.nansum(sa[before] * w[:, None], axis=0)
            else:
        ...
        if np.all(np.isnan(avg_sa)):
            continue
        if dynamic_combo:
            sel = select_dynamic_signals(avg_sa, cat_indices,
                                         budget=combo_budget,
                                         min_acc=min_acc_val)
        else:
    ...
    # Context keys are returned for both seasonal and fixed VT modes.
    # The fixed branch previously referenced these locals before assignment
    # when return_ctx=True.
    _unique_months = sorted(test_months.unique())
    _month_indices = {
        month: np.where(test_months == month)[0]
        ...
        for m in _unique_months:
            ...
            if len(lb) >= 5:
                ...
                for vtu, vtd in VT_CANDIDATES:
                    ...
                    if tr < 0.70:
                        continue
                    acc = float((p[traded] == lab_lb[traded]).mean()) if traded.sum() > 0 else 0.0
                    if acc > best_acc:
                        best_acc = acc
                        best_vt = (vtu, vtd)
    ...
    if ic_rebal or sig_decay or dynamic_combo:
        print(f"  V40: ic_rebal={ic_rebal} sig_decay={sig_decay} "
              f"dynamic_combo={dynamic_combo} combo_budget={combo_budget}")
    print(f"  seeds={seeds}")
    ...
    _ctx32 = {
        ...
        "sig_matrix": sig_matrix, "sa": sa, "cat_indices": cat_indices,
        "sig_mode": sig_mode, "min_acc_val": min_acc_val,
        "rebal": rebal, "combo_template": combo_template,
        "test_months": test_months,
        "lgbm_cache": _lgbm_cache,
    }
    _explore_flags = [
...
# ============================================================================
# INCREMENTAL TRAIN/LOAD
# ============================================================================
def train_or_load_incremental(cfg: dict, pkl_path, data_dir,
                              n_workers: int, ctx_keys: list,
                              logger=None) -> dict:
    """Train or load with incremental LGBM caching.

    If pkl exists and covers eval_end, load directly.
    If pkl exists but eval_end extends beyond cached range,
    pass cached LGBM results so run_prediction only trains on new dates.
    Otherwise train from scratch.
    """
    import pickle as _pickle
    _log = logger or logging.getLogger(__name__)
    eval_end = cfg.get("eval_end", "2026-04-30")
    target_end = pd.Timestamp(eval_end)
    ...
    if pkl_path.exists():
        ...
        cached_dates = pd.to_datetime(cached.get("test_dates", []))
        if len(cached_dates) > 0:
            cached_end = cached_dates.max()
            if cached_end >= target_end:
                _log.info(f"  Cache covers {eval_end}, loading: "
                          f"{pkl_path.name}")
                return cached
            if "lgbm_cache" in cached and cached["lgbm_cache"] is not None:
                _log.info(f"  Incremental: "
                          f"{cached_end.strftime('%Y-%m-%d')} → {eval_end}")
                cached_lgbm = cached["lgbm_cache"]
            else:
                _log.info(f"  No LGBM cache in pkl, full retrain "
                          f"for {eval_end}")

    cfg_run = {**cfg, "data_dir": data_dir, "n_workers": n_workers}
    if cached_lgbm is not None:
        cfg_run["cached_lgbm"] = cached_lgbm
    ...
    for k in ctx_keys:
        if k in ctx:
            save[k] = ctx[k]
    if ctx.get("lgbm_cache") is not None:
        save["lgbm_cache"] = ctx["lgbm_cache"]
    with open(pkl_path, "wb") as f:
        _pickle.dump(save, f)
    _log.info(f"  Saved: {pkl_path.name}")
    return save

"""Feature screening metrics: IC, AUC, Sharpe.

Drop-in replacement for ic_screen() in bond_common.
Usage:
...
def feat_screen(feat_vals, labels, n_self, mf_feat_cat, mf_col_names,
    ...
    """Drop-in replacement for ic_screen with configurable metric.
...
SCHEME = {'target_tenor': '5Y', 'close_col': 'TB5YWI0C', 'aux_pairs': (('1Y', 'TB1YWI0C'), ('3Y', 'TB3YWI0C'), ('7Y', 'TB7YWI0C')), 'model_family': 'xgb', 'signal': 'spr_3Y1Y_anti20', 'screen_metric': 'sharpe', 'screen_window': 756, 'top_self': 30, 'top_mf': 30, 'train_window': 504, 'n_estimators': 120, 'learning_rate': 0.025, 'max_depth': 2, 'min_child_weight': 20.0, 'reg_alpha': 1.0, 'reg_lambda': 5.0, 'colsample_bytree': 0.75, 'tree_weight': 0.75, 'margin': 0.05, 'balance_trigger': 8}
...
LOG = logging.getLogger("blackbox_t5")
...
def _date(value, field):
    if not isinstance(value, str):
        raise ContractError(field + " must be a YYYY-MM-DD string")
    try:
        parsed = datetime.strptime(value, DATE_FORMAT)
    except ValueError as exc:
        raise ContractError(field + " must be a YYYY-MM-DD string") from exc
...
def _validate(record):
    if set(record) != set(REQUEST_FIELDS):
        raise ContractError("request must contain exactly the seven Contract fields")
    if not isinstance(record["request_id"], str) or not record["request_id"]:
        raise ContractError("request_id must be a nonempty string")
    feature, predict, target = (_date(record[key], key) for key in ("feature_date", "predict_date", "target_date"))
    if not (feature <= predict <= target and feature < target):
        raise ContractError("feature_date <= predict_date <= target_date and feature_date < target_date are required")
    if record["daily_cutoff_key"] != _date(record["daily_cutoff_key"], "daily_cutoff_key").strftime(DATE_FORMAT):
        raise ContractError("daily_cutoff_key must be a YYYY-MM-DD string")
    for field in ("weekly_cutoff_key", "monthly_cutoff_key"):
        if not isinstance(record[field], str) or not KEY_RE.fullmatch(record[field]):
            raise ContractError(field + " must be a six-digit string")
    return record
...
def _read_many(path):
    ...
    if not rows or len(rows) > 100:
        raise ContractError("requests.csv must contain 1 to 100 requests")
    if set(rows[0]) != set(REQUEST_FIELDS) or any(set(row) != set(REQUEST_FIELDS) for row in rows):
        raise ContractError("requests.csv must contain exactly the seven Contract fields")
    rows = [_validate(row) for row in rows]
    if len({row["request_id"] for row in rows}) != len(rows):
        raise ContractError("request_id values must be unique")
    return rows
...
def _atomic(path, writer):
    ...
    with tempfile.NamedTemporaryFile("w", encoding="utf-8", newline="", dir=str(output.parent), delete=False) as handle:
        temporary = Path(handle.name); writer(handle)
    try:
...
def _read_snapshot(data_dir, request):
    data_dir = Path(data_dir)
    paths = {name: data_dir / name for name in ("daily_output.csv", "weekly_output.csv", "monthly_output.csv", "api_wind_date.csv")}
    missing = [name for name, path in paths.items() if not path.is_file()]
    if missing:
        raise ContractError("missing upstream input: " + ", ".join(missing))
    daily = pd.read_csv(paths["daily_output.csv"]); daily.columns = [str(c).strip().lstrip("\ufeff") for c in daily.columns]
    weekly = pd.read_csv(paths["weekly_output.csv"], dtype={"week_id": str}); weekly.columns = [str(c).strip().lstrip("\ufeff") for c in weekly.columns]
    monthly = pd.read_csv(paths["monthly_output.csv"], dtype={"month_id": str}); monthly.columns = [str(c).strip().lstrip("\ufeff") for c in monthly.columns]
    mapping = pd.read_csv(paths["api_wind_date.csv"], dtype={"week_id": str}); mapping.columns = [str(c).strip().lstrip("\ufeff") for c in mapping.columns]
    if not {"date"}.issubset(daily) or not {"week_id"}.issubset(weekly) or not {"month_id"}.issubset(monthly) or not {"rdate", "week_id"}.issubset(mapping):
        raise ContractError("upstream input schema is invalid")
    ...
    mapping["rdate"] = pd.to_datetime(mapping["rdate"], errors="raise").dt.normalize()
    mapping["week_id"] = mapping["week_id"].astype(str).str.zfill(6)
    if daily["date"].duplicated().any() or not daily["date"].is_monotonic_increasing:
        raise ContractError("daily date must be unique and ascending")
    if weekly["week_id"].duplicated().any() or not weekly["week_id"].is_monotonic_increasing:
        raise ContractError("weekly week_id must be unique and ascending")
    if monthly["month_id"].duplicated().any() or not monthly["month_id"].is_monotonic_increasing:
        raise ContractError("monthly month_id must be unique and ascending")
    daily_cutoff = _date(request["daily_cutoff_key"], "daily_cutoff_key")
    if daily_cutoff not in set(daily["date"]): raise ContractError("daily cutoff key is absent")
    if request["weekly_cutoff_key"] not in set(weekly["week_id"]): raise ContractError("weekly cutoff key is absent")
    if request["monthly_cutoff_key"] not in set(monthly["month_id"]): raise ContractError("monthly cutoff key is absent")
    daily = daily.loc[daily["date"] <= daily_cutoff].reset_index(drop=True)
    weekly = weekly.loc[weekly["week_id"] <= request["weekly_cutoff_key"]].reset_index(drop=True)
    monthly = monthly.loc[monthly["month_id"] <= request["monthly_cutoff_key"]].reset_index(drop=True)
    mapping = mapping.loc[mapping["rdate"] <= daily_cutoff].reset_index(drop=True)
    return daily, weekly, monthly, mapping


def _weekly_lagged(weekly, mapping, dates):
    existing = [column for column in WEEKLY_COLS if column in weekly.columns]
    if not existing: return pd.DataFrame(index=range(len(dates)))
    values = weekly.loc[:, existing].apply(pd.to_numeric, errors="coerce").shift(1)
    values.index = weekly["week_id"]
    date_to_week = dict(zip(mapping["rdate"], mapping["week_id"]))
    dates = pd.DatetimeIndex(dates).normalize(); ids = np.asarray([date_to_week.get(date, "") for date in dates], dtype=object)
    last = {}
    for index, week_id in enumerate(ids):
        if week_id: last[str(week_id)] = index
    result = {}
    for column in existing:
        placed = np.full(len(dates), np.nan, dtype=np.float64)
        for week_id, index in last.items():
            if week_id in values.index: placed[index] = values.at[week_id, column]
        result[column] = pd.Series(placed).ffill().to_numpy()
    return pd.DataFrame(result)


def _model():
    if SCHEME["model_family"] == "xgb":
        return xgb.XGBClassifier(objective="binary:logistic", eval_metric="logloss", use_label_encoder=False, random_state=RANDOM_STATE, n_jobs=1, verbosity=0, tree_method="hist", subsample=0.9, gamma=0.0, n_estimators=SCHEME["n_estimators"], learning_rate=SCHEME["learning_rate"], max_depth=SCHEME["max_depth"], min_child_weight=SCHEME["min_child_weight"], reg_alpha=SCHEME["reg_alpha"], reg_lambda=SCHEME["reg_lambda"], colsample_bytree=SCHEME["colsample_bytree"])
    return lgb.LGBMClassifier(objective="binary", random_state=RANDOM_STATE, n_jobs=1, verbosity=-1, subsample=0.9, force_col_wise=True, n_estimators=SCHEME["n_estimators"], learning_rate=SCHEME["learning_rate"], num_leaves=SCHEME["num_leaves"], min_child_samples=SCHEME["min_child_samples"], reg_alpha=SCHEME["reg_alpha"], reg_lambda=SCHEME["reg_lambda"], colsample_bytree=SCHEME["colsample_bytree"])


def _weights(binary):
    positive = max(int(binary.sum()), 1); negative = max(int(len(binary) - positive), 1)
    return np.where(binary == 1, len(binary) / (2.0 * positive), len(binary) / (2.0 * negative)).astype(np.float64)


def _build_dataset(daily, weekly, monthly, mapping):
    frame = daily.copy(); close = SCHEME["close_col"]
    aux_pairs = [tuple(pair) for pair in SCHEME["aux_pairs"]]
    weekly_frame = _weekly_lagged(weekly, mapping, frame["date"])
    monthly_frame = load_monthly_frame(monthly, frame["date"])
    bond = build_bond_features(frame, close, aux_pairs)
    market, categories = build_mf_features(frame)
    wkmo = build_wkmo_features(weekly_frame, monthly_frame)
    interaction, interaction_categories = build_interaction_features(frame, close, aux_pairs)
    categories.update(interaction_categories)
    for column in wkmo.columns: categories[column] = "weekly" if column.startswith("wk_") else "monthly"
    values = pd.concat([bond, market, wkmo, interaction], axis=1).to_numpy(dtype=np.float32)
    # Keep the feature row at the cutoff.  Only resolved labels enter
    # the training history in _raw_direction, so unresolved T+5 labels
    # cannot leak into fitting or feature selection.
    labels = make_labels(frame, close, horizon=HORIZON).astype(np.float64)
    signals = build_all_signals(frame, COL_MAP, SCHEME["target_tenor"]).astype(np.int8)
    dates = pd.DatetimeIndex(frame["date"])
    market_columns = list(market.columns) + list(wkmo.columns) + list(interaction.columns)
    return values, labels, signals, dates, categories, market_columns


def load_monthly_frame(monthly, daily_dates):
    existing = [column for column in MONTHLY_COLS if column in monthly.columns]
    if not existing: return pd.DataFrame(index=range(len(daily_dates)))
    indexed = monthly.set_index("month_id").loc[:, existing].apply(pd.to_numeric, errors="coerce")
    dates = pd.DatetimeIndex(daily_dates); prior = (dates.to_period("M") - 1).astype(str).str.replace("-", "", regex=False)
    return pd.DataFrame({column: indexed[column].reindex(prior).to_numpy(dtype=np.float64) for column in existing})


def _raw_direction(row, values, labels, signals, categories, market_columns):
    resolved = np.flatnonzero((np.arange(len(labels)) < row - HORIZON) & np.isin(labels, (-1, 1)))
    if len(resolved) < MIN_HISTORY: raise ContractError("insufficient resolved history")
    screen_history = resolved[-SCHEME["screen_window"]:]
    selected, _ = feat_screen(values[screen_history], labels[screen_history], values.shape[1] - len(market_columns), categories, market_columns, top_self=SCHEME["top_self"], top_mf=SCHEME["top_mf"], min_cats=6, metric=SCHEME["screen_metric"])
    history = resolved[-SCHEME["train_window"]:]; binary = (labels[history] > 0).astype(np.int8)
    if len(np.unique(binary)) == 1: probability = float(binary[0])
    else:
        model = _model(); model.fit(values[history][:, selected], binary, sample_weight=_weights(binary)); probability = float(model.predict_proba(values[row:row + 1, selected])[:, 1][0])
    score = SCHEME["tree_weight"] * (2.0 * probability - 1.0) + (1.0 - SCHEME["tree_weight"]) * int(signals.at[signals.index[row], SCHEME["signal"]])
    return 1 if score > SCHEME["margin"] else (-1 if score < -SCHEME["margin"] else 0)


def _direction(request, data_dir):
    daily, weekly, monthly, mapping = _read_snapshot(data_dir, request)
    values, labels, signals, dates, categories, market_columns = _build_dataset(daily, weekly, monthly, mapping)
    feature_date = _date(request["feature_date"], "feature_date")
    matched = np.flatnonzero(dates == feature_date)
    if len(matched) != 1: raise ContractError("feature_date is unavailable after T+5 label alignment")
    row = int(matched[0]); current = _raw_direction(row, values, labels, signals, categories, market_columns)
    # The B8/B12 rule is causal: it only inspects earlier rows whose realised
    # target date belongs to the current request's target month.
    trigger = int(SCHEME["balance_trigger"])
    target_month = _date(request["target_date"], "target_date").to_period("M")
    historical = [index for index in range(row) if index + HORIZON < len(dates) and dates[index + HORIZON].to_period("M") == target_month]
    position = len(historical) + 1
    if trigger > 0 and position == trigger:
        prior = [_raw_direction(index, values, labels, signals, categories, market_columns) for index in historical]
        seen_up, seen_down = any(value > 0 for value in prior), any(value < 0 for value in prior)
        if seen_up != seen_down: current = -1 if seen_up else 1
    return int(current)


def _result(request, data_dir):
    result = {key: request[key] for key in RESULT_FIELDS[:-1]}
    result["predicted_direction"] = _direction(request, data_dir)
    return result


def _write_json(path, result):
    _atomic(path, lambda handle: json.dump(result, handle, ensure_ascii=False, separators=(",", ":")))


def _write_csv(path, rows):
    def writer(handle):
        output = csv.DictWriter(handle, fieldnames=RESULT_FIELDS); output.writeheader(); output.writerows(rows)
    _atomic(path, writer)


def main(argv=None):
    parser = argparse.ArgumentParser(); sub = parser.add_subparsers(dest="command", required=True)
    one = sub.add_parser("predict"); one.add_argument("--request", required=True); one.add_argument("--data-dir", required=True); one.add_argument("--output", required=True)
    many = sub.add_parser("backtest"); many.add_argument("--requests", required=True); many.add_argument("--data-dir", required=True); many.add_argument("--output", required=True)
    args = parser.parse_args(argv); _stderr_logging()
    with contextlib.redirect_stdout(sys.stderr):
        if args.command == "predict": _write_json(args.output, _result(_read_one(args.request), args.data_dir))
        else: _write_csv(args.output, [_result(record, args.data_dir) for record in _read_many(args.requests)])
