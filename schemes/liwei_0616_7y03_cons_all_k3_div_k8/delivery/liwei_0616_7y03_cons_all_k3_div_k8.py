#!/usr/bin/env python3
"""7Y03 ALL 共识的自包含 Blackbox V2 草稿。

保留 7Y STD/ACCWT 双 seed、7Y DIV 三 seed 与独立 CROSS_5Y 三 seed 三个计算族。
仅在单次调用内共享 Phase A；历史同期全配置排序，当前月训练精确入选并集。
最终方向沿用原 vs_full 符号、四基线 k=3 共识与 DIV 连续方向控制器。
...
SOURCE_MODEL_ID = "7Y_03_cons_ALL_k_3_DIV_K_8"
TARGET_TENOR = "7Y"
...
BASELINE_CONFIGS: dict[str, dict[str, Any]] = {
    ...
    "DIV": {
        **_SHARED_LGBM,
        ...
        "seeds": [42, 314, 159],
        "K": 5,
        "combo_name": "butterfly2_policy2",
        "combo_template": {"butterfly": 2, "policy": 2},
        "lgbm_w": 0.30,
        "rebal": "quarterly",
        "ew": 378,
        "min_acc": 0.40,
    ...
    "CROSS_5Y": {
        **_SHARED_LGBM,
        ...
        "seeds": [42, 314, 159],
        "K": 10,
        "combo_name": "policy2_term_prem2_butterfly2",
...
PROD_CONFIG: dict[str, Any] = {
    "model_no": 3,
    "name": "cons(ALL,k=3)+DIV_K=8",
    "baselines": ["STD", "DIV", "ACCWT", "CROSS_5Y"],
    "k_agree": 3,
    "fallback": "DIV",
    "streak_K": 8,
}
...
def model_config(baseline: str = "STD", **overrides) -> dict[str, Any]:
    """返回 7Y_03 某个 baseline 的固定参数副本。"""
    if baseline not in BASELINE_CONFIGS:
        raise KeyError(f"unknown 7Y_03 baseline: {baseline}")
...
def run_config(config: dict) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"7Y03 LightGBM config failed: {config!r}") from exc
...
def run_7y03_for_feature_window(
    *,
    ...
    """运行 7Y_03 PIT 窗口，返回 current window 内逐 feature_date 明细。"""
    ...
    for baseline in required_baselines():
        ...
        if not isinstance(ctx, dict) or len(ctx.get("test_dates", [])) == 0:
            raise RuntimeError(f"7Y_03 baseline {baseline} produced no context rows")
...
def _run_month(
    *,
    ...
    test_ranges = None
    for baselines in (("STD", "ACCWT"), ("DIV",), ("CROSS_5Y",)):
        prepared = prepare_prediction_inputs(model_config(
            baselines[0],
            daily_df=daily_df,
            weekly_df=weekly_df,
            monthly_df=monthly_df,
    ...
    def run_context(caches: Mapping[str, Any]) -> pd.DataFrame:
        with redirect_stdout(sys.stderr):
            return run_7y03_for_feature_window(
                daily_df=daily_df,
                weekly_df=weekly_df,
                monthly_df=monthly_df,
                date_to_week=date_to_week,
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="liwei_0616 7Y03 Blackbox V2 successor"
    )
