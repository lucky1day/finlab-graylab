#!/usr/bin/env python3
"""10Y01 SAY 共识的自包含 Blackbox V2 草稿。

保留原 STD/ACCWT/DIV 三种 10Y 基线及独立 V55_7Y 跨期限基线。
复用自包含双族 full-OOS 私有状态，仅将 prior 月切片交回原月度排名；当前月精确重算。
最终方向沿用原 vs_full 符号、三基线共识与 DIV 连续方向控制器。
尚未完成同输入完整等价或目标环境性能验收。
"""
...
def _phase_a_config_key(config: Mapping[str, Any]) -> str:
    return json.dumps(dict(config), sort_keys=True, separators=(",", ":"))


def _weekly_prediction_feature_overrides(
    *,
    ...
    """只覆盖各 Request 当日的未完周特征，保持历史行不变。"""
    weekly = _normalize_aux_frame(weekly_df, "week_id")
    if weekly is None:
        return {}
    weekly = weekly.dropna(subset=["week_id"]).copy()
    weekly["week_id"] = weekly["week_id"].astype(int)
    weekly = weekly.sort_values("week_id").drop_duplicates(
        "week_id", keep="last"
    ).set_index("week_id")
    feature_positions = {
        str(name): index
        for index, name in enumerate(prepared_inputs["feature_names"])
    }
    daily_dates = pd.to_datetime(prepared_inputs["df"]["date"])
    row_by_date = {
        day.strftime("%Y-%m-%d"): index
        for index, day in enumerate(daily_dates)
    }
    aligned_weekly = prepared_inputs["weekly_feature_values"]
    overrides: dict[int, np.ndarray] = {}
    for day in requested_dates:
        row_index = row_by_date[day]
        week_id = int(date_to_week[day])
        if week_id not in weekly.index:
            raise ContractError(f"weekly cutoff is missing for {day}")
        feature_row = np.asarray(
            prepared_inputs["feat_all"][row_index], dtype=np.float64
        ).copy()
        for column in aligned_weekly.columns:
            val_name = f"wk_{column}_val"
            chg_name = f"wk_{column}_chg"
            if val_name not in feature_positions or chg_name not in feature_positions:
                continue
            previous = (
                float(aligned_weekly.iloc[row_index - 1][column])
                if row_index > 0
                else 0.0
            )
            raw_current = weekly.at[week_id, column]
            current = previous if pd.isna(raw_current) else float(raw_current)
            feature_row[feature_positions[val_name]] = current
            feature_row[feature_positions[chg_name]] = current - previous
        overrides[row_index] = feature_row
    return overrides
...
def _two_stage_phase_a(
    *,
    ...
    n_workers: int,
    baselines: tuple[str, ...],
    predict_feat_overrides: Mapping[int, np.ndarray],
    prior_phase_cache: Mapping[str, Any] | None = None,
) -> tuple[
    dict[str, Any],
    dict[str, Any],
    tuple[tuple[str, str], ...],
]:
    """历史月跑全 grid，当前月只跑原算法实际选择的配置并集。"""
    prior_start = str(
        (pd.Timestamp(current_start) - pd.DateOffset(years=1)).date()
    )
    prior_end = str(
        (
    ...
    if prior_phase_cache is None:
        prior = _train_phase_a(
            daily_df=daily_df,
            weekly_df=weekly_df,
            monthly_df=monthly_df,
            date_to_week=date_to_week,
            prepared_inputs=prepared_inputs,
            test_range=prior_range,
            n_workers=n_workers,
        ...
        indices = np.flatnonzero(
            (dates >= pd.Timestamp(prior_start)).to_numpy()
            & (dates <= pd.Timestamp(prior_end)).to_numpy()
            & ~np.isnan(prepared_inputs["close"]))
        test_dates = pd.to_datetime(dates.iloc[indices])
        prior = {
            "phase_a_cache": {
                "test_dates": test_dates.dt.strftime("%Y-%m-%d").tolist(),
                "results": _slice_phase_a_cache(prior_phase_cache, test_dates),
            },
            "true_labels": np.asarray(prepared_inputs["labels"])[indices],
        }
    prior_cache = prior["phase_a_cache"]
    prior_results = list(prior_cache["results"])
    true_labels = np.asarray(prior["true_labels"])
    usable_end = max(0, len(prior_cache["test_dates"]) - HORIZON)
    if usable_end < 5:
        # 原 Phase C 在历史不足时直接平均全网格概率，不做配置排名。
        selected_indices = list(range(len(prior_results)))
    else:
        scores = np.asarray([
            float(
                (
                    np.asarray(item["preds"])[:usable_end]
                    == true_labels[:usable_end]
                ).mean()
            )
            for item in prior_results
        ])
        windows = np.asarray([
            item["config"]["window"] for item in prior_results
        ])
        selected: set[int] = set()
        for name in baselines:
            cfg = BASELINE_CONFIGS[name]
            top = (
                _diverse_topk(scores, windows, int(cfg["K"]))
                if cfg["ens_mode"] == "diverse"
                else np.argsort(-scores)[:int(cfg["K"])]
            )
            selected.update(int(index) for index in top)
        selected_indices = sorted(selected)
    selected_grid = [
        dict(prior_results[index]["config"])
        for index in selected_indices
    ]
    current = _train_phase_a(
        daily_df=daily_df,
        weekly_df=weekly_df,
        monthly_df=monthly_df,
        date_to_week=date_to_week,
        prepared_inputs=prepared_inputs,
        test_range=current_range,
        n_workers=n_workers,
        baseline=baselines[0],
        grid=selected_grid,
        predict_feat_overrides=predict_feat_overrides,
    )
    expected_keys = {
        _phase_a_config_key(config) for config in selected_grid
    }
    current_caches = (
        current["phase_a_cache"],
        current["override_phase_a_cache"],
    )
    for cache in current_caches:
        actual_keys = [
            _phase_a_config_key(item["config"])
            for item in cache["results"]
        ]
        if len(actual_keys) != len(expected_keys) or set(actual_keys) != expected_keys:
            raise RuntimeError("current Phase A config set is incomplete")
    current_count = len(current["phase_a_cache"]["test_dates"])

    def combine(current_cache: Mapping[str, Any]) -> dict[str, Any]:
        current_by_config = {
            _phase_a_config_key(item["config"]): item
            for item in current_cache["results"]
        }
        combined_results = []
        for prior_item in prior_results:
            config = dict(prior_item["config"])
            preds = np.asarray(prior_item["preds"], dtype=np.int32)
            probs = np.asarray(prior_item["probs"], dtype=np.float64)
            current_item = current_by_config.get(_phase_a_config_key(config))
            if current_item is None:
                current_preds = np.zeros(current_count, dtype=np.int32)
                current_probs = np.full(current_count, 0.5, dtype=np.float64)
            else:
                current_preds = np.asarray(
                    current_item["preds"], dtype=np.int32
                )
                current_probs = np.asarray(
                    current_item["probs"], dtype=np.float64
                )
            combined_results.append({
                "config": config,
                "preds": np.concatenate([preds, current_preds]),
                "probs": np.concatenate([probs, current_probs]),
            })
        return {
            "test_dates": (
                list(prior_cache["test_dates"])
                + list(current_cache["test_dates"])
            ),
            "results": combined_results,
            "selected_config_count": len(selected_indices),
        }

    return (
        combine(current_caches[0]),
        combine(current_caches[1]),
...
def _run_month(
    *,
    ...
    current_end: str,
    n_workers: int = 4,
    prior_phase_by_baseline: Mapping[str, Any] | None = None,
    inputs_by_baseline: Mapping[str, Any] | None = None,
) -> pd.DataFrame:
    prepared_by_baseline: dict[str, Any] = {}
    base_caches: dict[str, Any] = {}
    override_caches: dict[str, Any] = {}
    test_ranges = None
    for baselines in (("STD", "ACCWT", "DIV"), ("V55_7Y",)):
        prepared = (inputs_by_baseline[baselines[0]] if inputs_by_baseline is not None else
                    prepare_prediction_inputs(model_config(
                        baselines[0], daily_df=daily_df, weekly_df=weekly_df,
                        monthly_df=monthly_df, date_to_week=date_to_week)))
        overrides = _weekly_prediction_feature_overrides(
            prepared_inputs=prepared,
            weekly_df=weekly_df,
            date_to_week=date_to_week,
            requested_dates=requested_dates,
        )
        base_cache, override_cache, ranges = _two_stage_phase_a(
            daily_df=daily_df,
            weekly_df=weekly_df,
            monthly_df=monthly_df,
            date_to_week=date_to_week,
            ...
            n_workers=n_workers,
            baselines=baselines,
            predict_feat_overrides=overrides,
            prior_phase_cache=(prior_phase_by_baseline[baselines[0]]
                               if prior_phase_by_baseline is not None else None),
        )
        if test_ranges is not None and ranges != test_ranges:
            raise RuntimeError("Phase A family windows differ")
        test_ranges = ranges
        for baseline in baselines:
            prepared_by_baseline[baseline] = prepared
            base_caches[baseline] = base_cache
            override_caches[baseline] = override_cache

    def run_context(caches: Mapping[str, Any]) -> pd.DataFrame:
        with redirect_stdout(sys.stderr):
            return run_10y01_for_feature_window(
                daily_df=daily_df,
                weekly_df=weekly_df,
                monthly_df=monthly_df,
                date_to_week=date_to_week,
                feature_date=current_end,
                test_ranges=test_ranges,
                current_start=current_start,
                current_end=current_end,
                require_labels=False,
                ...
                prepared_by_baseline=prepared_by_baseline,
                include_full_context=True,
            )

    base_detail = run_context(base_caches)
    override_detail = run_context(override_caches)
    base_dates = base_detail["anchor_date"].astype(str).tolist()
    if base_dates != override_detail["anchor_date"].astype(str).tolist():
        raise RuntimeError("base/override Phase A dates differ")
    row_by_date = {day: index for index, day in enumerate(base_dates)}
    baselines = tuple(required_baselines())
    base_signs = {
        name: np.asarray(
            [int(item[name]) for item in base_detail["baseline_signs"]],
            dtype=np.int32,
        )
        for name in baselines
    }
    ...
    for day in requested_dates:
        row_index = row_by_date[day]
        override_signs = override_detail.iloc[row_index]["baseline_signs"]
        signs = {
            name: values[:row_index + 1].copy()
            for name, values in base_signs.items()
        }
        for name in baselines:
            signs[name][-1] = int(override_signs[name])
        prediction = int(build_prediction(signs, row_index + 1)[-1])
        records.append({"anchor_date": day, "prediction": prediction})
...
def generate_results(
    ...
    for request in requests:
        grouped.setdefault(
            (request["feature_date"][:7], request["monthly_cutoff_key"]), []
        ).append(request)
    predictions: dict[str, int] = {}
    for (month, _monthly_key), batch in sorted(grouped.items()):
        ...
        daily_input = daily.loc[
            daily["date"] <= pd.Timestamp(feature_end)
        ].copy()
        ...
        details = _run_month(
            ...
            requested_dates=tuple(sorted({
                item["feature_date"] for item in batch
            })),
...
def _trial_identity() -> dict[str, Any]:
    ...
    return {"schema": "10y01-monthly-pit-full-state-1", "code": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
...
def _inspect_state_archive(source: Any, max_rows: int, config_count: int) -> None:
    ...
    if os.fstat(source.fileno()).st_size > max_bytes:
        raise ContractError("rebuild_required: state exceeds input-derived size bound")
    with zipfile.ZipFile(source) as archive:
        infos = archive.infolist()
        if len(infos) != len(expected) or {info.filename for info in infos} != expected:
            raise ContractError("rebuild_required: invalid state archive members")
        for info in infos:
            if info.compress_type != zipfile.ZIP_STORED or info.flag_bits & 1 or info.file_size > max_bytes:
                raise ContractError("rebuild_required: state must contain bounded uncompressed arrays")
            with archive.open(info) as handle:
                ...
                if version == (1, 0):
                    ...
                    raise ContractError("rebuild_required: unsupported state array version")
                if dtype.hasobject or fortran:
                    raise ContractError("rebuild_required: executable or noncanonical state dtype")
                name = info.filename
                if name == "header.npy":
                ...
                if not valid:
                    raise ContractError("rebuild_required: state shape/dtype exceeds input-derived bound")
                count = 1
                for size in shape:
                    count *= size
                if handle.tell() + count * dtype.itemsize != info.file_size:
                    raise ContractError("rebuild_required: state array payload length mismatch")


def _save_trial_state(path: Path, header: dict, states: Mapping[str, Any]) -> None:
    """将本方案两族派生数组写入同一份新状态，禁止覆盖输入或已有状态。"""
    if path.exists():
        raise ContractError("rebuild_required: trial state output already exists")
    arrays = {
    ...
    try:
        with tempfile.NamedTemporaryFile("wb", dir=path.parent, delete=False) as handle:
            ...
            if handle.tell() > STATE_MAX_BYTES:
                raise ContractError("rebuild_required: state exceeds 16 MiB bound")
...
def generate_with_private_state(
    ...
    """复用双族全日期派生状态；月度PIT排名、当前行及原控制器仍独立重算。"""
    ...
    if args.state_input is not None:
        ...
        if set(header) != {"identity", "authority", "cutoff", "input_prefixes", "payload_sha256"}:
            raise ContractError("rebuild_required: invalid private state header")
        if header["identity"] != identity or header["authority"] != authority:
            raise ContractError("rebuild_required: private state identity mismatch")
        _date(header["cutoff"], "state cutoff")
        if set(header["input_prefixes"]) != {"daily_df", "weekly_df", "monthly_df", "calendar_df"}:
            raise ContractError("rebuild_required: missing input prefix proof")
        for name, proof in header["input_prefixes"].items():
            if (set(proof) != {"rows", "sha256"} or type(proof["rows"]) is not int
                ...
                raise ContractError("rebuild_required: invalid input prefix proof")
        ...
        if header["payload_sha256"] != _payload_digest(*arrays):
            raise ContractError("rebuild_required: private state payload mismatch")
        for family, state in states.items():
            ...
            if (dates.ndim != 1 or dates.dtype != np.dtype("U10") or not len(dates)
                ...
                raise ContractError("rebuild_required: invalid private state arrays")
    answers = {}
    ...
    for (day, week, month), batch in sorted(by_cutoff.items()):
        ...
        if header is not None and day < header["cutoff"]:
            raise ContractError("rebuild_required: future state cannot serve historical Request")
            ...
            for name, proof in header["input_prefixes"].items():
                ...
                if name == "monthly_df":
                    ...
                    if len(previous_frame) != proof["rows"]:
                        raise ContractError("rebuild_required: consumed monthly domain changed; explicit rebuild required")
                if len(previous_frame) < proof["rows"] or _digest_frame(previous_frame.iloc[:proof["rows"]]) != proof["sha256"]:
                    raise ContractError("rebuild_required: source prefix changed; explicit rebuild required")
        phase_by_baseline = {}
        ...
        for family, baselines in PHASE_FAMILIES.items():
            ...
            if keep and (len(new_dates) < keep or not np.array_equal(new_dates[:keep], dates)):
                raise ContractError("rebuild_required: OOS date prefix changed")
            if len(features):
                if len(new_features) < len(features):
                    raise ContractError("rebuild_required: feature prefix shortened")
        ...
        detail = _run_month(
            **raw, requested_dates=(day,),
            current_start=day[:7] + "-01", current_end=day, n_workers=4,
            prior_phase_by_baseline=phase_by_baseline,
            inputs_by_baseline=prepared_by_baseline)
        if len(detail) != 1 or str(detail.iloc[0]["anchor_date"]) != day:
            raise ContractError("missing exact monthly-PIT result")
...
def main() -> int:
    ...
    try:
        ...
        if args.state_input is not None:
            if args.state_output is None or not args.state_input.is_file() or args.state_input.is_symlink():
                raise ContractError("rebuild_required: state-input needs an existing regular file and fresh state-output")
        requests = read_requests(args)
        snapshot = read_snapshot(args.data_dir, requests)
        if args.state_output is None:
            rows = generate_results(requests, *snapshot)
        else:
            rows = generate_with_private_state(args, requests, *snapshot)
        emit(args, rows)
        succeeded = True
        return 0
    except (ContractError, OSError, ValueError, KeyError) as exc:
        prefix = "rebuild_required: " if args is not None and args.state_input is not None else ""
