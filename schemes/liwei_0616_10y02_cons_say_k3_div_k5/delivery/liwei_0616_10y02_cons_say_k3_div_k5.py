#!/usr/bin/env python3
"""10Y02 SAY full-OOS 的自包含 Blackbox V2 私有状态草稿。

从2024-01-01累计完整OOS历史，保留Native三方共识与DIV连续方向阈值5。
...
SOURCE_MODEL_ID = "10Y_02_cons_SAY_k_3_DIV_K_5"
TARGET_TENOR = "10Y"
...
def model_config(baseline: str = "STD", **overrides) -> dict[str, Any]:
    """返回 10Y_02 某个 baseline 的固定参数副本。"""
    if baseline not in BASELINE_CONFIGS:
        raise KeyError(f"unknown 10Y_02 baseline: {baseline}")
...
def run_config(config: dict) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"10Y02 LightGBM config failed: {config!r}") from exc
...
def run_10y02_for_feature_window(
    *,
    ...
    """运行 10Y_02 PIT 窗口，返回 current window 内逐 feature_date 明细。"""
    ...
    for baseline in required_baselines():
        ...
        if not isinstance(ctx, dict) or len(ctx.get("test_dates", [])) == 0:
            raise RuntimeError(f"10Y_02 baseline {baseline} produced no context rows")
...
def _trial_identity() -> dict[str, Any]:
    ...
    return {"schema": "10y02-full-oos-private-1", "code": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
...
def generate_with_private_state(
    ...
    for (day, week, month), batch in sorted(by_cutoff.items()):
        ...
        with redirect_stdout(sys.stderr):
            detail = run_10y02_for_feature_window(**raw, test_ranges=(("2024-01-01", day),),
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="liwei_0616 10Y02 Blackbox V2 successor"
    )
