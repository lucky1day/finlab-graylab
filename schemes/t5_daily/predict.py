...
def _target_date_from_feature_date(feature_date: str, horizon: int, engine: Engine) -> str:
    """按原始 T+5 标签语义，取特征日后第 horizon 个交易日。"""
    return get_calendar(engine=engine).nth_trading_day_after(feature_date, horizon)
...
def run(predict_date: str) -> list[PredictionRecord]:
    ...
    try:
        ...
        for tenor in ("3Y", "5Y", "7Y", "10Y"):
            ...
            target_date = _target_date_from_feature_date(result.feature_date, HORIZON, engine)
            records.append(
                PredictionRecord(
                    scheme_id=SCHEME_ID,
                    target_tenor=tenor,
