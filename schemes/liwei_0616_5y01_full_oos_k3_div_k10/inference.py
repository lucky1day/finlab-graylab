...
CACHE_CONSUMER_ID = "liwei_0616_5y01_full_oos_k3_div_k10"
CACHE_PUBLISHER_CONSUMER_ID = (
    "liwei_0616_5y01_full_oos_k3_div_k10"
)
MODEL_VERSION = "liwei_0616_5y01_full_oos_v1"


@dataclass(frozen=True)
class PitWindow:
    """5Y01 source-original continuous full-OOS window."""
...
def full_oos_window(
    ...
    *,
    ...
    """Return the original single continuous full-OOS source sequence."""
...
def run_for_feature_date(
    *,
    ...
    """Run 5Y01 full-OOS and select one exact feature date."""
    ...
    if matched.empty:
        raise RuntimeError(f"liwei_0616 5Y01 full-OOS produced no row for feature_date={feature_date}")
...
def run_for_window_silent(**kwargs: Any) -> pd.DataFrame:
    ...
    with redirect_stdout(sys.stderr):
        return run_5y01_for_feature_window(**kwargs)
