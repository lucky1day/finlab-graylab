#!/usr/bin/env python3
"""Liwei 5Y full-OOS 的独立 Blackbox V2 增量实现。

仅消费平台 Request、DataBridge 输入和受控私有派生状态。
保存可重建的 Phase-A 聚合值；标签、排名、信号和控制器仍按 cutoff 重新计算。
历史依赖修订时明确拒绝复用，不访问 Native cache 或平台代码。
"""
...
def run_prediction(cfg: dict) -> np.ndarray | pd.DataFrame | dict[str, Any]:
    ...
    return _prediction_detail_frame(
        df, test_idx, np.sign(vs_full).astype(np.int32), true_labels,
        ens_prob, ml_base, sig_avg, has_sig, vs_full,
    )
...
def _trial_identity() -> dict[str, Any]:
    """试点身份；正式平台接口必须在 A2 后另行冻结。"""
    import lightgbm
    numba_version = None
    if _HAS_NUMBA:
        import numba
        numba_version = numba.__version__
    return {"schema": "full-oos-a2-private-4", "code": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
...
def _inspect_state_archive(source: Any, max_rows: int, config_count: int) -> None:
    """先验证未压缩 NPY 成员及声明形状，再允许 NumPy 分配数组。"""
    expected = {"header.npy", "dates.npy", "features.npy", "preds.npy", "probs.npy"}
    # 按当前输入行数、实际265配置和冻结dtype推导容量；header实测不足4KiB，格式限制16K字符。
    max_bytes = max_rows * (40 + 256 + config_count * 9) + 65536 + 8192
    ...
    with zipfile.ZipFile(source) as archive:
        ...
        for info in infos:
            ...
            with archive.open(info) as handle:
                ...
                elif name in {"dates.npy", "features.npy"}:
                    want = np.dtype('U10' if name == 'dates.npy' else 'U64')
                    valid = len(shape) == 1 and 0 < shape[0] <= max_rows and dtype == want
                else:
                    want = np.dtype('int8' if name == 'preds.npy' else 'float64')
...
def _save_trial_state(path: Path, header: dict, dates: np.ndarray,
                      features: np.ndarray, preds: np.ndarray, probs: np.ndarray) -> None:
    """只写本次私有新快照，不覆盖已有文件或原输入。"""
    if path.exists():
        raise ContractError("trial state output already exists")
    header = header | {"payload_sha256": _payload_digest(dates, features, preds, probs)}
    temporary = None
    published = False
    try:
        with tempfile.NamedTemporaryFile("wb", dir=path.parent, delete=False) as handle:
            temporary = Path(handle.name)
            np.savez(handle, header=np.array(json.dumps(header, sort_keys=True)),
                     dates=dates, features=features, preds=preds, probs=probs)
            handle.flush()
            os.fsync(handle.fileno())
        # 私有试点拒绝覆盖；生产锁与发布仅在 A3 接入。
...
def generate_with_private_state(args: argparse.Namespace, requests: list[dict[str, str]],
                                daily: pd.DataFrame, weekly: pd.DataFrame,
                                monthly: pd.DataFrame, calendar: pd.DataFrame) -> list[dict[str, Any]]:
    """按 cutoff 向前推进全 grid；仅用于 A1 私有目录，未接入平台。"""
    ...
    grid = build_lgbm_grid(*[model_config()[key] for key in
    ...
    if args.state_input is not None:
        with args.state_input.open('rb') as source:
            _inspect_state_archive(source, len(daily), len(grid))
            source.seek(0)
            with np.load(source, allow_pickle=False) as loaded:
                if set(loaded.files) != {"header", "dates", "features", "preds", "probs"}:
                    raise ContractError("invalid private state fields")
                header = json.loads(str(loaded["header"].item()))
                dates, features = loaded["dates"].copy(), loaded["features"].copy()
                preds, probs = loaded["preds"].copy(), loaded["probs"].copy()
        ...
        for proof in header["input_prefixes"].values():
            if (set(proof) != {"rows", "sha256"} or type(proof["rows"]) is not int
                    or proof["rows"] <= 0 or not re.fullmatch(r"[0-9a-f]{64}", proof["sha256"])):
        ...
        if (dates.ndim != 1 or dates.dtype != np.dtype('U10') or not len(dates)
                ...
                or header["payload_sha256"] != _payload_digest(dates, features, preds, probs)
                or preds.dtype != np.dtype('int8') or probs.dtype != np.dtype('float64')
                or preds.shape != (len(grid), len(dates)) or probs.shape != preds.shape
    ...
    for (day, week, month), batch in sorted(by_cutoff.items()):
        ...
        raw = {"daily_df": daily.loc[daily.date <= pd.Timestamp(day)].reset_index(drop=True),
               ...
               "date_to_week": date_to_week}
        # 只证明真实算法依赖；当月输入尚未被上一月对齐消费，不冻结为历史依赖。
        ...
        if header is not None:
            for name, proof in header["input_prefixes"].items():
                ...
                if name == "monthly_df":
                    # 补回旧支持域中缺失的末月也属于历史修订，不能冒充正常跨月追加。
        ...
        df = prepared["df"]
        # 相同输入下逐行 SHA 仅保存摘要。相较保存控制器、标签和多种窗口，先选择较少状态种类。
        ...
        if not len(new_dates) or new_dates[-1] != day:
            raise ContractError("no exact current full-OOS row")
        keep = len(dates)
        ...
        if len(features):
            ...
            if len(changed):
                # 历史输入未修订时，周频投影仍可能改变旧尾部；保守重算全部受影响 suffix。
        ...
        # 每个自然月一个训练调用，避免 Dataset 临时字典跨全历史无界积累。
        for month_key in sorted({value[:7] for value in remaining}):
            ...
            context = _train_phase_a(**raw, prepared_inputs=prepared,
                                     test_range=(rows[0], rows[-1]), n_workers=4)
        ...
        dates, features = new_dates, new_features
        phase = {"test_dates": dates.tolist(), "results": [
            {"config": config, "preds": preds[index], "probs": probs[index]}
            for index, config in enumerate(grid)]}
        with redirect_stdout(sys.stderr):
            detail = run_5y01_for_feature_window(**raw, test_ranges=(("2024-01-01", day),),
                        current_start=day, current_end=day, require_labels=False, n_workers=4,
                        phase_a_caches={name: phase for name in PROD_CONFIG["baselines"]},
        ...
        print(json.dumps({"trial_cutoff": day, "oos_rows": len(dates), "reused_rows": keep,
                          "trained_rows": len(remaining), "seconds": time.perf_counter() - started}), file=sys.stderr, flush=True)
    if args.state_output is not None:
        _save_trial_state(args.state_output, header, dates, features, preds, probs)
