#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""cgb_a4_fundseason_10y_hl18 —— 10Y 国债月频方向算法方案 (Blackbox V2 / Contract 1.0)。

单文件交付物。两个命令复用完全相同的数据处理、算法逻辑与方向映射:

    python cgb_a4_fundseason_10y_hl18.py predict  --request  request.json  \
        --data-dir <data-dir> --output prediction.json
    python cgb_a4_fundseason_10y_hl18.py backtest --requests requests.csv  \
...
4. 5 个期限 (1Y/3Y/5Y/7Y/10Y) 池化为一个样本表 + 期限哑变量; 用趋势状态核
   w=exp(-d^2/2h^2) 乘以 18 个月指数时间衰减 0.5^(age/18) 对历史月加权, 再对权重做
   均值归一化, 训练 Logistic (C=0.3, class_weight=balanced, liblinear);
   只用 month_id < 当前特征月 且有已实现标签的行做训练。
5. 取 10Y 行的概率 p: p>=0.5 -> predicted_direction=+1 (收益率上行), 否则 -1。
   二分类, 不输出 0。
...
周频截止键
----------
daily_cutoff_key / monthly_cutoff_key 按 SOP §6.3 精确定位, 缺失即失败。
weekly_cutoff_key 额外允许一种情况: 本方案在月中 (15 日) 运行, 截止周当时往往尚未收周、
还没进入 weekly_output.csv。此时收缩到 **不晚于截止周的最新可用周**, 照常出预测, 并在
stderr 记录实际使用的周键; 与生产入口 run_monthly.py 的 allow_weekly_before_cutoff=True
同语义。该放宽只会把可见边界往更早移, 不会读到晚于截止周的任何一行, 也不伪造行或改写
week_id; 周键本身的正确性仍由 api_wind_date.csv 的 daily_cutoff_key -> weekly_cutoff_key
映射强校验。若连一条不晚于截止周的行都没有, 仍然失败。
"""
...
SCHEME_ID = "cgb_a4_fundseason_10y_hl18"
...
HALFLIFE_MONTHS = 18.0            # v1.1.0: 训练样本时间衰减半衰期(月), 安全带 15~21
...
def _truncate_at_key(frame: pd.DataFrame, key_col: str, cutoff: str,
                     filename: str,
                     allow_latest_before_cutoff: bool = False) -> pd.DataFrame:
    """保留首行至截止键所在行 (含)。时间键必须非空、唯一、升序。

    allow_latest_before_cutoff 只对 weekly_output.csv 打开, 且只允许把可见边界
    **往更早** 移: 截止周尚未出现在文件中时, 收缩到不晚于截止周的最后一行, 不伪造
    任何行、不改写 week_id、绝不会看到晚于截止周的数据。见本文件顶部 "周频截止键"。
    """
    keys = frame[key_col]
    ...
    if hit.size == 1:
        stop = int(hit[0])
    elif allow_latest_before_cutoff:
        eligible = np.flatnonzero(keys.le(cutoff).to_numpy(dtype=bool))
        if eligible.size == 0:
            raise SchemeError(
                f"{filename} 中没有任何不晚于截止键 {key_col}={cutoff} 的行")
        stop = int(eligible[-1])
        _log(f"{filename}: 截止键 {key_col}={cutoff} 尚未出现在文件中, "
             f"按周频未发布处理, 退用不晚于它的最新可用键 {keys.iloc[stop]}")
    else:
        raise SchemeError(f"{filename} 中不存在截止键 {key_col}={cutoff}")
    return frame.iloc[: stop + 1].reset_index(drop=True)
...
def load_calendar(data_dir: Path, req: Dict[str, str]) -> pd.Series:
    """平台制品 api-wind-date-v1: week_id -> 周末日期。只从 --data-dir 读取。

    SOP §1.3/§6.1: 日历由平台在 Intake 时以 --platform-input api-wind-date-v1 声明并
    随 --data-dir 提供。本脚本不内嵌日历、不读脚本同目录、不做网络或数据库 fallback;
    文件缺失、列不合法或截止映射不一致时一律 fail-closed。

    日历不按截止键截断: 它只提供"某个 week_id 对应的周末自然日", 用完整日历得到
    的是该周真实的周末日; 截断反而会把周末日提前到截止日, 造成周频数据提前可见。
    """
    path = data_dir / CALENDAR_FILENAME
    if not path.is_file():
        raise SchemeError(
            f"缺少平台周历制品 {CALENDAR_FILENAME} (需要 week_id -> 周末日期 映射); "
            f"该文件必须由平台在 --data-dir 中提供")
    try:
        cal = pd.read_csv(path, dtype="string", encoding="utf-8-sig",
                          keep_default_na=False)
    except (OSError, ValueError) as exc:
        raise SchemeError(f"无法读取 {CALENDAR_FILENAME}: {exc}") from exc
    if list(cal.columns) != ["rdate", "week_id"]:
        raise SchemeError(
            f"{CALENDAR_FILENAME} 的列必须恰好是 rdate,week_id, 实际是 "
            f"{list(cal.columns)}")
    if cal.empty:
        raise SchemeError(f"{CALENDAR_FILENAME} 没有任何记录")
    rdate = cal["rdate"].astype("string").str.strip()
    week_id = cal["week_id"].astype("string").str.strip()
    if rdate.eq("").any() or rdate.duplicated().any():
        raise SchemeError(f"{CALENDAR_FILENAME} 的 rdate 必须非空且唯一")
    try:
        parsed = pd.to_datetime(rdate, format="%Y-%m-%d", errors="raise")
    except (ValueError, TypeError) as exc:
        raise SchemeError(f"{CALENDAR_FILENAME} 的 rdate 必须是规范 YYYY-MM-DD") from exc
    if not parsed.is_monotonic_increasing:
        raise SchemeError(f"{CALENDAR_FILENAME} 的 rdate 必须严格升序")
    if not week_id.str.fullmatch(r"\d{6}").all():
        raise SchemeError(f"{CALENDAR_FILENAME} 的 week_id 必须是六位数字字符串")
    hit = np.flatnonzero(rdate.eq(req["daily_cutoff_key"]).to_numpy())
    if hit.size != 1:
        raise SchemeError(
            f"{CALENDAR_FILENAME} 中 daily_cutoff_key={req['daily_cutoff_key']} "
            f"必须恰好映射到一个 week_id (实际命中 {int(hit.size)} 行)")
    mapped = str(week_id.iloc[int(hit[0])])
    if mapped != req["weekly_cutoff_key"]:
        raise SchemeError(
            f"日历把 daily_cutoff_key={req['daily_cutoff_key']} 映射到 week_id={mapped}, "
            f"与 Request 的 weekly_cutoff_key={req['weekly_cutoff_key']} 不一致")
    week_end = pd.DataFrame({"rdate": parsed.to_numpy(), "week_id": week_id}) \
        .groupby("week_id")["rdate"].max()
    if req["weekly_cutoff_key"] not in week_end.index:
        raise SchemeError(
            f"{CALENDAR_FILENAME} 未覆盖 weekly_cutoff_key={req['weekly_cutoff_key']}, "
...
def predict_direction(data_dir: Path, req: Dict[str, str]) -> int:
    ...
    # 先用平台周历校验 daily_cutoff_key -> weekly_cutoff_key 的映射, 再按周键截断周频:
    # 周频允许回退到"不晚于截止周的最新可用周", 因此必须先确认这个周键本身是对的,
    # 不能在一个未经校验的周键上做回退。
    week_end = load_calendar(data_dir, req)
    weekly = _load_keyed(data_dir, "weekly_output.csv", CONSUMED_WEEKLY,
                         req["weekly_cutoff_key"],
                         allow_latest_before_cutoff=True)
    ...
    # v1.1.0 / P2.2 训练样本时间衰减 (半衰期 HALFLIFE_MONTHS 个月)。
    # 依据: 冻结训练段(2016-2021)与冻结后(2022-2026)的因子 IC 相关系数仅 0.028,
    # 八个通道中四个 IC 符号反转, 因子关系约每一年半换一轮, 因此给近期月份更高权重。
    # 真 OOS(2022-01~2026-07) 上半衰期 12~24 个月构成平台, 15/21 两侧同号为正。
    age_months = (
        (int(month_id) // 100 * 12 + int(month_id) % 100)
        - (hist["month_id"].astype(int) // 100 * 12 + hist["month_id"].astype(int) % 100)
    ).to_numpy(dtype=float)
    decay = 0.5 ** (np.maximum(age_months, 0.0) / float(HALFLIFE_MONTHS))
    w = decay if w is None else w * decay

    # v1.1.0 / P0.1 归一化 sample_weight: 把有效正则强度 C x sum(w) 钉死,
    # 消除 sklearn 在 class_weight x sample_weight 归一化约定上的版本自由度。
...
def cmd_backtest(args: argparse.Namespace) -> None:
    ...
    # 用 csv.writer 序列化: 任何字段里出现逗号/引号/换行时也能被平台原样解析回来
    # (SOP §7 "四个 Request 字段必须原样回传")。对不含特殊字符的常规值输出完全相同。
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=f"{SCHEME_ID}.py",
        description=(f"{TARGET_TENOR} 国债月频方向算法方案 (Blackbox V2 Contract 1.0)。"
...
def _validate_output_path(output: Path, data_dir: Path) -> None:
    """SOP §5: 脚本不得写入 --data-dir。--data-dir 是只读快照。"""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        _validate_output_path(Path(args.output), data_dir)
        args.func(args)
