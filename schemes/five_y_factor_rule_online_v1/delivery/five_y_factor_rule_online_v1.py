#!/usr/bin/env python3
"""Blackbox V2 Contract 1.0: fixed 5Y T+1 factor/rule ensemble."""
...
TRAIN_WINDOW = 756
SLOPE_COEFFICIENT = 0.75
FACTOR_SCALE = 0.0035967111348445
RULE_WEIGHT = 0.60
SIGNAL_THRESHOLD = 0.3357620921777115
CONTROLLER_LOOKBACK = 10
CONTROLLER_MIN_OBSERVATIONS = 5
...
SLOPE_FEATURES = (
    "bond_3Y_ret_lag1", "rv_10Y_d1", "bond_10Y_ret_lag1",
    "bond_5Y_sign_lag1", "bond_3Y_z2", "bond_3Y_sign_lag1", "bond_5Y_z2",
    "bond_3Y_streak_count", "bond_5Y_z3", "bond_3Y_z3",
    "bond_10Y_sign_lag1", "bond_10Y_z2", "bond_5Y_ret_lag1",
    "bond_10Y_z3", "bond_3Y_z5", "bond_3Y_mom_sum2",
    "bond_10Y_streak_count", "bond_5Y_streak_count", "bond_3Y_mom_sign2",
    "bond_10Y_z5", "bond_spread_5Y_10Y", "rv_10Y_d2", "bond_3Y_z10",
    "bond_5Y_z5", "bond_10Y_mom_sum2", "bond_3Y_mom_sign3",
    "bond_10Y_mom_sign2", "bond_5Y_mom_sum2", "bond_3Y_mom_sum3",
    "bond_3Y_mom_sum5",
)

LOG = logging.getLogger("five_y_factor_rule_online_v1")
...
def bond_feature_set(daily: pd.DataFrame) -> pd.DataFrame:
    ...
    needed = set(LEVEL_FEATURES) | set(SLOPE_FEATURES)
    ...
    missing = (set(LEVEL_FEATURES) | set(SLOPE_FEATURES)) - set(frame.columns)
    if missing:
...
def base_predictions(
    ...
    slope_target = delta_10y - delta_5y
    level = factor_forecast(features, level_target, dates, positions, LEVEL_FEATURES)
    slope = factor_forecast(features, slope_target, dates, positions, SLOPE_FEATURES)
    raw_factor = level - SLOPE_COEFFICIENT * slope

    slope_change = daily["TB0YWI0C"].diff() - daily["TB5YWI0C"].diff()
    aggregate = slope_change.rolling(10, min_periods=10).sum()
    scale = aggregate.shift(1).rolling(60, min_periods=20).std().replace(0.0, np.nan)
    rule = (aggregate / scale).to_numpy(np.float64)[positions]
    factor_z = np.clip(raw_factor / FACTOR_SCALE, -5.0, 5.0)
    rule_z = np.clip(np.nan_to_num(rule, nan=0.0), -5.0, 5.0)
    score = (1.0 - RULE_WEIGHT) * factor_z + RULE_WEIGHT * rule_z
    directions = np.where(
        score > SIGNAL_THRESHOLD, 1,
        np.where(score < -SIGNAL_THRESHOLD, -1, 0),
    ).astype(np.int8)
    labels = np.sign(delta_5y[positions]).astype(np.float64)
    return directions, labels, raw_factor, rule_z, score
...
def controller_decision(
    ...
    *,
    ...
    earliest_month: pd.Period,
) -> tuple[bool, int, int, float | None]:
    """返回是否已决、最终方向、控制器样本数和后验概率。"""
    current_direction = int(directions[-1])
    if current_direction == 0:
        return True, 0, 0, None
    traded_history = np.flatnonzero(directions[:-1] != 0)
    if len(traded_history) < CONTROLLER_LOOKBACK and current_month > earliest_month:
        return False, 0, len(traded_history), None
    chosen = traded_history[-CONTROLLER_LOOKBACK:]
    if len(chosen) < CONTROLLER_MIN_OBSERVATIONS:
        return True, current_direction, len(chosen), None
    if not np.isfinite(labels[chosen]).all():
        raise ContractError("controller history contains an unavailable label")
    correct = int((directions[chosen] == labels[chosen]).sum())
    posterior = (correct + 5.0) / (len(chosen) + 10.0)
    if posterior > 0.5:
        ...
        final_direction = 0
    return True, final_direction, len(chosen), posterior
...
def prediction_trace(
    *,
    ...
    controller_observations: int,
    controller_posterior: float | None,
) -> dict[str, Any]:
    return {
        "base_direction": int(base_direction),
        "final_direction": int(final_direction),
        "raw_factor": float(raw_factor),
        "rule_score": float(rule_score),
        "combined_score": float(combined_score),
        "controller_observations": int(controller_observations),
        "controller_posterior": (
            None if controller_posterior is None else float(controller_posterior)
        ),
    }


def predict_trace(daily: pd.DataFrame) -> tuple[int, dict[str, Any]]:
    features = bond_feature_set(daily)
    dates = pd.DatetimeIndex(daily["date"])
    current_position = len(daily) - 1
    current_month = dates[current_position].to_period("M")
    earliest_month = SIM_START.to_period("M")
    while True:
        start = max(current_month.to_timestamp(), SIM_START)
        selected_positions = np.flatnonzero((dates >= start) & (dates <= dates[current_position]))
        directions, labels, raw_factor, rule_score, combined_score = base_predictions(
            daily,
            features,
            selected_positions,
        )
        resolved, final_direction, observations, posterior = controller_decision(
            directions,
            labels,
        ...
        if resolved:
            trace = prediction_trace(
                base_direction=int(directions[-1]),
                final_direction=final_direction,
                raw_factor=float(raw_factor[-1]),
                rule_score=float(rule_score[-1]),
                combined_score=float(combined_score[-1]),
                controller_observations=observations,
                controller_posterior=posterior,
            )
            return final_direction, trace
        current_month -= 1
...
def batch_predict_traces(
    clipped_inputs: list[pd.DataFrame],
) -> tuple[list[int], list[dict[str, Any]]]:
    """在最大因果截止内按月训练一次，返回每个 Request 的等价结果。"""
    longest = max(clipped_inputs, key=len)
    features = bond_feature_set(longest)
    dates = pd.DatetimeIndex(longest["date"])
    positions = np.flatnonzero((dates >= SIM_START) & (dates <= dates[-1]))
    directions, labels, raw_factor, rule_score, combined_score = base_predictions(
        longest,
        features,
        positions,
    ...
    earliest_month = SIM_START.to_period("M")
    output_directions: list[int] = []
    traces: list[dict[str, Any]] = []
    for clipped in clipped_inputs:
        current_position = len(clipped) - 1
        current_month = dates[current_position].to_period("M")
        current_local = int(np.flatnonzero(positions == current_position)[0])
        search_month = current_month
        while True:
            start = max(search_month.to_timestamp(), SIM_START)
            selected = np.flatnonzero(
                (dates[positions] >= start)
                & (dates[positions] <= dates[current_position])
            )
            resolved, final_direction, observations, posterior = controller_decision(
                directions[selected],
                labels[selected],
            ...
            if resolved:
                output_directions.append(final_direction)
                traces.append(
                    prediction_trace(
                        base_direction=int(directions[current_local]),
                        final_direction=final_direction,
                        raw_factor=float(raw_factor[current_local]),
                        rule_score=float(rule_score[current_local]),
                        combined_score=float(combined_score[current_local]),
                        controller_observations=observations,
                        controller_posterior=posterior,
                    )
                )
                break
            search_month -= 1
    return output_directions, traces
...
def results_for_requests(
    ...
    for index in sample_indices:
        ...
        if directions[index] != expected_direction or traces[index] != expected_trace:
    ...
    LOG.info(
        ...
        [requests[index]["request_id"] for index in sample_indices],
    )
    return [
