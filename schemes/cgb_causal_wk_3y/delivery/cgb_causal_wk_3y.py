#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""cgb_causal_wk_3y —— 3Y 国债周频方向算法方案 (Blackbox V2 / Contract 1.0)。

单文件交付物。两个命令复用完全相同的数据处理、算法逻辑与方向映射:

    python cgb_causal_wk_3y.py predict  --request  request.json \
        --data-dir <data-dir> --output prediction.json
    python cgb_causal_wk_3y.py backtest --requests requests.csv \
        --data-dir <data-dir> --output backtest.csv

算法概要 (CV2.0.0-rc1)
--------
1. 周频面板: 以 weekly_output.csv 的 week_id 为主键; 51 个因果指标按"通道(5) x
   子通道"组织, 逐码按冻结的来源频率取值(44 个直接取周频列, 7 个由日频按周聚合,
   聚合口径 mean/last 冻结), 其中 4 个为对基准码的利差。本方案不消费月频文件。
...
3. CV2 三年期专属流动性因子(7 维, 不参与通道聚合): R007/1Y同业存单/MLF 三个资金
   价格分别减 ZY000075 得到利差, 各取 level 与 delta 的 52 周 z; 另加
   "TB3YWI3C 4 周动量 z x R007 水平 z" 的交互项。全部 clip 到 [-3,3]。
4. 标签 y=+1 当且仅当下一周 TB3YWI3C 高于本周(阈值 0)。TB3YWI3C 是
   3Y 活跃券周度收盘观测, 单位为到期收益率(%)，因此 y=+1 即"收益率上行"。
5. Walk-forward: 对每个特征周, 取该周之前标签已实现且非持平的样本, 只保留最近
   156 条(滚动窗口, 非扩张窗口), 按 exp(-ln2 * 周龄 / 104) 做半衰期加权, 并对
   y=+1 的样本再乘 0.8; 用该权重训练 Logistic(C=0.6, liblinear, balanced) 与
   RidgeClassifier(alpha=1.0), 65 维特征(51 因子 + 7 CV2 流动性 + 5 通道 + 2 汇总);
   两模型上行概率取均值 p。
6. 方向映射: p>=0.50 -> +1, 否则 -1; 随后应用 CV2 低置信反转政策 —— 若
   |p-0.5| <= 0.10 则把该周方向取反。本方案不使用任何外部 base 预测。
...
# --------------------------------------------------------------------------- #
TARGET_COL = "TB3YWI3C"          # 预测标的: 活跃券周度收盘观测(到期收益率 %)
YIELD_COL = "ZY000075"            # 同期限中债到期收益率周处理值 (诊断/极端态判定)
...
# CV2.0.0-rc1 冻结常量 (对应研究基准 weekly_3y_causal_cv2_0_0_rc1.py)
CV2_TRAIN_WINDOW = 156                 # 训练用滚动窗口(条), 非扩张窗口
CV2_HALF_LIFE = 104                    # 样本时间衰减半衰期(周)
CV2_UP_MULTIPLIER = 0.8                # y=+1 样本的额外权重系数
CV2_DECISION_THRESHOLD = 0.50          # 概率 -> 方向的阈值
CV2_LOW_CONF_GATE = 0.10               # 低置信带宽 |p-0.5| <= 该值时触发反转
CV2_LOW_CONF_POLICY = "flip"           # 低置信政策: 反转方向

# CV2 三年期专属流动性因子: 资金价格相对本期限收益率的利差
CV2_LIQUIDITY_GAPS = (
    ("three_y_r007_gap", "M1001795"),   # R007
    ("three_y_cd1y_gap", "M1014902"),   # AAA 同业存单 1Y
    ("three_y_policy_gap", "M1543249"), # MLF 1Y
)
CV2_MOMENTUM_FUNDING_CODE = "M1001795"  # 交互项里的资金水平锚
CV2_MOMENTUM_LAG = 4                    # 交互项里的目标动量回看周数
...
def build_feature_frame(weekly: pd.DataFrame, daily: pd.DataFrame) -> pd.DataFrame:
    ...
    cv2_cols = add_cv2_liquidity_features(features, weekly)
    features.attrs["model_feature_cols"] = pressure_cols + cv2_cols + module_cols + [
...
def add_cv2_liquidity_features(features: pd.DataFrame, weekly: pd.DataFrame) -> List[str]:
    """CV2 三年期专属流动性因子; 就地写入 features, 返回新增列名(顺序与研究基准一致)。

    这些列不在 CAUSAL_SPECS 目录里, 因此不参与通道聚合, 只进入模型特征矩阵。
    """
    anchor = safe_numeric(weekly[YIELD_COL])
    added: List[str] = []
    for prefix, code in CV2_LIQUIDITY_GAPS:
        if code not in weekly.columns:
            continue
        gap = safe_numeric(weekly[code]).sub(anchor)
        for suffix, series in (("level", rolling_z(gap)), ("delta", rolling_z(gap.diff()))):
            column = f"{prefix}_{suffix}_yield_pressure"
            features[column] = clipped(series)
            added.append(column)

    momentum = rolling_z(safe_numeric(weekly[TARGET_COL]).diff(CV2_MOMENTUM_LAG))
    funding = rolling_z(safe_numeric(weekly[CV2_MOMENTUM_FUNDING_CODE]))
    interaction = "three_y_momentum_funding_interaction_yield_pressure"
    features[interaction] = clipped(momentum.mul(funding))
...
def fit_walk_forward(features: pd.DataFrame, labels: pd.DataFrame, request_id: str) -> Dict[str, float]:
    ...
    # CV2: 只保留最近 CV2_TRAIN_WINDOW 条(滚动窗口), 再判断样本量是否达标。
    train_idx = train_idx[-CV2_TRAIN_WINDOW:]
    ...
    # CV2: 半衰期时间衰减 + y=+1 样本降权。
    ages = last - train_idx
    sample_weight = np.exp(-math.log(2.0) * ages / CV2_HALF_LIFE)
    sample_weight = sample_weight * np.where(y_train == 1, CV2_UP_MULTIPLIER, 1.0)
    ...
    probs: List[float] = []
    try:
        logit.fit(x_train, y_train, model__sample_weight=sample_weight)
    ...
    try:
        ridge.fit(x_train, y_train, model__sample_weight=sample_weight)
    ...
    return {
        "prob_up": prob_up,
        "pred_label": 1.0 if prob_up >= CV2_DECISION_THRESHOLD else -1.0,
...
# 单条 Request 的完整算法
# --------------------------------------------------------------------------- #
def apply_cv2_confidence_policy(prob_up: float, pred_label: float) -> Tuple[float, str]:
    """CV2 低置信反转政策: |p-0.5| <= CV2_LOW_CONF_GATE 时把方向取反。"""
    if pred_label not in (-1.0, 1.0):
        return pred_label, "model"
    if abs(prob_up - 0.5) <= CV2_LOW_CONF_GATE:
        return -pred_label, CV2_LOW_CONF_POLICY
    return pred_label, "model"
...
def predict_one(frames: Dict[str, Any], request: Dict[str, str]) -> int:
    ...
    direction, policy = apply_cv2_confidence_policy(model["prob_up"], model["pred_label"])
    ...
    _log(
        ...
        f"raw={int(model['pred_label'])} policy={policy} direction={int(direction)}"
    )
