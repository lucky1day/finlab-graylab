...
from .screen_metrics import feat_screen
...
def run_prediction(cfg: dict) -> np.ndarray | tuple[np.ndarray, dict[str, Any]]:
    ...
    print(
        f"  {SCREEN_METRIC.upper()} screening on source pre-test data "
        "(strict gap)..."
    )
...
def run_5y_all_for_feature_window(
    *,
    ...
    """运行 5Y ALL_K10 PIT 窗口，返回 current window 内逐 feature_date 明细。"""
    ...
    for baseline in required_baselines():
        ...
        if not isinstance(ctx, dict) or len(ctx.get("test_dates", [])) == 0:
            raise RuntimeError(
                f"5Y ALL_K10 baseline {baseline} produced no context rows"
            )
