#!/usr/bin/env python3
"""5Y AUC yearly ALL_K10 full-OOS 的自包含 Blackbox V2 私有状态草稿。

从2024-01-01累计完整OOS历史，保留active platform_live_pit_variant四方共识与DIV连续方向阈值10。
5Y与7Y独立维护Phase A派生数组；仅通过显式state-input/state-output读取发布。
每年按源年度首个可用行减horizon截止筛选；每个Request重算受周频投影影响的尾部和控制器。
...
MODEL_VERSION = "liwei_0616_5y_auc_yearly_all_k10_core_v1"
SOURCE_MODEL_ID = "5Y_AUC_YEARLY_cons_ALL_k_3_DIV_K_10"
TARGET_TENOR = "5Y"
...
_SCREENING_CONFIG: dict[str, Any] = {
    "screen_metric": SCREEN_METRIC,
    "screen_rebal": SCREEN_REBAL,
}
if SCREEN_REBAL == "yearly":
    _SCREENING_CONFIG["wf_ic"] = {"rebal": "yearly", "cap": 2000}
...
def model_config(baseline: str = "STD", **overrides) -> dict[str, Any]:
    """返回 5Y ALL_K10 某个 baseline 的固定参数副本。"""
    if baseline not in BASELINE_CONFIGS:
        raise KeyError(f"unknown 5Y ALL_K10 baseline: {baseline}")
...
def _wf_screen_first_row(
    *,
    wf_rebal: str,
    period: pd.Period,
    daily_dates: pd.Series,
    period_test_rows: np.ndarray,
    yearly_eligible_rows: np.ndarray,
) -> int:
    """返回当前重筛周期的 cutoff 锚点行。"""
    rows = np.asarray(period_test_rows, dtype=np.int64)
    if rows.size == 0:
        raise ValueError("period_test_rows must not be empty")
    if wf_rebal != "yearly":
        return int(rows[0])

    eligible_rows = np.asarray(yearly_eligible_rows, dtype=np.int64)
    dates = pd.DatetimeIndex(pd.to_datetime(daily_dates, errors="coerce"))
    eligible_dates = dates.take(eligible_rows)
    in_period = (
        (eligible_dates >= period.start_time)
        & (eligible_dates <= period.end_time)
    )
    period_eligible_rows = eligible_rows[np.asarray(in_period)]
    if len(period_eligible_rows) == 0:
        raise ValueError(f"daily dates contain no eligible rows for yearly period {period}")
    # Cache-context invariant：cold full-year 与 singleton extend 使用同一年度
    # 锚点；只稳定增量上下文，不改变 source cold-run 的年首锚点。
    return int(period_eligible_rows[0])


def feat_screen(feat_vals, labels, n_self, mf_feat_cat, mf_col_names,
                top_self=50, top_mf=30, min_cats=6, *, metric="auc"):
    """保留源 AUC rank-sum 与类别配额筛选逻辑。"""
    ...
    if metric != "auc":
        raise ValueError("AUC yearly delivery requires auc screening")
...
def run_config(config: dict) -> dict:
    """有界线程内按原 grid、seed 和校准阈值计算各站位日概率。"""
    try:
        ...
        raise RuntimeError(f"5Y AUC yearly LightGBM config failed: {config!r}") from exc
...
def run_prediction(cfg: dict) -> dict[str, Any]:
    ...
    print("  AUC yearly screening on source pre-test data (strict gap)...")
    ...
    selected_feats, n_cats = feat_screen(
        feat_all[pre_idx], labels[pre_idx], n_self, mf_feat_cat, mf_cols,
        top_self=ic_top_self, top_mf=ic_top_mf, min_cats=6, metric=SCREEN_METRIC)
    ...
    yearly_eligible_mask = ~np.isnan(close)
    if require_labels:
        yearly_eligible_mask = yearly_eligible_mask & ~np.isnan(labels)
    yearly_eligible_rows = np.flatnonzero(yearly_eligible_mask)
    test_mask = test_mask & yearly_eligible_mask
    ...
    # -- Walk-forward feature screening (V39) --
    wf_ic = cfg.get("wf_ic")
    ...
    if wf_ic:
        ...
        for p_i, period in enumerate(_wf_unique):
            ...
            first_row = _wf_screen_first_row(
                wf_rebal=wf_rebal,
                period=period,
                daily_dates=df["date"],
                period_test_rows=test_idx[pi],
                yearly_eligible_rows=yearly_eligible_rows,
            )
...
def run_5y_all_for_feature_window(
    *,
    ...
    """运行 5Y_AUC_YEARLY_ALL_K10 PIT 窗口，返回 current window 内逐 feature_date 明细。"""
    ...
    for baseline in required_baselines():
        ...
        if not isinstance(ctx, dict) or len(ctx.get("test_dates", [])) == 0:
            raise RuntimeError(f"5Y_AUC_YEARLY_ALL_K10 baseline {baseline} produced no context rows")
...
def _train_phase_a(
    *,
    ...
    """为指定期限族计算缺失的完整 grid 派生数组。"""
    with redirect_stdout(sys.stderr):
        result = run_prediction(model_config(
            baseline, daily_df=daily_df, weekly_df=weekly_df,
            monthly_df=monthly_df, date_to_week=date_to_week,
            test_start=test_range[0], test_end=test_range[1],
            test_ranges=(test_range,), require_labels=False,
            phase_a_only=True, n_workers=n_workers,
...
def _trial_identity() -> dict[str, Any]:
    ...
    return {"schema": "5y-auc-yearly-all-k10-private-1", "code": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
...
def generate_with_private_state(
    ...
    """逐cutoff推进独立5Y/7Y Phase A；全OOS排名与控制器每次重算。"""
    ...
    if args.state_input is not None:
        with args.state_input.open("rb") as source:
            _inspect_state_archive(source, len(daily), len(grids["five_y"]))
            source.seek(0)
        ...
        for family, state in states.items():
            ...
            if (dates.ndim != 1 or dates.dtype != np.dtype("U10") or not len(dates)
                    ...
                    or (family == "five_y" and dates[-1] != header["cutoff"])
    ...
    for (day, week, month), batch in sorted(by_cutoff.items()):
        ...
        for family, baselines in PHASE_FAMILIES.items():
            ...
            if not len(new_dates) or (family == "five_y" and new_dates[-1] != day):
            ...
            # 已缓存日期必须是当前完整OOS前缀；新年新日期没有旧数组可复用。
            # 旧年selector依赖严格早于年首，源前缀和全特征摘要检查已覆盖该历史域。
        ...
        with redirect_stdout(sys.stderr):
            detail = run_5y_all_for_feature_window(**raw, test_ranges=(("2024-01-01", day),),
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="liwei_0616 5Y_AUC_YEARLY_ALL_K10 Blackbox V2 successor"
    )
