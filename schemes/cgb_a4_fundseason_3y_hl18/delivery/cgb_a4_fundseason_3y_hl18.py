#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""cgb_a4_fundseason_3y_hl18 —— 3Y 国债月频方向算法方案 (Blackbox V2 / Contract 1.0)。

单文件交付物。两个命令复用完全相同的数据处理、算法逻辑与方向映射:

    python cgb_a4_fundseason_3y_hl18.py predict  --request  request.json  \
        --data-dir <data-dir> --output prediction.json
    python cgb_a4_fundseason_3y_hl18.py backtest --requests requests.csv  \
...
5. 取 3Y 行的概率 p: p>=0.5 -> predicted_direction=+1 (收益率上行), 否则 -1。
   二分类, 不输出 0。
...
SCHEME_ID = "cgb_a4_fundseason_3y_hl18"
