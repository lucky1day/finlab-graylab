#!/usr/bin/env python3
"""Blackbox V2 10Y T+5 adaptive 1Y amplitude-reversal scheme."""
...
SEED = 42
RANDOM_STATE = 42
SCHEME_ID = "ten_y_t5_short_amp_adapt756_v1"
DAILY_FILENAME = "daily_output.csv"
TARGET_COLUMN = "TB0YWI0C"
SIGNAL_COLUMN = "TB1YWI0C"
REQUIRED_COLUMNS = ("date", TARGET_COLUMN, SIGNAL_COLUMN)
HORIZON = 5
AMPLITUDE_WINDOW = 5
...
def load_daily_data(data_dir: Path) -> pd.DataFrame:
    ...
    for column in (TARGET_COLUMN, SIGNAL_COLUMN):
...
def build_raw_signal(daily: pd.DataFrame) -> np.ndarray:
    close = daily[SIGNAL_COLUMN]
    returns = close.pct_change(fill_method=None)
    movement = returns.rolling(
        AMPLITUDE_WINDOW, min_periods=AMPLITUDE_WINDOW
    ).sum()
    volatility = (
        returns.rolling(
            VOLATILITY_WINDOW, min_periods=VOLATILITY_WINDOW
        ).std()
        * math.sqrt(AMPLITUDE_WINDOW)
    )
    amplitude = movement / volatility.replace(0, np.nan)
    raw = np.where(
        amplitude > 1.5,
        -1,
        np.where(amplitude < -1.5, 1, 0),
    )
    return (
        pd.Series(raw, index=daily.index, dtype=float)
        .replace(0, np.nan)
        .ffill()
        .fillna(-1)
        .to_numpy(dtype="int8")
