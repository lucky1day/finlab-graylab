#!/usr/bin/env python3
"""7Y01 SAY 共识的自包含 Blackbox V2 草稿。
...
def model_config(baseline: str = "STD", **overrides) -> dict[str, Any]:
    """返回 7Y_01 某个 baseline 的固定参数副本。"""
    if baseline not in BASELINE_CONFIGS:
        raise KeyError(f"unknown 7Y_01 baseline: {baseline}")
...
def run_config(config: dict) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"7Y01 LightGBM config failed: {config!r}") from exc
...
def run_7y01_for_feature_window(
    *,
    ...
    for baseline in required_baselines():
        ...
        if not isinstance(ctx, dict) or len(ctx.get("test_dates", [])) == 0:
            raise RuntimeError(f"7Y_01 baseline {baseline} produced no context rows")
...
def _run_month(
    *,
    ...
    def run_context(caches: Mapping[str, Any]) -> pd.DataFrame:
        with redirect_stdout(sys.stderr):
            return run_7y01_for_feature_window(
                daily_df=daily_df,
                weekly_df=weekly_df,
                monthly_df=monthly_df,
                date_to_week=date_to_week,
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="liwei_0616 7Y01 Blackbox V2 successor"
    )
