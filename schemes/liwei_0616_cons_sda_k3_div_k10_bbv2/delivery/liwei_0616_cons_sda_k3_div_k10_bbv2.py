#!/usr/bin/env python3
"""liwei_0616 5Y01 SDA 共识方案的自包含 Blackbox V2 successor。
...
BASELINE_CONFIGS: dict[str, dict[str, Any]] = {
    "STD": {
        **_SHARED_LGBM,
        ...
        "seeds": [42, 314],
        "K": 10,
        "combo_name": "policy3_term_prem1_self_anti1",
    ...
    "DIV": {
        **_SHARED_LGBM,
        ...
        "seeds": [42, 314],
        "K": 5,
        "combo_name": "policy2_term_prem1_cross_anti2",
...
PROD_CONFIG: dict[str, Any] = {
    "model_no": 1,
    "name": "cons(SDA,k=3)+DIV_K=10",
    "baselines": ["STD", "DIV", "ACCWT"],
    "k_agree": 3,
    "fallback": "DIV",
    "streak_K": 10,
}
...
def model_config(baseline: str = "STD", **overrides) -> dict[str, Any]:
    """返回 5Y_01 某个 baseline 的固定参数副本。"""
    if baseline not in BASELINE_CONFIGS:
        raise KeyError(f"unknown 5Y_01 baseline: {baseline}")
...
def run_config(config: dict) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"5Y01 LightGBM config failed: {config!r}") from exc
...
def run_5y01_for_feature_window(
    *,
    ...
    for baseline in PROD_CONFIG["baselines"]:
        ...
        if not isinstance(detail, pd.DataFrame) or detail.empty:
            raise RuntimeError(f"5Y_01 baseline {baseline} produced no detail rows")
        baseline_frames[str(baseline)] = detail.copy()

    ref_name = str(PROD_CONFIG["baselines"][0])
    ref = baseline_frames[ref_name].copy()
    ref_dates = ref["anchor_date"].astype(str).tolist()
    n_rows = len(ref_dates)
    signs: dict[str, np.ndarray] = {}
    baseline_scores: dict[str, np.ndarray] = {}
    for baseline, frame in baseline_frames.items():
        by_date = frame.set_index(frame["anchor_date"].astype(str))
        missing = [day for day in ref_dates if day not in by_date.index]
        if missing:
            raise RuntimeError(f"5Y_01 baseline {baseline} missing dates: {missing[:3]}")
        scores = by_date.loc[ref_dates, "vote_score"].to_numpy(dtype=float)
        baseline_scores[baseline] = scores
        signs[baseline] = np.sign(scores).astype(np.int32)
    ...
    current_mask = (
        ...
        if include_full_context
            ...
            (ref["anchor_date"].astype(str) >= current_start)
            & (ref["anchor_date"].astype(str) <= current_end)
        )
    )
    result = ref.loc[current_mask, ["anchor_date", "true_label"]].copy()
    selected_indices = np.flatnonzero(np.asarray(current_mask))
...
def _two_stage_phase_a(
    *,
    ...
    if usable_end < 5:
        raise RuntimeError("prior Phase A window is too short for exact ranking")
    scores = np.asarray([
        float(
            (
    ...
    standard_top = np.argsort(-scores)[:10]
    diverse_top = _diverse_topk(scores, windows, 5)
    selected_indices = sorted(
        set(standard_top.tolist()) | set(diverse_top.tolist())
    )
    selected_grid = [
...
def _run_month(
    *,
    ...
    monthly_df: pd.DataFrame,
    date_to_week: Mapping[str, int | str],
    week_end_by_id: Mapping[int, str],
    requested_dates: tuple[str, ...],
    current_start: str,
    current_end: str,
    n_workers: int = 4,
    ...
    prepared_inputs = prepare_prediction_inputs(model_config(
        ...
        monthly_df=monthly_df,
        date_to_week=date_to_week,
        week_end_by_id=week_end_by_id,
    ))
    predict_feat_overrides = _weekly_prediction_feature_overrides(
        prepared_inputs=prepared_inputs,
        weekly_df=weekly_df,
        date_to_week=date_to_week,
        requested_dates=requested_dates,
    )
    base_phase_a, override_phase_a, test_ranges = _two_stage_phase_a(
        daily_df=daily_df,
        weekly_df=weekly_df,
        monthly_df=monthly_df,
        date_to_week=date_to_week,
    ...
    def run_context(phase_a: Mapping[str, Any]) -> pd.DataFrame:
        caches = {name: phase_a for name in PROD_CONFIG["baselines"]}
        with redirect_stdout(sys.stderr):
            return run_5y01_for_feature_window(
                daily_df=daily_df,
                weekly_df=weekly_df,
                monthly_df=monthly_df,
                date_to_week=date_to_week,
    ...
    base_detail = run_context(base_phase_a)
    override_detail = run_context(override_phase_a)
...
def generate_results(
    ...
    week_end_by_id = {
        int(week_id): str(group["rdate"].max().date())
        for week_id, group in calendar.groupby("week_id", sort=False)
    }
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="liwei_0616 5Y01 Blackbox V2 successor"
    )
