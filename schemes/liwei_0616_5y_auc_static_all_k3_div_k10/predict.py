...
def run(predict_date: str) -> list[PredictionRecord]:
    """执行 liwei_0616 5Y AUC static ALL_K10 日频 T+5 实盘预测。"""
...
def _record_extra(
    *,
    ...
    return {
        ...
        "source_model_version": _clean_optional(result.get("source_model_version")),
        "screen_metric": SCREEN_METRIC,
        "screen_rebal": SCREEN_REBAL,
        "vote_baselines": list(PROD_CONFIG["baselines"]),
        "fallback": str(PROD_CONFIG["fallback"]),
        ...
        "phase_a_cache_family": cache_audit.get("cache_family", CACHE_FAMILY),
        "phase_a_cache_root": cache_audit.get("cache_root"),
