#!/usr/bin/env python3
"""5Y AUC static ALL_K10 full-OOS 的自包含 Blackbox V2 私有状态草稿。
...
MODEL_VERSION = "liwei_0616_5y_auc_static_all_k10_core_v1"
SOURCE_MODEL_ID = "5Y_AUC_STATIC_cons_ALL_k_3_DIV_K_10"
TARGET_TENOR = "5Y"
...
def feat_screen(feat_vals, labels, n_self, mf_feat_cat, mf_col_names,
    ...
    if metric != "auc":
        raise ValueError("AUC static delivery requires auc screening")
...
def run_config(config: dict) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"5Y AUC static LightGBM config failed: {config!r}") from exc
...
def run_prediction(cfg: dict) -> dict[str, Any]:
    ...
    print("  AUC static screening on source pre-test data (strict gap)...")
...
def run_5y_all_for_feature_window(
    *,
    ...
    """运行 5Y_AUC_STATIC_ALL_K10 PIT 窗口，返回 current window 内逐 feature_date 明细。"""
    ...
    for baseline in required_baselines():
        ...
        if not isinstance(ctx, dict) or len(ctx.get("test_dates", [])) == 0:
            raise RuntimeError(f"5Y_AUC_STATIC_ALL_K10 baseline {baseline} produced no context rows")
...
def _trial_identity() -> dict[str, Any]:
    ...
    return {"schema": "5y-auc-static-all-k10-private-1", "code": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="liwei_0616 5Y_AUC_STATIC_ALL_K10 Blackbox V2 successor"
    )
