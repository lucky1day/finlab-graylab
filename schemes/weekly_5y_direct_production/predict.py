...
from .core.predictors import REQUIRED_WEEKLY_COLUMNS, predict_w5y
...
class Weekly5YPredictionContext:
    predict_date: str
    feature_week_id: int
    feature_date: str
    target_week_id: int
...
def resolve_weekly_prediction_context(
...
) -> Weekly5YPredictionContext:
    """按源表 week_id 生成周度 5Y 预测上下文。"""
    ...
    return Weekly5YPredictionContext(
...
def validate_weekly_data_coverage(weekly_df: pd.DataFrame, context: Weekly5YPredictionContext) -> None:
    """确认 DB 周频输入已经覆盖本次 5Y 预测所需特征周。"""
    if weekly_df.empty or "week_id" not in weekly_df.columns:
        raise ValueError("weekly source data is empty; cannot run weekly_5y_direct_production")
    weekly = weekly_df.copy()
    weekly["_week_id"] = pd.to_numeric(weekly["week_id"], errors="coerce").astype("Int64")
    weekly = weekly.dropna(subset=["_week_id"]).copy()
    if weekly.empty:
        raise ValueError("weekly source data has no valid week_id; cannot run weekly_5y_direct_production")
    weekly["_week_id"] = weekly["_week_id"].astype(int)
...
def run(predict_date: str) -> list[PredictionRecord]:
    """执行周度 5Y direct-production 方案预测；只读源数据，不写库。"""
    run_date = pd.Timestamp(predict_date).normalize()
    feature_date = (run_date - pd.Timedelta(days=1)).strftime("%Y-%m-%d")
    ...
    result = predict_w5y(weekly_df, context.predict_date, target_week_id=context.feature_week_id)
    extra = {
        "frequency": "weekly",
