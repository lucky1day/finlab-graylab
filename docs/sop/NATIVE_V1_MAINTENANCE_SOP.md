...
本 SOP 只保障九套 W4 已有版本的输入、执行依赖和现有日/周/月调度持续可用，不提供独立历史回测重跑；已有历史结果与来源原件保留。后续算法、方案和版本修订全部走 [Blackbox V2 平台 SOP](BLACKBOX_V2_PLATFORM_ONBOARDING_V1.md)；不再使用 Native `onboard/all/native-maintenance` 或重新激活建立新版本。已迁移 Blackbox 的方案不维护 Native Harness。该决定不自动迁移 W4，也不授权删除原件、历史数据或生产资源。
...
受影响 W4 仍保持原版本、身份、输入和执行依赖；本次授权修复通过对应运行检查，且未改变其他方案或已发布事实。自然运行完成须有相应真实时钟的日志、run、prediction 与 Dashboard 一致性；只完成环境修复、模拟或补缺时分别报告，不能推定自然运行通过。当前状态仅保留结论、剩余问题和必要定位信息。
