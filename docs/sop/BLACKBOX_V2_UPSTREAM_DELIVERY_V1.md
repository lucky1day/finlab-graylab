...
- 只读取平台传入的 `--request/--requests` 和只读 `--data-dir`；显式启用增量状态的方案另可读取第 6.2 节的受控状态。
...
- 默认不得保留跨调用状态；只有第 6.2 节显式启用的派生状态例外。同一 Request 的结果不得因执行顺序改变。
...
仅在完整冷/热等价与性能验收证明需要后，由平台 Intake 显式声明 `incremental_state: true`。Metadata 1.0、两文件
交付和 Result 不增加字段。启用后两个 CLI 接受 `--state-output <fresh-path>`，恢复计算时另接收
`--state-input <read-only-path>`。没有 state-input 的私有回测或显式重建必须从本次原始输入计算，不寻找旁路缓存。

- 状态仅保存可由源输入重建的本方案派生计算，不包含模型对象、额外代码、数据库事实或跨方案缓存。
- 算法负责内部格式/维度、历史依赖、修订拒绝、尾部重算和 cutoff 检查；不能用未来状态服务历史 Request。
- 状态复用必须与同输入冷路径完全等价。输入中不相关字段或尚未消费区间变化可以复用，但遗漏真实历史依赖属于算法错误。
- 必须将完整新状态写到平台提供的 state-output，最大 16 MiB；不得访问或覆盖 canonical 状态目录。
- 算法失败时不留下 Result 或新状态；历史修订/损坏/无法证明复用必须非零退出，不自动全量训练或回退旧结果。
...
|---|---:|
| 单条 `predict` | 120 秒 |
| 显式首次预热／故障重建 | 离线维护上限 7200 秒，不适用于日常增量 predict |
| 离线 `backtest`（100 条及完整正式区间） | 每次调用 7200 秒安全上限 |
| 单个算法子进程峰值 RSS | 4 GiB |

离线耗时单独记录，不再以 100 条／600 秒或完整区间／1800 秒拒绝候选。7200 秒是防止失控运行的
安全预算，不是每日预测 SLA，也不代表正常回测应当耗时两小时。调用方设置的更短 deadline 仍然生效；
不为离线计时优化增加模型缓存或更改算法语义。结果、截止隔离、资源及安全验收不放宽。
该表是本次验收调用的预算要求，不把现有 stateless Runtime Profile 的通用默认值改成 7200；
执行离线验收时须显式传入该预算。stateful backtest 执行器另有不超过 7200 秒的保护上限。
