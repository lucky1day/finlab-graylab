...
以下事项属于已知控制边界，不能在报告中扩大解释：

- Input Gate 不独立证明当天 freshness，也不记录 generation ID；
- JSON Result 解析器当前接受字符串方向，与“JSON 必须为整数”的上游规范存在待整改差异；
- JSON runtime profile 目前是发布基准，不是执行器唯一加载来源；
- Harness 控制面持久化是 best-effort，shadow 前必须确认审计 DB 已保存 exact run；
- `business_tables_written: false` 是 Gate evidence，不是数据库前后计数；
- `api-readiness` 不执行真实 HTTP API、Registry 或 scheduler 探针；
- shadow 配置和数据库更新不是单事务，失败后需要 reconciliation；
- Contract 1.0 禁止 activate/live 目前是操作政策，不是专用代码 hard-stop；
- sandbox 重点限制网络和写路径，不等价于全局文件读取 allowlist；
- 临时 Snapshot、原始 Result 和 stderr 不作为永久回放资产。

因此，“七个 Gate 通过”只表示技术契约验收通过，不等于 active、正式生产、已调度、API 可见或算法效果达标。
