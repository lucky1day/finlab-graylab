...
- [x] 在测试中将预期脚本 SHA 和 scheme version 更新为新交付身份。
- [x] 先运行测试确认旧脚本导致红灯。
- [x] 仅用上游新 `.py` 替换仓库交付脚本。
- [x] 运行 identity、discovery、Contract 测试并确认绿灯。
...
- [x] 编写红灯测试：正常切换、旧版本非唯一 active、新版本身份漂移、
  Registry 非 active、dry-run 零写入。
- [x] 在 `scheduler.repository` 实现单事务 exact-version 切换：
  new active、old retired、Registry active、读回唯一 active。
- [x] 运维脚本必须显式传入 scheme、old version、new version、harness run、
  `--apply`；默认 dry-run。
- [x] 运行目标测试和 repository/lifecycle 回归。
...
- [x] 在平台同一代 DataBridge 和 canonical calendar 下运行新旧历史 72 周、
  灰度 9 周对比。
- [x] 运行 `harness onboard ... --stage all --check-only`，确认 7/7 PASS。
- [x] 记录 exact generation、combined snapshot、calendar hash、harness run 和
  新 scheme version。
- [x] 任何方向、日期或 cutoff 不一致时停止生产切换。
...
- [x] 对版本替换脚本执行 dry-run，核对旧 active/new candidate/Registry。
- [x] 用 exact all-stage run 和 operator 身份执行 `--apply`。
- [x] 持久化 `2025-01-01` 起、`target_date < 2026-06-01` 的新历史回测。
- [x] 核对新 run 为 canonical latest、72 条预测、月度指标非空。
- [x] 调用 `scripts/delete_backtest_runs.py` dry-run 后精确删除旧 run `192`。
...
- [x] 红灯测试覆盖 exact old version、9 个目标、唯一键、事务回滚和 dry-run。
- [x] 复用 Blackbox Request、当前 input bundle 和执行器在事务外计算 9 条结果。
- [x] repository 在每个目标的事务中替换 prediction/run/log，并校验写后版本。
- [x] 执行 dry-run 后 `--apply`，确认 9 条全部归属新 version。
- [x] 核对旧 version 在 prediction/run/log 中计数为零。
...
- [x] 核对 `/api/schemes`、`/api/metrics/{registry_id}`、
  `/api/backtests/factor-lab` 和前端验证脚本。
- [x] 核对 scheduler 未加载该方案。
- [x] 运行目标测试、相关回归、compileall、`git diff --check`。
