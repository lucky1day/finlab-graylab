...
Document the mutually exclusive full-`all` and maintenance profiles. For maintenance, document prior `all+compare=passed`, matching prior `static.business_identity`, current exact `t_scheme_versions` as native `draft|active`, the same expected Registry identity uniformly paused pre-activation or active post-activation, exact current version/run, persisted evidence, and fail-closed behavior; a draft candidate with active Registry must fail. Explicitly say a legacy snapshot cannot be retroactively inferred from current Registry/config; only Task 5’s fixed, canonical receipt may supplement one explicitly missing identity field, and it is not a generic Compare waiver. Do not alter Blackbox instructions.

- [x] **Step 3: Update G4 to the single authorized record.**

Remove the obsolete benchmark waiver/rebuild decision from current status documents. Record that 14/45 historical differences and three direction changes are archived diagnostics, that the legacy prior `static.business_identity` snapshot is absent, and that the G4 current exact candidate is `t_scheme_versions=native_adapter/draft` with uniformly paused expected Registry—a normal pre-activation state, not an extra blocker. The business write remains unready: Task 5’s fixed 10Y receipt was executed on 2026-08-04, but six maintenance Gates and normal activation must still pass. The receipt is a narrow control-plane identity proof, not an alternative business-write path:
...
### Task 4: Historical full-`all` fallback record (superseded for G4 by Task 5)
...
registry_scheme_id=weekly_10y_d_overlay_0529__h6__10Y
```

The command re-reads the maintenance-selected active prior version/run rather than hard-coding
them. Its current business identity is frozen to the exact scope above (including the composite
Registry ID) and must match before token consumption; it accepts exactly one passed CompareGate
and one passed StaticGate whose `native_business_identity` evidence is explicitly absent. A
malformed, duplicate or mismatched old snapshot is not eligible for attestation.

- [x] **Step 2: Write RED tests for the receipt and its maintenance fallback.**
...
- [x] **Step 3: Implement the smallest dedicated command and persistence path.**
...
no current code/config/version hash and does not overwrite historical results. An existing
receipt blocks a second write; it is never reported as a successful idempotent command. The
command validates expiry and replay before any write, then consumes its token immediately before
the one receipt transaction. A receipt persistence failure after consumption is a failed result
that requires a new token; it remains safe because no business table was touched.
...
- [x] **Step 4: Verify and execute the authorized control-plane receipt.**

Focused tests and code review passed. On 2026-08-04, a ≤900-second scoped token bound to prior
version `63ffb52105ee` and run `hr_20260611T055610Z_8742d5bc99c9` wrote the sole receipt run
`lna_hr_20260611T055610Z_8742d5bc99c9`. Readback confirmed that only `t_harness_runs` and
`t_harness_gate_results` changed and the maintenance verifier reports
`admission_identity_source=legacy_operator_attestation_v1`. No activation, launchd action,
backend restart, prediction write, or signal-gap fill occurred.

- [x] **Step 5: Update current policy documents and remove closed implementation documents.**

Replace every obsolete pre-implementation statement in current architecture/SOP/status documents
