...
6. 经一次性授权最多登记为 `shadow + paused`；当前禁止 activate/live。
...
没有这些证据时不得标记技术入库完成。Blackbox 即使全部自动 Gate 通过，也只能按当前政策登记 `shadow + paused`，不得宣称 active、live 或 Production Observed。
