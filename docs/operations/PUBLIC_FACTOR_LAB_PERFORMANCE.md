...
- 已有成功快照后失败：保留完整已提交统计视图、区间标题和已开月历，显示“刷新失败，显示 `<generated_at>` 数据”，
...
- 日期输入保留用户当前选择，不因加载、失败或旧响应退回已提交日期。应用中的同一区间不重复请求，按钮显示“应用中…”；失败后显示“重试区间”，手动及自动重试均继续最后提交的区间。用户应用另一区间后只接受最新请求；尚未点击应用的输入草稿不随在途请求成功而丢失，也不参与自动重试。
