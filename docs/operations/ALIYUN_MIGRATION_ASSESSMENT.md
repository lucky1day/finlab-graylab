...
dependency declaration 只接受两种精确形式；没有字符串 trim、数值转换或其它 normalization：

- legacy 形式必须是 literal `daily_dependency_lookback_rows is None` 且
  `daily_dependency_proof is None`；它只用于精确识别旧 fingerprint，不具备 revision suffix 资格；
- proof-bearing 形式要求 rows 是非 bool 的 `int`，且精确等于 `max(horizon, purge_gap)`，proof 也必须
  精确等于 canonical `liwei_0616_daily_revision_suffix_v1`。

任何其它 partial、空字符串、带空白 proof、bool/float、偏小或偏大的 rows 都无效：不能申请 suffix，不能
匹配 legacy fingerprint；publisher 按 full/fail-closed 处理，secure consumer 保持 fail-closed。兼容性
复用因此只接受上述 canonical proof-bearing fingerprint，或上述 literal `None`/`None` 且其余身份完全相同的
...
2. legacy `None`/`None` 的 revision 仍为 full；只有 canonical proof-bearing spec 才能按
   `max(horizon, purge_gap)` 计算 suffix 边界。
...
只补 CompareGate 证据的 `qualification` generation 现在也可被 secure consumer 严格 replay，但必须同时满足：
输入及 daily/weekly/monthly frame 均为 unchanged；父子每个 baseline 的 cache 精确相同；每个 baseline 的
`affected_dates=[]` 且 `preserved_dates` 完整等于 parent `test_dates`；parent 尚未 qualified（只允许
`unqualified` 或 `phase_a_compared`）；candidate 具有完整、绑定当前输入与 cache 内容且校验通过的
`qualified` CompareGate evidence。任一条件不成立都不会识别为 qualification，也不放宽 full、append、
suffix、rebind 或其它 lineage mode。

generation publication 顺序固定为：candidate 完整落盘并 finalize（包括 move 后 generation-root fsync；该步
失败会清理已移动 candidate）→只读计算 prune plan→以 `current.json` atomic replace 作为唯一 commit point
→提交后 best-effort prune。prune 始终保护 candidate 与 old current。任一 commit 前失败都保持旧 pointer 和
既有 generations 不变并清理未发布 candidate；commit 后普通 `Exception` 清理失败不会把已成功 publication
报告为失败。`KeyboardInterrupt`、`SystemExit` 等 `BaseException` 仍向上传播，但已提交 candidate 保留，
不得因错误回滚路径删除 current 指向的 generation。
...
结果为 `80 passed, 1 warning, 41 subtests passed in 1.72s`。全量 command（`exit 0`）为：

```bash
PYTHONNOUSERSITE=1 conda run -n bond_factor_lab_service python -m pytest -q
```

结果为 `923 passed, 44 warnings, 476 subtests passed in 54.28s`。warning 仍包括 `backend/main.py` 的既有
`SyntaxWarning` 及 SQLite datetime/timestamp adapter 的 deprecation warnings。

canonical proof、qualification lineage replay 与 publication/prune 事务分别在独立实现任务中完成，并且
Task 6A、6B、6C 均通过独立 spec review 与 code-quality review；上述 focused 与全量结果是在三项修复全部
进入当前代码 HEAD 后重新取得的本地 fresh evidence。
