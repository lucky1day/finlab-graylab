...
- [事实] `harness/daily_real_replay.py`、`harness/daily_real_replay_operator.py`、`harness/daily_real_replay_mysql.py` 仍构成隔离 replay 闭包：临时 mysqld 以私有 datadir、loopback 和 `--no-defaults` 启动；完整 operator 被调用时还会读取受控的 production/source 配置、对生产控制面执行只读一致性快照和 source readiness 预检，并只向已验证的隔离 Engine 应用全量 migration（包含 019）。这不把 replay 描述为生产 scheduler，也不表示完整 operator 从不读取生产或 source 配置。
...
|---|---|---|---|---|---|
| 隔离 real replay：`harness/daily_real_replay.py`、`daily_real_replay_operator.py`、`daily_real_replay_mysql.py` | replay 创建带专用 namespace 的 occurrence，只接受已验证的隔离 Engine 和冻结 generation。临时 mysqld 的 `--no-defaults`、私有 datadir 与 loopback 只约束该临时服务；完整 operator 预检还会读取受控 production/source 配置、读取生产控制面的一致性只读快照和 source input/readiness evidence。执行路径以 `apply_migration_files(engine, sorted(MIGRATIONS_DIR.glob("*.sql")))` 向隔离 Engine 应用 migration，因此包含 019；它不会在本代码路径向 production Engine 应用 migration。它仍直接依赖 daily policy、ledger repository API、coordinator/epoch 和 `daily_control_plane_probe`。 | 它保留了一条严格隔离的“真实联跑/恢复诊断”能力；不是自然 `scheduled_live` writer。operator 的当前预检仍把旧 LaunchAgent labels 与 `legacy` mode 当作边界条件，并把 production/source 的受控只读事实纳入 dispatch identity。 | **isolated recovery**。保留恢复/诊断目的，但把它明确为单独的隔离资产，不是 production control plane。 | 当前事实不能声称 replay 已不依赖 production/source preflight。若未来要把 replay 收敛为不读取 production/source 配置或快照的闭包，这只是未经确认的 G8.1 候选迁移目标：必须先定义替代输入、隔离配置、审计证据和测试，不能在清理时假定已实现。 | 用户确认 replay 的保留目的；任何实际 replay、临时 MySQL 启动、production/source 配置读取、production/source 只读快照或 source DB 预检都需专项操作授权。该授权最多覆盖指定的只读访问和隔离 MySQL 的创建/migration，绝不授权 production 写库、production DDL、手工 SQL、launchd 或 installed plist 变更。 |
...
5. **replay 价值、读取与授权边界**：用户确认是否保留 isolated recovery，以及它允许使用的隔离 MySQL、受控 production/source 配置、只读 production/source snapshot、operator 角色、保留期限和失败时的 stop 条件。任何实际 replay 或 source/production preflight 都必须另取专项操作授权；授权文本须明确只读对象及隔离 MySQL 的 migration 范围，并明确不授权 production 写库、DDL、手工 SQL、launchd 或 installed plist 变更。若未来要求 replay 不再读取 production/source 输入，先批准其替代设计，不能把它当作当前事实。若不保留 replay，才能把其专用闭包列入 repo retire 范围。
...
|---|---|---|---|
| A. repo 清理与隔离闭包 | 先实现已确认的 admission vocabulary、legacy mode replacement、repo-only no-consumer checks。G8.1 必须同时处理 executor 的 no-item/mode 分支和 Phase-A cache 的 CompareGate mode 语义；若保留 replay，不得假定其当前 production/source preflight 已被移除。不得改 DB、installed plist、服务或 launchd。 | 单元/contract tests：launchd one-shot、executor 的 no-item/mode 分支、admission、DataBridge authority、repository no-ledger paths、Phase-A cache CompareGate、replay 的隔离 migration containment 与 preflight identity、architecture boundaries；`git diff --check`。 | 普通 Git revert；不得用“恢复旧 scheduler”作为回滚。 |
...
3. **replay 隔离与输入合约**：保留 replay 时，测试须继续证明 migration（包含 019）只应用至经验证的 isolated Engine，并证明 source/production preflight 的一致性只读属性；若未来批准去除这些读取，须先新增替代输入测试，不能删除现有 preflight 便宣称隔离闭包完成。覆盖 `tests.test_daily_real_replay_operator_migrations`、019/017 migration tests 与 replay MySQL isolation tests。
4. **schema 退役前置条件**：只在上述 code consumer 已迁移、archive parity/readback 通过且 ledger 相关 contract 不再被生产路径调用后，才可提出 forward DDL。schema 改动仍是 C 阶段的独立授权，不能由 A 阶段测试推定获批。
...
- [建议] 实际 replay 的专项授权应把“production/source 的指定只读预检”与“隔离 MySQL 上的 migration”逐项列出；它始终不等同于 production DB 写入或 DDL 授权。
...
1. 是否将 real replay 保留为 **isolated recovery**；若保留，是否继续允许其当前受控 production/source 配置与只读 snapshot/preflight 输入、允许的用途和保留期限是什么。若目标是不再读取这些输入，需先批准替代模型与迁移验收，不能视为已经实现。
