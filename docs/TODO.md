...
3. full-OOS 首次显式预热于 2026-09-08 14:52 达到 1800 秒硬限失败，未发布状态，跟进已暂停。
   用户现已批准独立冷重建维护预算最多 7200 秒（日常仍120秒）并再执行一次受控初始化；先通过预算隔离
   测试和独立审查，核对与定时任务之间的保护窗口后启动。已有算法/正式回测不重跑，失败不自动重试；
   cons-sda 保持 stateless，不增加预热。
   已于15:32使用私有immutable `8eb2df2` 启动这唯一一次初始化；跟进完成记录、完整状态/身份和现场不变后
   再推进后续切换，目前不能重复启动。新日志目录与精确archive身份见迁移计划顶部。
   不复制正式回测私有状态、不写业务事实。ECS release 严格完整性、部署矩阵、增量预热和证据均就绪后，推进 W3A
