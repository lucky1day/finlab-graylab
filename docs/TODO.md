...
1. 跟进 cons-sda 已启动的一次独立 Native 参考：只读复用 run279 的 333 条正式结果、不再执行 Blackbox，
   补齐前轮已清理的原始等价证据。完成后按同代输入逐行复验并保留原件，失败不自动重试。
   W3A 两个正式回测及 full-OOS 已完成原始等价均不重跑，不恢复重复/倒序/乱序/子集矩阵。
