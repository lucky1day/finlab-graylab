...
5. 页面右上角展示用户名。点击后只显示：
   - 修改密码
   - 退出登录
6. 管理员拥有独立的“用户管理”板块，可以创建用户、调整角色、重置密码、停用用户和恢复用户。
...
| 路由 | 页面 | 访问条件 |
|---|---|---|
| `/bond-factor-lab/` | 登录、强制改密、因子实验室或用户管理视图 | 根据当前会话与角色决定 |
...
菜单严格只包含“修改密码”和“退出登录”。管理员的“用户管理”是应用主导航，不放入账户菜单。菜单必须支持：
...
修改成功后撤销该用户的全部会话，包括发起改密的当前浏览器会话，并返回登录页。初次登录或管理员重置密码后的
强制修改流程不允许关闭或进入其它页面；完成修改后仍须使用新密码重新登录，才能访问 Dashboard。
...
| 状态 | 正常或已停用 |
| 创建时间 | 账户创建时间 |
| 操作 | 修改用户名、编辑角色、重置密码、停用或恢复 |

新增用户对话框只包含用户名、初始密码和角色。新用户第一次登录必须修改初始密码。
...
| `must_change_password` | `BOOLEAN` | 首次登录/重置后强制改密 |
...
4. 初始密码从部署机权限受限的私有 secret 读取，必须符合统一密码规则，且不出现在仓库、命令行、shell history
   或日志；
5. 创建首个 `admin`，设置 `is_protected_admin = true`，并要求首次登录修改密码；
...
3. 从部署机权限受限的私有 secret 读取符合统一密码规则的预先约定初始密码，不把密码写入仓库、命令参数、
   shell history 或日志；
4. 使用 Argon2id 重新生成密码哈希，并设置 `must_change_password = true`；
5. 在同一事务中撤销该管理员的全部现有会话并写入审计日志；
6. 重置后必须用初始密码重新登录并立即修改密码。
...
4. 页面右上角只显示用户名；点击后只显示“修改密码”和“退出登录”。
5. 普通用户看不到用户管理入口，直接调用管理员 API 返回 `403`。
6. 管理员可以创建普通用户和管理员，新用户首次登录必须修改密码。
7. 管理员可以修改用户名、调整角色、重置密码、停用和恢复用户。
8. 修改用户名、停用、改角色、重置密码和用户改密后，目标用户在全部浏览器中的会话立即失效。
9. 并发管理操作也不能删除、停用或降级最后一个有效管理员。
10. 数据库、API 响应、日志和审计记录中不存在明文密码或原始会话令牌。
11. 管理员变更均有不可修改的审计记录。
...
17. 恢复用户不会改变旧会话的过期或撤销状态，任何旧会话都不能因此恢复有效。
18. 管理员重置命令拒绝不符合统一密码规则的初始密码，不从参数、输出、日志或版本控制中暴露密码，并在重置后
    撤销管理员全部会话。
