...
| `harness.blackbox_v2.gates.BlackboxInputGate` | Blackbox 记录 generation 级三频父快照、平台注册制品、组合输入身份和 Request 截止键；缓存命中只核验现有 receipt、manifest 身份和文件类型，不读取或扫描三频 CSV 内容；运行视图从本次 SHA 校验已经读取的字节复核首列 cutoff | 不重复解析、重写或复制同代父快照，不新增第二套缓存协议，不把摘要证据夸大为 generation freshness |
...
`--stage all` 按 `runtime_type` 分派：Blackbox V2 为三段 `static -> input -> compare`；Native V1 为四段 `static -> dry-run -> compare -> backtest-no-persist`。任何一步失败都停止。Native DryRun 从同一真实执行中读取共享 builder 写入私有临时目录的 receipt，并与最终普通文件、配置和 PredictionRecord 交叉核对 source、data version、请求截止、实际覆盖、表头和必需列；不再先按另一套通用窗口构建一次输入。Native 不再通过 scheme_id 文本命中仓库测试：这种选择既覆盖不了多数方案，又会误选平台测试；Static/Dry-run/Compare/Backtest 已形成更精确的正确性闭环。首次 Native 技术入库必须保留 source benchmark/CompareGate 证据；Blackbox Compare 只做平台输入校验和一次有效 predict 冒烟，非法 Request 与失败无 Output 行为由上游交付契约负责。技术 `all` 不访问 Backend；`dashboard`、持久化 backtest 和 `activate` 都不属于 `all`。
...
Blackbox 三段 evidence profile 不读取方案级历史报告，也不把旧四段 `all` 静默折算为新证据。升级前已通过四段流程、但尚需执行 shadow、activate 或持久化 backtest 的 exact version，必须先用新的三段 `all` 快速重跑一次；自然调度和既有预测结果不受影响。
...
3. 首次技术入库使用按 runtime_type 分派的 `all`（Blackbox 三段、Native 四段），其中 Native 的 source benchmark/CompareGate 是必留证据。
