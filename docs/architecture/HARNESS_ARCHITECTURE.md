...
| `harness.gates.compare_gate` | Native 执行 source benchmark；Blackbox 读取 DataBridge producer 已准备的 generation 快照，绑定 Request 并做一次冒烟 predict | 不构建、修复或重新验证 DataBridge，不用调参、改脚本或伪造结果；不重验交付自身的性质 |
...
`--stage all` 按 `runtime_type` 分派：Blackbox V2 为两段 `static -> compare`；Native V1 为四段 `static -> dry-run -> compare -> backtest-no-persist`。任何一步失败都停止。Native DryRun 从同一真实执行中读取共享 builder 写入私有临时目录的 receipt，并与最终普通文件、配置和 PredictionRecord 交叉核对 source、data version、请求截止、实际覆盖、表头和必需列；不再先按另一套通用窗口构建一次输入。Native 不再通过 scheme_id 文本命中仓库测试：这种选择既覆盖不了多数方案，又会误选平台测试；Static/Dry-run/Compare/Backtest 已形成更精确的正确性闭环。首次 Native 技术入库必须保留 source benchmark/CompareGate 证据；Blackbox Compare 只绑定 producer-ready generation、Request 并执行一次有效 predict 冒烟，非法 Request 与失败无 Output 行为由上游交付契约负责。技术 `all` 不访问 Backend；`dashboard`、持久化 backtest 和 `activate` 都不属于 `all`。

Native builder receipt 环境变量只由 DryRun 父进程显式注入，目录为本次运行独占的 `0700` 临时目录，
receipt 为 `0600` 普通文件并在 Gate 返回前删除；自然调度不传该变量，不增加 sidecar 写入或解析。
StaticGate 同时拒绝 Native `predict.py` 的直接文件写入；方案代码只能经平台共享 builder 生成输入。

Blackbox 两段 evidence profile 不读取方案级历史报告，也不把旧流程静默折算为新证据。升级前已有旧 Gate 证据、但尚需执行 shadow、activate 或持久化 backtest 的 exact version，必须先用新的两段 `all` 快速重跑一次；自然调度和既有预测结果不受影响。

Native 四段 evidence profile 同样不把旧五段 `all` 静默折算为新证据。尚未激活的 current exact
version 若只有旧五段记录，须用新四段 `all` 重跑；这只刷新 Harness 控制面证据，不修改预测结果。

`native-maintenance` 仅给已完成首次技术入库、且有可比较 prior snapshot 的同一 Native 业务身份使用，固定三段顺序为 `static -> native-maintenance-admission -> dry-run`。`native-maintenance-admission` 必须只读证明不同的旧 Native active version 已有 passed `all` 和 passed `compare`，并从该 prior `all` 的 `static.business_identity` 读取与当前精确匹配的业务快照：`scheme_id`、`runtime_type`、`horizon`、`task_type`、`frequency`、target tenors 与 composite Registry IDs；不得比较或持久化代码/config/version hash 作为身份字段。current exact `t_scheme_versions` 行必须为 native `draft|active`；expected Registry identity 要么全 paused（预激活），要么全 active（激活后），且 draft version 配 active Registry 必须失败。只有 ActivationGate 才能原子翻转至 active。prior snapshot 缺失、重复、损坏或不匹配时一律阻断。ActivationGate 在这一路径复核 prior 前提与当前精确 version 的三个 Gate，并返回 `native_post_admission_revision_v1`；若当前 exact version 已有 passed `all`，则改走互斥的 `full_initial_onboarding_v1`，不要求 prior snapshot 或三段 Gate。这个 stage 不执行当前 historical `compare/backtest`，只持久化 Harness 审计证据且不写业务表。Blackbox V2 不接受该 stage，仍走既有 `all`。
...
3. DataBridge producer 独立发布并准备 generation 快照；方案只选择现有 ready snapshot 和七字段 Request，不构建、修复或验证 DataBridge。
4. 依次执行 `static -> compare`。
...
3. 首次技术入库使用按 runtime_type 分派的 `all`（Blackbox 两段、Native 四段），其中 Native 的 source benchmark/CompareGate 是必留证据。
...
- 输入 artifact 结论: Blackbox 保留 generation 级父快照身份；Native 在 DryRun 证据中保留实际主/辅助输入的 builder receipt、frequency、普通文件 path、source、data_version、row count、实际覆盖、CSV 列和缺列结论，不再执行第二次 DB 构建或完整 DataFrame 解析。
- 执行结论: Blackbox 保留 Compare 的单次冒烟 PredictionRecord、DataBridge Snapshot identity 和正式表零写入；Native 保留 dry-run 的 JSON 输出、预测条数、关键字段和正式表行数不变。
...
- 算法保真结论: Native 首次技术入库记录 source 口径、L0/L1/L2 分级、原始 hash 和内部 benchmark；当前 exact version 若通过 full `all`，记录 `full_initial_onboarding_v1` 的四个 Gate。仅走 maintenance 时，另记录 prior `all + compare`、其匹配的 `static.business_identity` 业务快照、精确 Registry identity、`native-maintenance` 三个 Gate 与 live-safe oracle。历史 benchmark vintage 漂移只能标为归档诊断。Blackbox 记录上游脚本/Metadata hash 与冒烟 predict 的标准结果，不宣称平台已检查黑盒内部模型；确定性、分批/顺序一致性与未来行隔离由上游按交付契约保证，平台不重验也不据此背书。
