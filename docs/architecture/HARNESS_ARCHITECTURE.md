...
- ActivationGate 的 Native profile 互斥：current exact version 的完整 `all` 通过时使用 `full_initial_onboarding_v1`，只要求当前七个 Gate（含 Compare），不要求 maintenance/prior snapshot；只有 prior `all` 的 `static.business_identity` 已持久化且与当前身份精确匹配的修订才使用 `native-maintenance` 和其六个 Gate。该快照只含业务字段，不含代码/config/version hash。maintenance 的 current exact `t_scheme_versions` 行必须是 `native_adapter` 且 status 为 `draft|active`；expected Registry identity 可在预激活时统一为 `paused`，或在激活后统一为 `active`，但 draft version 配 active Registry 必须 fail-closed。ActivationGate 是唯一原子建立 active 状态的操作。legacy admission 缺快照时仍 fail-closed，除非是固定 `weekly_10y_d_overlay_0529`：专用命令只可为 maintenance 选定的 prior `all + compare=passed`、且 StaticGate 已通过但明确缺 identity 字段写入 canonical receipt。它需要 issuer 与 exact prior version/run 绑定的 ≤900 秒一次性 token，只写 Harness run/gate 表，不能生成、推断、激活或写业务表；它只提供 `legacy_operator_attestation_v1` 身份来源，之后仍须全部六段 Gate。满足已实现路径后，历史 source-benchmark 输入 vintage 漂移才只作归档诊断，不是 activation、历史补数、`gray_live`、`scheduled_live` 或 API 的独立 blocker。
