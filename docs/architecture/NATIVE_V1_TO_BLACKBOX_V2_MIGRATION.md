...
最近20次平均12.895秒。该计数仅表示计算进度，不是54条已通过与Native比较；最终五字段比较尚未发生。
算法峰值RSS1168920KiB、current未变；08:30 Actuals已于08:31:07自然成功。保持本次单一计算，不重复派发。
...
- 04:59只读细化：W3A现有5Y Blackbox也保留相同的“先IC筛选、后判断Phase A cache”顺序，
  不能直接当作已优化模板。Full候选的cache命中分支中，`selected_feats/feat_selected`只用于此前日志，
  训练才消费筛选矩阵；这是待计时验证的窄范围冗余，不需新增跨cutoff缓存框架。
