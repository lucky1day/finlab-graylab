...
- Blackbox 的精确 version、generation、snapshot、持久化回测、激活与授权证据，以及 Native 的 Harness run/Gate 证据，由控制面数据库及本机 ignored reports 保存，不提交到 `docs/`。
...
4. 修改文档时必须保持相对链接和章节锚点可解析；不得引用已删除的脚本或测试。
