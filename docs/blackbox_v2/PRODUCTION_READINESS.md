...
> PR-01 至 PR-08 的代码前置项已在隔离库完成生产路径认证，但真实交付覆盖门槛尚未满足。本文件仍不是生产操作授权；生产 trial 继续保持 `shadow + paused`。
...
- 生产 trial 未获得业务和运维上线授权，仍保持 `shadow + paused`。
...
4. 由业务、平台和运维审核认证证据并明确生产激活、暂停和回退责任人。
5. 将平台生产晋级操作文档标记为 `CURRENT` 后，才允许对具体方案签发生产授权。
