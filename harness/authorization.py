...
def verify_authorization(
    ...
    *,
    ...
    if (
        auth.harness_run_id is not None
        and auth.harness_run_id != harness_run_id
    ):
        errors.append(
            "harness_run_id mismatch: "
            f"operation={auth.harness_run_id}, ctx={harness_run_id}"
        )
    ...
    if not errors and auth.harness_run_id is None:
        auth = replace(auth, harness_run_id=harness_run_id)
    return auth, errors
