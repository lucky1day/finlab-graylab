...
当前阶段：**新模拟生产机器已完成服务环境、正式平台表、actuals、周度 actuals、历史回测、周度 10Y 回测接入、周度 5Y/7Y 回测落库、launchd 常驻验证和 Git baseline 管理**。基础设施、t1/t5/weekly adapter、调度器、FastAPI 后端、前端 API 对接和 launchd 部署均已落地；`t1_daily` / `t5_daily` / `weekly_10y_d_overlay` 均为 `active`。2026-06-05 09:25 常驻 scheduler 已写入 t1/t5 正式预测记录；2026-06-06 已完成周度 10Y 受控 live 写库验收、单方案 scheduler 手动补跑，并注册周六 `11:30`（`30 11 * * 6`）自动调度。2026-06-08 已将用户提供的统一 `data_service (1).py` 接入为 `shared.data_service`: 日/周/月算法输入均先通过 `shared.input_artifacts` 调用这个统一数据层导出 CSV，再读回给算法；公共层只做文件路径、保存、读回和 metadata 记录，不再让周频方案绕到 scheme 内部数据服务生成输入。2026-06-08 已按 SOP 接入 `weekly_5y_direct_production` 和 `weekly_7y_cross_d_overlay`，当前保持 `paused`，标准 dry-run、DB 回测落库和 backend factor-lab 数据函数验证均通过；方案仍未同步 registry、未写入实盘预测表。artifact 命名已统一: 运行期输入位于 `backtest_artifacts/runtime_inputs/{scheme_id}/`，历史回测和数据差异报告位于 `backtest_artifacts/backtests/{benchmark_id}/`，前端展示使用 API 的中文 `display_name`。旧 `_original_source` 运行依赖已移除，历史 benchmark CSV 已固化到 `benchmarks/model_muti_0529/`。10Y 周度实际方向已写入独立 `t_scheme_weekly_actuals` 表 794 条；历史回测结果已写入独立 `t_backtest_*` 表，前端 API 数据源已包含当前 DB 版本的 `10Y国债活跃 · 周度` 回测格子 run_id=`13`，`68.9% (31/45)`，`5Y国债活跃 · 周度` run_id=`32`，`58.4% (294/503)`，以及 `7Y国债活跃 · 周度` run_id=`34`，`62.8% (27/43)`。详见 [当前状态](docs/CURRENT_STATUS.md)。算法预测使用 conda `forecast_env`，后端/API/调度器使用独立 conda `bond_factor_lab_service`。
...
└── schemes/
    ...
    └── weekly_7y_cross_d_overlay/ # 周度7Y cross-D-overlay adapter与冻结原始脚本
```
...
写库类命令（例如 `scheduler.executor`、`scheduler.daily_actuals_updater`、`scheduler.weekly_actuals_updater`、历史回测 runner）只在明确需要刷新正式表时执行；执行前后应记录受保护表和目标表行数。`scheduler.weekly_actuals_updater` 只写独立 `t_scheme_weekly_actuals`；`backtests.weekly_10y_d_overlay_reproduction`、`backtests.weekly_5y_direct_production_reproduction` 和 `backtests.weekly_7y_cross_d_overlay_reproduction` 只写各自 `scheme_id` 对应的 `t_backtest_*` 回测记录，不写实盘预测表；当前 5Y 周度已受控写入回测 run_id=`32`，7Y 周度已受控写入回测 run_id=`34`，实盘 prediction/run_log/actuals 表未变。`/api/backtests/factor-lab` 已改为只读获取 scheme metadata；`/api/schemes` 仍会同步 scheme registry，不适合作为严格只读数据库保护探针。
