...
- Mac3 的开发 Git 工作区与生产 runtime 根必须分离；R2 已冻结为 `mac3-immutable-r2-20260820`，目标结构固定为 `/Users/macstudio0/bond-factor-lab-production/current -> releases/<release_id>`，运行状态与日志外置到 `/Users/macstudio0/bond-factor-lab-runtime`。2026-08-23 ECS 已晋级到 close-period job 参数修复 release `2394711e1ac5f97b73af8ede4ded5163bb0a254e`，`previous` 为 `00558d175cdffd0d2aae4b51e1aea6e8adda8923`；archive SHA-256 为 `d52c1c56dc3fc5dee053d77c3d62c67e8b01665346126297485cdfa40c057c77`，候选回归、Backend、Dashboard HTTP 和非到期 one-shot 均已通过。Mac3 `current` 仍为 `00558d175cdffd0d2aae4b51e1aea6e8adda8923`、`previous` 为 `bbd7fe7dbf8f3ee7dc74b5d6dbae06af38d98b9d`，数据库 migration 001–020 已精确 `APPLIED`；daily 18:00 close-period monthly plist 因旧 release 的 job 刷新窗口与全局 service 环境冲突而安全回滚到每月 15 日 18:00 的旧入口，数据库、DataBridge 与业务表无副作用。Mac3 后续只能使用上述 ECS 已验证的同一 archive，不能现场重建、修改 release 或放宽 `scripts/run_launchd_release.py` 的全局环境冲突保护。Mac3 七个 installed/loaded plist 已对回滚后的旧稳定模板读回 `ok=true`；launchd 只经 immutable launcher 加载精确 release 环境，现场 authority 仍是 installed plist、`launchctl` 读回与 immutable `current`。
