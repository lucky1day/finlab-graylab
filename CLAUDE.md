...
- Mac3 的开发 Git 工作区与生产 runtime 根必须分离；R1 已冻结为 `mac3-immutable-r1-20260820`，目标结构固定为 `/Users/macstudio0/bond-factor-lab-production/current -> releases/<release_id>`，运行状态与日志外置到 `/Users/macstudio0/bond-factor-lab-runtime`。launchd 只经 `scripts/run_launchd_release.py` 加载安装器生成的精确 release 环境；该精确 archive 已于 2026-08-20 完成 ECS 灰度晋级和读回，但 Mac3 installed plist 尚未切换，Mac3 生产仍运行 `/Users/macstudio0/bond-factor-lab`，现场 authority 仍是 installed plist 与 `launchctl` 读回。只有在新的 Mac3 夜间窗口专项授权、切换和验收完成后，才能把开发 Git 根工作区改到 `codex/develop`。
