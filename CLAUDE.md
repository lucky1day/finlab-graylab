...
- Mac3 的开发 Git 工作区与生产 runtime 根必须分离；R2 已冻结为 `mac3-immutable-r2-20260820`，目标结构固定为 `/Users/macstudio0/bond-factor-lab-production/current -> releases/<release_id>`，运行状态与日志外置到 `/Users/macstudio0/bond-factor-lab-runtime`。2026-08-23 ECS 与 Mac3 的 `current` 均已晋级到周期均值基础 release `00558d175cdffd0d2aae4b51e1aea6e8adda8923`，archive SHA-256 为 `8f408ddb960ea514a45dc8cd9613a4e42515b922f1fc6a3f8015de2ddfeddc12`；两端 `previous` 均为 `bbd7fe7dbf8f3ee7dc74b5d6dbae06af38d98b9d`。Mac3 数据库 migration 001–020 已精确 `APPLIED`，但 daily 18:00 close-period monthly plist 因 job 刷新窗口与全局 service 环境冲突而安全回滚到每月 15 日 18:00 的旧入口；数据库、DataBridge 与业务表无副作用。修复只能从 `codex/develop` 的精确提交构建新 archive，经 ECS 先验证后再由 Mac3 使用同一 archive，不能在 Mac3 现场修改 release 或放宽 `scripts/run_launchd_release.py` 的全局环境冲突保护。Mac3 七个 installed/loaded plist 已对回滚后的旧稳定模板读回 `ok=true`；launchd 只经 immutable launcher 加载精确 release 环境，现场 authority 仍是 installed plist、`launchctl` 读回与 immutable `current`。
