...
- Mac3 的开发 Git 工作区与生产 runtime 根必须分离；R2 已冻结为 `mac3-immutable-r2-20260820`，目标结构固定为 `/Users/macstudio0/bond-factor-lab-production/current -> releases/<release_id>`，运行状态与日志外置到 `/Users/macstudio0/bond-factor-lab-runtime`。Mac3 `current` 仍为 `71a31b0d14bb88d383ea2dc5b495cdc984001461`，`previous` 为 `eca1f0fa9fb6eb8c0d0e1b933634d7940df7ba25`；migration 001–020 已精确 `APPLIED`，monthly plist 每日 18:00 触发并以显式参数使用 `18:00/18:55`，全局 `service.env` 继续保留晨间 `05:30/06:45`，七个 installed/loaded plist drift audit 为 `ok=true`。2026-08-24 ECS 已晋级到 M0 15 方案入库代码 release `859610d09d46af096e3666594267c50297a36a80`，`previous` 为 `e07de5f9f158644d014021e09c3817190559b1b2`；archive SHA-256 为 `27776a88184e1d7c4a2b86018cf47219d2b3bffe6c48ac74f0b11f0771def1ba`，manifest SHA-256 为 `e448bc65a8521f02dcdf969ea2d9e2e98a2ce3702e4d3f0e4d9debb3c3dbb244`，安装后 source-tree SHA-256 为 `65813baf71ae5862e61e58256e9dcdbafd82209882442539266f52d3dbee1c24`。月均、季均、年均各五个 M0 Blackbox V2 方案已达到 Onboarding Complete，ECS active base/composite 为 `79/83`，五个 systemd timer 保持 `enabled/active/waiting`；首次真实 `scheduled_live` 与本轮激活后的首次自然 Actuals 尚待时钟观察，不得由 gray-live 或人工运行冒充。launchd/systemd 只经 immutable launcher 加载精确 release 环境，现场 authority 仍是 installed plist/unit、控制面读回与 immutable `current`。
