...
- 已批准迁移的 Blackbox canonical 目录可临时保留 Native 附件，必须以精确文件清单与摘要绑定候选版本，且只有 Blackbox 执行入口；不允许附件 fallback 或第二 Writer，不放宽普通交付合同。接管与回滚边界验证后删除附件及临时例外。源数据历史修订不触发已发布预测重算或覆盖，只允许更新当前预测必需的内部派生状态。
