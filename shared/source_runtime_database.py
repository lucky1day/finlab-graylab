...
_SOURCE_DATABASE_CONNECT_TIMEOUT_SECONDS = 5
_SOURCE_DATABASE_REQUIRED_TABLES = (
    *FACTOR_SOURCE_TABLES,
    METADATA_SOURCE_TABLE,
    *CALENDAR_SOURCE_TABLES,
)
_MYSQL_GRANT_PATTERN = re.compile(
    r"\AGRANT (?P<privileges>.+?) ON (?P<target>.+?) TO .+\Z",
    re.IGNORECASE,
)
...
class SourceRuntimeDatabasePreflight:
    """source-readonly 运行账号的只读启动预检证据。"""
...
class SourceRuntimeDatabasePreflightError(RuntimeError):
    """source-readonly 启动预检的稳定、脱敏错误。"""
    ...
    def __init__(
        ...
        *,
        ...
        self.code = code
        self.table = table
        message = code if table is None else f"{code}: {table}"
        super().__init__(message)


def preflight_source_runtime_database_access(
    config: SourceRuntimeDatabaseConfig,
) -> SourceRuntimeDatabasePreflight:
    """验证 source 账号只读授权及九张必需源表，不执行任何写探针。"""
    connection = None
    try:
        connection = pymysql.connect(
            ...
            connect_timeout=_SOURCE_DATABASE_CONNECT_TIMEOUT_SECONDS,
            read_timeout=_SOURCE_DATABASE_CONNECT_TIMEOUT_SECONDS,
            write_timeout=_SOURCE_DATABASE_CONNECT_TIMEOUT_SECONDS,
            autocommit=True,
        )
    except Exception:
        raise SourceRuntimeDatabasePreflightError(
            "SOURCE_DB_CONNECT_FAILED"
        ) from None
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if (
                ...
                raise SourceRuntimeDatabasePreflightError(
                    "SOURCE_DB_IDENTITY_INVALID"
                )
            actual_database = str(identity[0] or "")
            authenticated_principal = str(identity[1] or "")
            authenticated_user = authenticated_principal.split("@", 1)[0]
            if (
                actual_database != config.database
                or authenticated_user != config.user
            ):
                raise SourceRuntimeDatabasePreflightError(
                    "SOURCE_DB_IDENTITY_MISMATCH"
                )
            ...
            try:
                ...
                raise SourceRuntimeDatabasePreflightError(
                    "SOURCE_DB_GRANTS_UNREADABLE"
                ) from None
            _validate_source_runtime_database_grants(
                grant_rows,
                database=config.database,
            )

            for table in _SOURCE_DATABASE_REQUIRED_TABLES:
                try:
                    cursor.execute(
                        "SELECT * FROM "
                        f"{_quote_mysql_identifier(table)} LIMIT 0"
                    )
                except Exception:
                    raise SourceRuntimeDatabasePreflightError(
                        "SOURCE_DB_TABLE_UNREADABLE",
                        table=table,
                    ) from None
    finally:
        connection.close()

    return SourceRuntimeDatabasePreflight(
        database=config.database,
        authenticated_user=authenticated_user,
        tables=_SOURCE_DATABASE_REQUIRED_TABLES,
    )


def _validate_source_runtime_database_grants(
    rows: object,
    *,
    database: str,
) -> None:
    """仅接受当前 schema 的直接 SELECT 或全局 USAGE。"""
    if not isinstance(rows, (tuple, list)) or not rows:
        raise SourceRuntimeDatabasePreflightError(
            "SOURCE_DB_GRANTS_UNREADABLE"
        )
    schema_scope = f"{_quote_mysql_identifier(database)}.*"
    table_scopes = {
        (
            f"{_quote_mysql_identifier(database)}."
            f"{_quote_mysql_identifier(table)}"
        )
        for table in _SOURCE_DATABASE_REQUIRED_TABLES
    }
    for row in rows:
        ...
        if (
            " WITH GRANT OPTION" in upper_statement
            or upper_statement.startswith("GRANT PROXY ")
        ):
            raise SourceRuntimeDatabasePreflightError(
                "SOURCE_DB_GRANT_UNSAFE"
            )
        match = _MYSQL_GRANT_PATTERN.fullmatch(statement)
        if match is None:
            # 包括角色授予和未来未知 grant 语法，均保守拒绝。
            raise SourceRuntimeDatabasePreflightError(
                "SOURCE_DB_GRANT_UNSAFE"
            )
        privileges = {
            value.strip().upper()
            for value in match.group("privileges").split(",")
            if value.strip()
        }
        ...
        if (
            privileges == {"SELECT"}
            and target in ({schema_scope} | table_scopes)
        ):
            continue
        raise SourceRuntimeDatabasePreflightError(
            "SOURCE_DB_GRANT_UNSAFE"
