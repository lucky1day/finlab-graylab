...
def test_edit_user_saves_all_changed_fields_in_one_transaction() -> None:
    engine = MagicMock()
    ...
    with (
        ...
        patch.object(repository, "update_password") as update_password,
        patch.object(repository, "revoke_all_sessions") as revoke_sessions,
        patch.object(repository, "insert_audit") as insert_audit,
        patch("backend.auth.service.hash_password", return_value="new-hash"),
        patch("backend.auth.service.verify_password", return_value=False),
    ):
    ...
    assert insert_audit.call_count == 5
    ...
    assert all(
        "Changed123" not in str(item)
        for item in insert_audit.call_args_list
    )
