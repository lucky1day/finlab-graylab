...
class NativeMaintenanceStageRegistryTests(unittest.TestCase):
    def test_stage_all_and_auto_sequence_remain_unchanged(self) -> None:
    ...
    def test_native_maintenance_stage_expands_only_to_its_six_gates(self) -> None:
...
class NativeMaintenanceApiReadinessTests(unittest.TestCase):
    def test_native_maintenance_readiness_accepts_draft_paused_hidden_candidate(self) -> None:
        """维护末段必须允许 active YAML 的激活前 draft/paused 生命周期。"""
        from harness.registry import gates_for_stage
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                root = Path(tmpdir)
                ctx, _config = _prepare_native_maintenance_readiness(root, engine)
                ...
                with patch(
                    ...
                    readiness_gate = gates_for_stage(
                        "native-maintenance", ctx=ctx
                    )[-1]
            ...
            evidence = {item.key: item.value for item in result.evidence}
            self.assertEqual(
                evidence["readiness_profile"],
                "native_maintenance_pre_activation",
            )
            self.assertEqual(evidence["expected_registry_status"], "paused")
            self.assertFalse(evidence["public_factor_lab_visible"])
            self.assertFalse(evidence["public_metrics_visible"])
        finally:
            engine.dispose()

    def test_native_maintenance_readiness_rejects_public_visibility_leaks(self) -> None:
        from harness.registry import gates_for_stage
        ...
        for case, probe_responses, expected_endpoint in cases:
            with self.subTest(case=case):
                engine = _sqlite_engine()
                try:
                    with tempfile.TemporaryDirectory() as tmpdir:
                        root = Path(tmpdir)
                        ctx, _config = _prepare_native_maintenance_readiness(
                            root,
                            engine,
                        )
                        ...
                        with patch(
                            "harness.gates.api_readiness_gate.fetch_json",
                            side_effect=probe_responses,
                        ):
                            result = gates_for_stage(
                                "native-maintenance", ctx=ctx
                            )[-1].run(ctx)

                    self.assertFalse(result.passed)
                    self.assertTrue(
                        any(expected_endpoint in error for error in result.errors),
                        result.errors,
                    )
                finally:
                    engine.dispose()

    def test_native_maintenance_readiness_rejects_non_draft_or_nonpaused_lifecycle(self) -> None:
        from harness.registry import gates_for_stage

        cases = (
            ("active_candidate", "active", "paused"),
            ("active_registry", "draft", "active"),
        )
        for case, candidate_status, registry_status in cases:
            with self.subTest(case=case):
                engine = _sqlite_engine()
                try:
                    with tempfile.TemporaryDirectory() as tmpdir:
    ...
    def test_native_maintenance_readiness_probes_every_composite_metrics_endpoint(self) -> None:
        from harness.registry import gates_for_stage
        ...
        try:
            ...
            evidence = {item.key: item.value for item in result.evidence}
            self.assertEqual(
                evidence["metrics_endpoints"],
                [
                    "http://127.0.0.1:8100/api/metrics/native_daily__h1__5Y",
                    "http://127.0.0.1:8100/api/metrics/native_daily__h1__10Y",
                ],
            )
            self.assertEqual(
                evidence["public_metrics_visibility"],
                {registry_ids[0]: False, registry_ids[1]: True},
            )
            self.assertTrue(
                any(registry_ids[1] in error for error in result.errors),
                result.errors,
            )
        finally:
            engine.dispose()

    def test_native_maintenance_readiness_reloads_canonical_config_before_admission(self) -> None:
        from harness.registry import gates_for_stage
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                root = Path(tmpdir)
                ctx, current = _prepare_native_maintenance_readiness(root, engine)
                stale = replace(
                    current,
                ...
                stale_ctx = replace(ctx, config=stale)
                ...
                with patch(
                    ...
                    result = gates_for_stage(
                        "native-maintenance", ctx=stale_ctx
                    )[-1].run(stale_ctx)

            self.assertFalse(result.passed)
            evidence = {item.key: item.value for item in result.evidence}
            self.assertTrue(evidence["native_maintenance_admission_verified"])
            self.assertTrue(
                any("context config identity mismatch" in error for error in result.errors),
                result.errors,
            )
        finally:
            engine.dispose()

    def test_native_maintenance_readiness_rejects_non_native_or_non_active_raw_config(self) -> None:
        from harness.registry import gates_for_stage

        cases = (
            ("non_native", "blackbox_v2", "active", "runtime_type=native_adapter"),
            ("non_active", "native_adapter", "paused", "status=active"),
        )
        for case, runtime_type, status, expected_error in cases:
            with self.subTest(case=case):
                engine = _sqlite_engine()
                try:
                    with tempfile.TemporaryDirectory() as tmpdir:
                        root = Path(tmpdir)
                        _write_raw_scheme_config(
                            root,
                            runtime_type=runtime_type,
                            status=status,
                        )
    ...
    def test_native_maintenance_readiness_fails_closed_when_admission_fails(self) -> None:
        from harness.registry import gates_for_stage
        ...
        try:
            ...
            self.assertFalse(result.passed)
            self.assertTrue(
                any("api-readiness admission failed" in error for error in result.errors),
                result.errors,
            )
        finally:
            engine.dispose()

    def test_all_and_direct_readiness_keep_ordinary_active_behavior(self) -> None:
        from harness.registry import gate_for_name, gates_for_stage
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                with patch(
                    ...
                    all_result = gates_for_stage("all", ctx=ctx)[-1].run(ctx)
                    direct_result = gate_for_name("api-readiness", ctx=ctx).run(ctx)

            self.assertTrue(all_result.passed, all_result.errors)
            self.assertTrue(direct_result.passed, direct_result.errors)
            self.assertEqual(fetch_json.call_count, 4)
        finally:
            engine.dispose()

    def test_blackbox_all_stage_keeps_its_own_readiness_dispatch(self) -> None:
        from harness.blackbox_v2.gates import BlackboxApiReadinessGate
        from harness.registry import gates_for_stage

        ctx = GateContext(
            scheme_id="blackbox_daily",
            predict_date="2026-08-04",
            project_root=Path("/tmp/blackbox-readiness"),
            report_dir=Path("/tmp/blackbox-readiness/reports"),
            config=SimpleNamespace(runtime_type="blackbox_v2"),
        )

        readiness_gate = gates_for_stage("all", ctx=ctx)[-1]

        self.assertIsInstance(readiness_gate, BlackboxApiReadinessGate)
...
def _write_native_scheme_config(
    root: Path,
    *,
    tenors: tuple[str, ...] = ("10Y",),
    status: str = "active",
):
    """写入可由 scheduler.discovery 重载的最小 Native 配置。"""
    config_path = root / "schemes" / _SCHEME_ID / "config.yaml"
    config_path.parent.mkdir(parents=True, exist_ok=True)
    config_path.write_text(
        "\n".join(_native_scheme_config_lines(tenors=tenors, status=status)),
        encoding="utf-8",
    )
...
def _write_raw_scheme_config(
    root: Path,
    *,
    runtime_type: str,
    status: str,
) -> None:
    """写入仅供 raw-YAML fail-closed 分支使用的配置。"""
    config_path = root / "schemes" / _SCHEME_ID / "config.yaml"
    config_path.parent.mkdir(parents=True, exist_ok=True)
    config_path.write_text(
        "\n".join(
            _native_scheme_config_lines(
                runtime_type=runtime_type,
                status=status,
            )
        ),
...
def _native_scheme_config_lines(
    *,
    runtime_type: str = "native_adapter",
    tenors: tuple[str, ...] = ("10Y",),
    status: str = "active",
    ...
    return [
        f"scheme_id: {_SCHEME_ID}",
        f"runtime_type: {runtime_type}",
        'name: "Native Daily"',
        'description: "Native readiness fixture"',
        "horizon: 1",
        "task_type: T+1",
        f"tenors: {json.dumps(list(tenors))}",
        "frequency: daily",
        "schedule:",
        '  cron: "25 9 * * 1-5"',
...
def _prepare_native_maintenance_readiness(
    root: Path,
    engine,
    *,
    tenors: tuple[str, ...] = ("10Y",),
    candidate_status: str = "draft",
    registry_status: str = "paused",
    include_prior_admission: bool = True,
):
    """构造真实 canonical config 绑定的 Native maintenance readiness 状态。"""
    from harness.gates.native_maintenance_admission_gate import (
    ...
    _write_policy(root, (_SCHEME_ID,))
    config = _write_native_scheme_config(root, tenors=tenors)
    for tenor in tenors:
        _insert_registry_row(
            engine,
            scheme_id=registry_scheme_id(_SCHEME_ID, config.horizon, tenor),
            target_tenor=tenor,
            tenors=json.dumps([tenor]),
            status=registry_status,
            deployed_at=(
                None if registry_status == "paused" else "2026-08-01 10:00:00"
            ),
        )
    if include_prior_admission:
        _seed_prior_admission(
            engine,
    ...
    _seed_api_readiness_backtest(engine)
    return (
        GateContext(
            scheme_id=_SCHEME_ID,
            predict_date="2026-08-04",
...
def _seed_api_readiness_backtest(engine) -> None:
    with engine.begin() as conn:
        conn.execute(
            text(
                """
                ...
                VALUES
                    (120, :scheme_id, 'native-maintenance-benchmark',
                     'framework_db_aligned', '2026-08-04 09:00:00', 'success')
                """
            ),
            {"scheme_id": _SCHEME_ID},
        )
        conn.execute(
            text(
                """
                INSERT INTO t_backtest_predictions (id, run_id, scheme_id)
                VALUES (1, 120, :scheme_id)
                """
            ),
            {"scheme_id": _SCHEME_ID},
        )
