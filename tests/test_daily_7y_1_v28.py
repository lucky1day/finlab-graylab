...
class Daily7Y1ConfigTests(unittest.TestCase):
    """config.yaml 入库契约测试。"""
    ...
    def test_config_declares_auxiliary_inputs_and_benchmark_required(self) -> None:
        ...
        config = load_config_raw(project_root / "schemes" / "daily_7y_1_v28" / "config.yaml")

        self.assertEqual(config["scheme_id"], "daily_7y_1_v28")
        ...
        self.assertEqual(config["backtest"]["runner"], "backtests.daily_7y_1_v28_reproduction")
        self.assertEqual(config["backtest"]["benchmark_id"], "v28_daily_7y_1")


class Daily7Y1AlignmentTests(unittest.TestCase):
    """core 输入对齐和日期语义测试。"""

    def test_make_labels_uses_anchor_to_t_plus_5(self) -> None:
        from schemes.daily_7y_1_v28.core.v28_common import make_labels

        df = pd.DataFrame({"TB7YWI0C": [100.0, 101.0, 102.0, 103.0, 104.0, 99.0, 105.0]})

        labels = make_labels(df, "TB7YWI0C", horizon=5)
    ...
    def test_weekly_alignment_uses_previous_complete_week_only(self) -> None:
        from schemes.daily_7y_1_v28.core.data_alignment import align_weekly_previous_complete
    ...
    def test_monthly_alignment_uses_previous_month_id_without_zfill(self) -> None:
        from schemes.daily_7y_1_v28.core.data_alignment import align_monthly_previous_month
    ...
    def test_backtest_passes_month_window_and_db_calendar_week_mapping_to_shared_inference(self) -> None:
        ...
        daily_df = pd.DataFrame({"date": pd.to_datetime(["2025-01-02", "2025-01-03"]), "TB7YWI0C": [2.0, 2.1]})
...
class Daily7Y1InferenceTests(unittest.TestCase):
    """V28 共享 inference 入口测试。"""

    def test_v28_feature_month_window_uses_feature_month_and_never_future(self) -> None:
        from schemes.daily_7y_1_v28.inference import v28_feature_month_window
    ...
    def test_run_v28_for_feature_date_passes_month_window_to_core_and_selects_exact_row(self) -> None:
        from schemes.daily_7y_1_v28 import inference
        ...
        self.assertEqual(row["model_version"], "7y_1_v28")


class Daily7Y1PredictionRecordTests(unittest.TestCase):
    """predict.py adapter 输出合规性测试。"""

    @patch("schemes.daily_7y_1_v28.predict.run_v28_for_feature_date")
    @patch("schemes.daily_7y_1_v28.predict.build_monthly_input_artifact")
    @patch("schemes.daily_7y_1_v28.predict.build_weekly_input_artifact")
    @patch("schemes.daily_7y_1_v28.predict.build_daily_input_artifact")
    @patch("schemes.daily_7y_1_v28.predict.create_input_engine")
    @patch("schemes.daily_7y_1_v28.predict.get_calendar")
    def test_run_uses_previous_trading_day_as_anchor_and_predict_date_as_signal_date(
        ...
        mock_daily_builder.return_value = SimpleNamespace(
            dataframe=pd.DataFrame({"date": pd.to_datetime(["2026-05-29"]), "TB7YWI0C": [2.0]}),
        ...
        mock_inference.return_value = {
            ...
            "vote_score": -0.72,
            "true_label": None,
            "model_version": "7y_1_v28",
        }

        from schemes.daily_7y_1_v28 import predict

        records = predict.run("2026-06-01")

        self.assertEqual(len(records), 1)
        record = records[0]
        self.assertEqual(record.scheme_id, "daily_7y_1_v28")
...
class Daily7Y1StaticBoundaryTests(unittest.TestCase):
    """确保 active core 不越过项目边界。"""

    def test_core_has_no_db_or_platform_imports(self) -> None:
        project_root = Path(__file__).resolve().parents[1]
        core_files = [
            project_root / "schemes" / "daily_7y_1_v28" / "core" / "data_alignment.py",
            project_root / "schemes" / "daily_7y_1_v28" / "core" / "v28_common.py",
        ]
    ...
    def test_core_has_no_csv_input_fallback(self) -> None:
        project_root = Path(__file__).resolve().parents[1]
        core_path = project_root / "schemes" / "daily_7y_1_v28" / "core" / "v28_common.py"
