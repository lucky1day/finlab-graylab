...
class Weekly7YCrossDOverlay0529BacktestTests(unittest.TestCase):
    ...
    def test_prediction_rows_use_target_date_and_db_calendar(self) -> None:
        ...
        runner._annotate_summary(
            output,
            SimpleNamespace(path=Path("/tmp/weekly_7y.csv"), source="unit_test"),
            weekly_df,
        )
    ...
    def test_missing_source_week_becomes_cross_d_overlay_policy_flat(self) -> None:
        from backtests import weekly_7y_cross_d_overlay_0529_reproduction as runner
            ...
            return pd.DataFrame(
                {
                    ...
                    "cross_d_prob_up": [0.61],
                    "cross_d_overlay": [True],
                    "cross_d_signal_source": ["source"],
                    "main_pred_label": [1],
                    "main_prob_up": [0.61],
                    "d5_d_pred_label": [-1],
                    "d5_d_prob_up": [0.39],
        ...
        self.assertEqual(rows[0]["extra"]["source_component"], "cross_d_overlay")
        self.assertEqual(rows[1]["predicted_direction"], 1)
        self.assertEqual(rows[1]["confidence"], 0.61)
    ...
    def test_empty_core_batch_fails_closed_instead_of_generating_all_flat(self) -> None:
        ...
        weekly_df = _weekly_frame([202601, 202602, 202603])
        with patch.object(runner, "build_cross_d_overlay", return_value=pd.DataFrame()):
            with self.assertRaisesRegex(RuntimeError, "Cross-D core 未产生任何输出"):
