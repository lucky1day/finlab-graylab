"""liwei_0616 10Y_02 SAY 共识方案单元测试。"""
...
class Liwei061610Y02ConfigTests(unittest.TestCase):
    """config.yaml 入库契约测试。"""

    def test_config_declares_10y_t5_lifecycle_auxiliary_inputs_and_benchmark(self) -> None:
        ...
        self.assertEqual(config["name"], "liwei_0616 10Y_02 SAY共识-DIV回退")
        self.assertIn("10Y_02_cons_SAY_k_3_DIV_K_5", config["description"])
        self.assertIn("STD/ACCWT/V55_7Y", config["description"])
        ...
        from schemes.liwei_0616_10y02_cons_say_k3_div_k5.core.v31_common import MONTHLY_COLS

        self.assertIn("M0041342", MONTHLY_COLS)
        self.assertEqual(config["backtest"]["runner"], f"backtests.{SCHEME_ID}_reproduction")
        self.assertEqual(config["backtest"]["runner_args"], ["--batch-mode", "monthly", "--n-workers", "20", "--phase-a-cache"])
        self.assertEqual(config["backtest"]["benchmark_id"], "liwei_0616_10y_02")
        self.assertEqual(config["backtest"]["end_date"], "2026-05-22")
        self.assertTrue(config["backtest"]["benchmark_required"])


class Liwei061610Y02CoreTests(unittest.TestCase):
    """共识、fallback 和 source 配置逻辑测试。"""

    def test_consensus_requires_all_three_say_baselines_to_agree(self) -> None:
        import numpy as np

        from schemes.liwei_0616_10y02_cons_say_k3_div_k5.core.v31_common import apply_consensus

        signs = {
            "STD": np.array([1, 1, -1, -1, 0, 1]),
            "ACCWT": np.array([1, 1, -1, 0, -1, 1]),
            "V55_7Y": np.array([1, -1, -1, -1, -1, 0]),
        }

        result = apply_consensus(signs, ["STD", "ACCWT", "V55_7Y"], k_agree=3, n_rows=6)

        self.assertEqual(result.tolist(), [1, 0, -1, 0, 0, 0])

    def test_build_prediction_uses_div_fallback_after_k5_and_resets_on_direction_change(self) -> None:
        import numpy as np
        ...
        self.assertEqual(PROD_CONFIG["baselines"], ["STD", "ACCWT", "V55_7Y"])
        self.assertEqual(PROD_CONFIG["fallback"], "DIV")
        self.assertEqual(PROD_CONFIG["streak_K"], 5)
        signs = {
            "STD": np.array([1] * 8, dtype=np.int32),
            "DIV": np.array([1] * 5 + [-1, -1, -1], dtype=np.int32),
            "ACCWT": np.array([1] * 8, dtype=np.int32),
            "V55_7Y": np.array([1] * 8, dtype=np.int32),
        }

        result = build_prediction(signs, 8)

        self.assertEqual(result.tolist(), [1] * 5 + [-1, 1, 1])

    def test_required_baselines_does_not_duplicate_div_when_fallback_is_in_vote_set(self) -> None:
        from schemes.liwei_0616_10y02_cons_say_k3_div_k5.core.v31_common import required_baselines

        self.assertEqual(required_baselines(), ["STD", "ACCWT", "V55_7Y", "DIV"])

    def test_lgbm_grid_and_worker_keep_source_training_knobs(self) -> None:
        import numpy as np

        from schemes.liwei_0616_10y02_cons_say_k3_div_k5.core import v31_common
    ...
    def test_weekly_alignment_matches_source_last_trading_day_then_ffill(self) -> None:
        from schemes.liwei_0616_10y02_cons_say_k3_div_k5.core.v31_common import prepare_model_frames
    ...
    def test_signal_universe_keeps_source_7y_cross_tenor_terms(self) -> None:
        import numpy as np

        from schemes.liwei_0616_10y02_cons_say_k3_div_k5.core.v31_common import build_all_signals
        ...
        self.assertIn("7Y_anti_sum20", signals.columns)
        self.assertIn("spr_10Y7Y_anti10", signals.columns)
        self.assertIn("7Y_rv_rev20", signals.columns)
        self.assertIn("tp_7Y1Y_anti20", signals.columns)
        self.assertIn("7Y_lead10Y_lag1", signals.columns)
        self.assertIn("7Y_concord_anti", signals.columns)
        self.assertIn("excess_7Y_anti5", signals.columns)
        self.assertIn("7Y_streak_anti3", signals.columns)
        self.assertIn("7Y_follow10", signals.columns)
        self.assertNotIn("spr_10Y10Y_anti10", signals.columns)
        self.assertNotIn("10Y_lead10Y_lag1", signals.columns)

    def test_report_masks_match_source_fixed_oos_periods(self) -> None:
        from schemes.liwei_0616_10y02_cons_say_k3_div_k5.core.v31_common import _dynamic_report_masks
    ...
    def test_ic_screening_cutoff_remains_source_20240101_when_test_window_moves(self) -> None:
        ...
        daily = pd.DataFrame(
            {
                "date": dates,
                "TB0YWI0C": np.linspace(2.0, 2.2, len(dates)),
            }
        )
...
class Liwei061610Y02InferenceTests(unittest.TestCase):
    """PIT 窗口与共享 inference 入口测试。"""

    def test_feature_window_uses_source_compatible_fixed_history_context(self) -> None:
        from schemes.liwei_0616_10y02_cons_say_k3_div_k5.inference import liwei_0616_pit_window
    ...
    def test_run_for_feature_date_uses_pit_window_and_selects_exact_feature_date(self) -> None:
        from schemes.liwei_0616_10y02_cons_say_k3_div_k5 import inference
        ...
        with patch.object(inference, "run_10y02_for_feature_window", return_value=detail) as mock_run:
            row = inference.run_10y02_for_feature_date(
        ...
        self.assertEqual(row["model_version"], "liwei_0616_10y_02_v61")

    def test_live_incremental_cache_preserves_full_oos_source_window(self) -> None:
        from schemes.liwei_0616_10y02_cons_say_k3_div_k5 import inference
        ...
        audit = {"status": "hit", "watermark": "2026-06-03", "missing_dates": []}
        with (
            patch.object(inference, "prepare_phase_a_caches", return_value=(caches, audit)),
            patch.object(inference, "run_10y02_for_feature_window", return_value=detail) as mock_run,
        ):
            row = inference.run_10y02_for_feature_date(
                daily_df=pd.DataFrame(
                    {"date": pd.to_datetime(["2026-06-03"]), "TB0YWI0C": [2.0], "TB7YWI0C": [2.1]}
                ),
        ...
        self.assertEqual(mock_run.call_args.kwargs["test_ranges"], (("2024-01-01", "2026-06-03"),))
        self.assertEqual(row["phase_a_cache_audit"], audit)


class Liwei061610Y02PredictionRecordTests(unittest.TestCase):
    """predict.py adapter 输出合规性测试。"""

    @patch("schemes.liwei_0616_10y02_cons_say_k3_div_k5.predict.run_10y02_for_feature_date")
    @patch("schemes.liwei_0616_10y02_cons_say_k3_div_k5.predict.build_monthly_input_artifact")
    @patch("schemes.liwei_0616_10y02_cons_say_k3_div_k5.predict.build_weekly_input_artifact")
    @patch("schemes.liwei_0616_10y02_cons_say_k3_div_k5.predict.build_daily_input_artifact")
    @patch("schemes.liwei_0616_10y02_cons_say_k3_div_k5.predict.create_input_engine")
    @patch("schemes.liwei_0616_10y02_cons_say_k3_div_k5.predict.get_calendar")
    def test_run_uses_signal_date_previous_trading_feature_date_and_t5_target_date(
        ...
        mock_daily_builder.return_value = SimpleNamespace(
            dataframe=pd.DataFrame({"date": pd.to_datetime(["2026-06-10"]), "TB0YWI0C": [2.0]}),
        ...
        mock_inference.return_value = {
            ...
            "model_version": "liwei_0616_10y_02_v61",
            "phase_a_cache_audit": {
                "status": "hit",
                "watermark": "2026-06-10",
                "missing_dates": [],
                "version": "liwei_0616.phase_a.v1",
                "fingerprint": "10y-cache",
                "baselines": {},
            },
        }

        from schemes.liwei_0616_10y02_cons_say_k3_div_k5 import predict
        ...
        self.assertEqual(record.extra["source_model_id"], "10Y_02_cons_SAY_k_3_DIV_K_5")
        self.assertEqual(record.extra["vote_baselines"], ["STD", "ACCWT", "V55_7Y"])
        self.assertEqual(record.extra["fallback_baseline"], "DIV")
        self.assertEqual(record.extra["streak_K"], 5)
        ...
        self.assertEqual(record.extra["phase_a_cache_fingerprint"], "10y-cache")
        self.assertTrue(mock_inference.call_args.kwargs["use_incremental_cache"])
        mock_engine.dispose.assert_called_once()


class Liwei061610Y02StaticBoundaryTests(unittest.TestCase):
