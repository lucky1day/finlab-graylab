...
class Weekly10YDOverlay0529BacktestTests(unittest.TestCase):
    ...
    def test_backtest_uses_single_original_batch_overlay_not_pit_slices(self) -> None:
        ...
        def fake_overlay(frame: pd.DataFrame) -> pd.DataFrame:
            return pd.DataFrame(
                {
                    "week_id": frame["week_id"].astype(int).tolist(),
                    "d_pred_label": [1 for _ in range(len(frame))],
                    "d_prob_up": [0.56 for _ in range(len(frame))],
                    "d_model2_overlay": [False for _ in range(len(frame))],
                    "d_signal_source": ["unit" for _ in range(len(frame))],
                    "score_pred_label": [1 for _ in range(len(frame))],
                    "score_prob_up": [0.56 for _ in range(len(frame))],
                    "model2_prob_up": [0.56 for _ in range(len(frame))],
                    "d_model2_pred_label": [1 for _ in range(len(frame))],
                }
            )
    ...
    def test_run_no_persist_returns_sop_payload_without_writes(self) -> None:
        ...
        self.assertIsNone(seed_kwargs["schema_columns"])
