"""liwei_0616 10Y_01 SAY 共识方案单元测试。"""
...
class Liwei061610Y01ConfigTests(unittest.TestCase):
    """config.yaml 入库契约测试。"""
    ...
    def test_config_declares_10y_t5_lifecycle_auxiliary_inputs_and_benchmark(self) -> None:
        ...
        self.assertEqual(config["name"], "liwei_0616 10Y_01 SAY共识-DIV回退")
        self.assertIn("10Y_01_cons_SAY_k_3_DIV_K_10", config["description"])
        ...
        from schemes.liwei_0616_10y01_cons_say_k3_div_k10.core.v31_common import MONTHLY_COLS
...
class Liwei061610Y01CoreTests(unittest.TestCase):
    ...
    def test_build_prediction_uses_div_fallback_after_k10_and_resets_on_direction_change(self) -> None:
        import numpy as np
        ...
        self.assertEqual(PROD_CONFIG["streak_K"], 10)
        signs = {
            "STD": np.array([1] * 13, dtype=np.int32),
            "DIV": np.array([1] * 10 + [-1, -1, -1], dtype=np.int32),
            "ACCWT": np.array([1] * 13, dtype=np.int32),
            "V55_7Y": np.array([1] * 13, dtype=np.int32),
        }

        result = build_prediction(signs, 13)

        self.assertEqual(result.tolist(), [1] * 10 + [-1, 1, 1])

    def test_required_baselines_does_not_duplicate_div_when_fallback_is_in_vote_set(self) -> None:
        from schemes.liwei_0616_10y01_cons_say_k3_div_k10.core.v31_common import required_baselines
    ...
    def test_lgbm_grid_and_worker_keep_source_training_knobs(self) -> None:
        ...
        from schemes.liwei_0616_10y01_cons_say_k3_div_k10.core import v31_common
    ...
    def test_weekly_alignment_matches_source_last_trading_day_then_ffill(self) -> None:
        from schemes.liwei_0616_10y01_cons_say_k3_div_k10.core.v31_common import prepare_model_frames
    ...
    def test_signal_universe_keeps_source_7y_cross_tenor_terms(self) -> None:
        ...
        from schemes.liwei_0616_10y01_cons_say_k3_div_k10.core.v31_common import build_all_signals
    ...
    def test_report_masks_match_source_fixed_oos_periods(self) -> None:
        from schemes.liwei_0616_10y01_cons_say_k3_div_k10.core.v31_common import _dynamic_report_masks
...
class Liwei061610Y01InferenceTests(unittest.TestCase):
    """PIT 窗口与共享 inference 入口测试。"""

    def test_feature_window_uses_source_compatible_fixed_history_context(self) -> None:
        from schemes.liwei_0616_10y01_cons_say_k3_div_k10.inference import liwei_0616_pit_window
    ...
    def test_run_for_feature_date_uses_pit_window_and_selects_exact_feature_date(self) -> None:
        from schemes.liwei_0616_10y01_cons_say_k3_div_k10 import inference
        ...
        with patch.object(inference, "run_10y01_for_feature_window", return_value=detail) as mock_run:
            row = inference.run_10y01_for_feature_date(
        ...
        self.assertEqual(row["model_version"], "liwei_0616_10y_01_v61")

    def test_live_incremental_cache_is_passed_to_two_range_source_window(self) -> None:
        from schemes.liwei_0616_10y01_cons_say_k3_div_k10 import inference
        ...
        with (
            ...
            patch.object(inference, "run_10y01_for_feature_window", return_value=detail) as mock_run,
        ):
            row = inference.run_10y01_for_feature_date(
        ...
        self.assertEqual(
            mock_run.call_args.kwargs["test_ranges"],
            (("2025-06-01", "2025-06-30"), ("2026-06-01", "2026-06-03")),
        )
        self.assertEqual(row["phase_a_cache_audit"], audit)


class Liwei061610Y01PredictionRecordTests(unittest.TestCase):
    """predict.py adapter 输出合规性测试。"""

    @patch("schemes.liwei_0616_10y01_cons_say_k3_div_k10.predict.run_10y01_for_feature_date")
    @patch("schemes.liwei_0616_10y01_cons_say_k3_div_k10.predict.build_monthly_input_artifact")
    @patch("schemes.liwei_0616_10y01_cons_say_k3_div_k10.predict.build_weekly_input_artifact")
    @patch("schemes.liwei_0616_10y01_cons_say_k3_div_k10.predict.build_daily_input_artifact")
    @patch("schemes.liwei_0616_10y01_cons_say_k3_div_k10.predict.create_input_engine")
    @patch("schemes.liwei_0616_10y01_cons_say_k3_div_k10.predict.get_calendar")
    def test_run_uses_signal_date_previous_trading_feature_date_and_t5_target_date(
        ...
        self.assertEqual(record.extra["source_model_id"], "10Y_01_cons_SAY_k_3_DIV_K_10")
        self.assertEqual(record.extra["vote_baselines"], ["STD", "ACCWT", "V55_7Y"])
        self.assertEqual(record.extra["fallback_baseline"], "DIV")
        self.assertEqual(record.extra["streak_K"], 10)
...
class Liwei061610Y01StaticBoundaryTests(unittest.TestCase):
